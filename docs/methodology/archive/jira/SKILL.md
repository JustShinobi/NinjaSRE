---
name: ticketing-jira
display_name: Jira investigation
description: What Jira already knows about a symptom: how many issues match, in what state, and which ones are worth reading before another is opened.
domain: ticketing
applies_when:
  alert_sources: [jira]
  tags: [ticketing, issues, jql]
directs_tools:
...
1. **Shape before detail.** Call `jira_issue_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `jira_recent_issues`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Jira
...
- JQL: `project = CHECKOUT AND status != Done AND created >= -7d ORDER BY created DESC`. Relative dates are the ones worth using during an incident.
- `fields` is what keeps a response small — asking for all fields on a large project returns megabytes of custom fields nobody reads.
- Grouping by `fields.status.name` answers 'is this already being worked'; grouping by `fields.assignee.displayName` answers 'by whom'.
