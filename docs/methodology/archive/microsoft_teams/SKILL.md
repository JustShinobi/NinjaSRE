---
name: communication-microsoft_teams
display_name: Microsoft Teams investigation
description: The Teams channel an incident is being run from: what has been said, and a finding posted where the responders are.
domain: communication
applies_when:
  alert_sources: [microsoft_teams]
...
1. **Shape before detail.** Call `microsoft_teams_recent_messages` over the symptom
...
4. **Read where the counts point.** Only now call
   `microsoft_teams_recent_messages`, narrowed to the group the distribution
...
`microsoft_teams_post_message` changes something outside NinjaSRE. It is
...
- **A question another system answers in one call.** Reaching for Microsoft Teams
...
- The team id is part of the path and is substituted by the proxy, so a capability cannot post into another team.
- `messageType` is `message` or `systemEventMessage`, and grouping by it separates conversation from membership churn.
- `@odata.nextLink` is a full URL rather than a token, and the client follows it as an absolute path while the proxy's allow-list still applies.
