---
name: data_platform-kafka
display_name: Kafka investigation
description: Kafka through its REST Proxy: which topics and consumer groups exist on a cluster, and which groups are not in a stable state.
domain: data_platform
applies_when:
  alert_sources: [kafka]
  tags: [data_platform, streaming, topics]
directs_tools:
...
1. **Shape before detail.** Call `kafka_pipeline_health` over the symptom
...
4. **Read where the counts point.** Only now call
   `kafka_recent_failures`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Kafka
...
- `state` on a consumer group is `STABLE`, `PREPARING_REBALANCE`, `COMPLETING_REBALANCE`, `DEAD`, or `EMPTY`. Anything other than `STABLE` during an incident is usually the incident.
- Topic records carry `partitions_count` and `replication_factor`, which is what an under-replicated partition question starts from.
- The v3 API pages with an opaque `next` URL in `metadata`, which the client passes back as a token.
