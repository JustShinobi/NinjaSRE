---
name: data_platform-dagster
display_name: Dagster investigation
description: Dagster run state through its GraphQL API: which runs are in which status, and the failures behind a stale asset.
domain: data_platform
applies_when:
  alert_sources: [dagster]
  tags: [data_platform, orchestration, assets]
directs_tools:
...
# Dagster investigation

## Order of operations

1. **Shape before detail.** Call `dagster_pipeline_health` over the symptom
...
4. **Read where the counts point.** Only now call
   `dagster_recent_failures`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Dagster
...
- `status` is `SUCCESS`, `FAILURE`, `STARTED`, `QUEUED`, or `CANCELED`; grouping by it is the health question.
- Asset materialisation staleness is a different query entirely and is what a data-freshness alert usually means.
- `runId` links a run here to its logs in whatever the deployment ships them to.
