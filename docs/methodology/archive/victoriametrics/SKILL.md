...
display_name: VictoriaMetrics investigation
description: MetricsQL against VictoriaMetrics and the alerts vmalert is holding, for the estates that use it as a long-term Prometheus store.
domain: metrics
applies_when:
  alert_sources: [victoriametrics]
  tags: [metrics, metrics, metricsql]
directs_tools:
  - victoriametrics_metric_statistics
  - victoriametrics_active_alerts
requires:
  integrations: [victoriametrics]
---
...
1. **Shape before detail.** Call `victoriametrics_metric_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `victoriametrics_active_alerts`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for VictoriaMetrics
...
- MetricsQL adds `rollup_rate`, `default`, and subquery syntax that PromQL lacks; everything PromQL can express is accepted unchanged.
- `step` decides the point count and is what a 'too many points' refusal is about.
- Retention here is usually months rather than the days a Prometheus keeps, which is the reason to reach for it during a post-incident review.
