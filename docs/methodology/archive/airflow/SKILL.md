---
name: data_platform-airflow
display_name: Airflow investigation
description: Airflow's scheduler state: which DAG runs are in which state, and the task instances that failed, which is where a data-freshness incident starts.
domain: data_platform
applies_when:
  alert_sources: [airflow]
  tags: [data_platform, orchestration, dags]
directs_tools:
...
1. **Shape before detail.** Call `airflow_pipeline_health` over the symptom
...
4. **Read where the counts point.** Only now call
   `airflow_recent_failures`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Airflow
...
- `state` is `queued`, `running`, `success`, or `failed`; grouping by it answers 'is the platform behind or broken', which are different problems.
- `start_date_gte` is ISO 8601 with a timezone, and a naive timestamp is rejected.
- A DAG run that is `running` long past its schedule is the signature of a stuck sensor, which no failure count will show.
