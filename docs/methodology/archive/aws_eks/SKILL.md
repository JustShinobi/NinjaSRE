---
name: cloud_control_plane-aws_eks
display_name: AWS EKS investigation
description: The EKS control plane: which clusters this account runs, their version and status, and the cluster updates that have been applied to them.
domain: cloud_control_plane
applies_when:
  alert_sources: [aws_eks]
  tags: [cloud_control_plane, aws, kubernetes]
directs_tools:
  - aws_eks_resource_inventory
  - aws_eks_recent_changes
requires:
  integrations: [aws_eks]
---
...
1. **Shape before detail.** Call `aws_eks_resource_inventory` over the symptom
...
4. **Read where the counts point.** Only now call
   `aws_eks_recent_changes`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for AWS EKS
...
- `nextToken` is EKS's own continuation token and is opaque; it expires, so a walk that is paused and resumed starts again rather than continuing.
- The signing service name is `eks`, and a request signed for another service is rejected with a message that blames the key rather than the scope.
- Cluster names are unique per account and region, not globally.
