---
name: ticketing-trello
display_name: Trello investigation
description: What a Trello board is holding: the cards on it, which list each is in, and which were touched most recently.
domain: ticketing
applies_when:
  alert_sources: [trello]
  tags: [ticketing, cards, boards]
directs_tools:
...
# Trello investigation

## Order of operations

1. **Shape before detail.** Call `trello_issue_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `trello_recent_issues`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Trello
...
- `fields` keeps a card small; without it every card carries its whole badge and label structure.
- `dateLastActivity` is ISO 8601 and is the only ordering signal a card carries.
- Board ids are 24-character hex strings and appear in the board URL.
