---
name: cloud_control_plane-aws_s3
display_name: AWS S3 investigation
description: What is in the bucket this team configured: the objects and their storage class, and the version history, which is the closest S3 has to a change log.
domain: cloud_control_plane
applies_when:
  alert_sources: [aws_s3]
  tags: [cloud_control_plane, aws, storage]
directs_tools:
...
1. **Shape before detail.** Call `aws_s3_resource_inventory` over the symptom
...
4. **Read where the counts point.** Only now call
   `aws_s3_recent_changes`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for AWS S3
...
- `list-type=2` is the current listing API; the version-one shape has different field names and a different continuation parameter.
- `prefix` is the only server-side filter, and it is a literal prefix rather than a glob.
- `StorageClass` grouping answers the question S3 is most often asked during a cost or restore incident: how much is in Glacier and therefore not immediately readable.
