---
name: metrics-new_relic
display_name: New Relic investigation
description: NRQL over New Relic's telemetry, and the alert violations currently open, for the accounts whose metrics and events live there.
domain: metrics
applies_when:
  alert_sources: [new_relic]
  tags: [metrics, metrics, nrql]
directs_tools:
...
1. **Shape before detail.** Call `new_relic_metric_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `new_relic_active_alerts`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for New Relic
...
- `filter[name]` is a substring match on the application name and is the only server-side narrowing the applications endpoint offers.
- `health_status` is `green`, `orange`, `red`, or `gray`; `gray` means the agent has not reported, which is a different problem from a failing service.
- The alert violations endpoint is page-numbered from one and caps at 25 per page.
