...
display_name: Honeycomb investigation
description: Honeycomb's query engine over trace events: where latency and errors concentrate in a dataset, and the slowest traces behind that concentration.
domain: tracing
applies_when:
  alert_sources: [honeycomb]
...
1. **Shape before detail.** Call `honeycomb_trace_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `honeycomb_slow_traces`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Honeycomb
...
- Grouping by `name` separates operations; grouping by `service.name` separates services, and which one concentrates the latency is the whole question.
- `duration_ms` is the field every span carries, and sorting by it is what makes the second capability exemplars rather than a sample.
- `trace.trace_id` links an exemplar here to the same trace in any other tool that speaks OpenTelemetry.
