---
name: ticketing-notion
display_name: Notion investigation
description: What the team has written in Notion: the pages matching a search, and the ones most recently edited.
domain: ticketing
applies_when:
  alert_sources: [notion]
...
1. **Shape before detail.** Call `notion_issue_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `notion_recent_issues`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Notion
...
- `object` is `page` or `database`, and grouping by it separates documents from the structures holding them.
- `last_edited_time` is what says whether a runbook is current, and it belongs in any finding that cites one.
- Page titles live inside `properties`, whose shape depends on the parent database, which is why the search result rather than the title is what gets reported.
