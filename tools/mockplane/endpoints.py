...
CONSOLE_ENDPOINTS: Final[tuple[ConsoleEndpoint, ...]] = (
    ...
    ConsoleEndpoint(
        method="GET",
        path="/v1/transit/ingress",
        slug="transit-ingress",
        source=_GATEWAY,
        summary="what each receiver has delivered, and which of them never has",
        records_key="sources",
    ),
    ConsoleEndpoint(
        method="GET",
        path="/v1/transit/rules",
        slug="transit-rules",
        source=_GATEWAY,
        summary="the ordered routing rules, with the catch-all last",
        records_key="rules",
    ),
    ConsoleEndpoint(
        method="GET",
        path="/v1/transit/destinations",
        slug="transit-destinations",
        source=_GATEWAY,
        summary="where results go, and the masking policy each one applies",
        records_key="destinations",
    ),
    ConsoleEndpoint(
        method="GET",
        path="/v1/transit/deliveries",
        slug="transit-deliveries",
        source=_GATEWAY,
        summary="what crossed the boundary lately, in either direction",
    ),
    ConsoleEndpoint(
        method="POST",
        path="/v1/transit/simulate",
        slug="transit-simulate",
        source=_GATEWAY,
        summary="what today's rules would do with this payload, before anything is saved",
    ),
    ConsoleEndpoint(
        method="POST",
        ...
        slug="transit-resend",
        source=_GATEWAY,
        summary="send a failed outbound delivery again, because a person asked",
    ),
    ConsoleEndpoint(
        method="GET",
