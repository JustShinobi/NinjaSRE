"""Proves a Microsoft Teams credential works, and proves what it is allowed to do.
...
#: What each failure means for Microsoft Teams, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Microsoft Teams rejected the credential. Re-issue it: Entra ID → App registrations → your application → Certificates & secrets, exchanged for a token against https://graph.microsoft.com"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Microsoft Teams credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Entra ID → App registrations → your application → Certificates & secrets, exchanged for a token against https://graph.microsoft.com"
...
_NO_INTROSPECTION: Final = (
    "Microsoft Teams has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="ChannelMessage.Read.All",
    grants="read channel messages",
    capabilities=("microsoft_teams_recent_messages",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="ChannelMessage.Send",
    grants="post a message to a channel",
    capabilities=("microsoft_teams_post_message",),
    where=_GRANTED_AT,
)
...
class MicrosoftTeamsVerifier:
    """Checks a stored Microsoft Teams credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v1.0/teams/{team_id} — the cheapest read Microsoft Teams offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Microsoft Teams accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Microsoft Teams accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Microsoft Teams said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> MicrosoftTeamsClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a microsoft_teams probe needs a proxy transport and a request context")
        return MicrosoftTeamsClient(transport=transport, context=context, region=self.region)
