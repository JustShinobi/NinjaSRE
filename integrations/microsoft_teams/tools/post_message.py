"""Microsoft Teams: the one change this integration is allowed to make.
...
TOOL_NAME = "microsoft_teams_post_message"
...
class MicrosoftTeamsPostMessageRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the Microsoft Teams change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what Microsoft Teams returned for {target}, including its identifier.",
                "Undo the change in Microsoft Teams using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="Microsoft Teams post message",
    description=(
    ...
    rollback_planner=MicrosoftTeamsPostMessageRollback(),
    tags=(
        "communication",
        "microsoft_teams",
        "write",
...
async def microsoft_teams_post_message(channel: str, text: str) -> CapabilityResult:
    """Make the change in Microsoft Teams and return what it said."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"Microsoft Teams accepted the change to {channel!r}",
                reference=f"microsoft_teams:post_message:{channel}",
            ),
        ),
    )
