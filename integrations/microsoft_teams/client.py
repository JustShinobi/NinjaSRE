"""Microsoft Teams's API, over the proxy, holding no credential.
...
Microsoft Teams's authentication changed tomorrow, because the authentication is
...
from integrations.microsoft_teams.schema import INTEGRATION, base_url

PING_PATH: Final = "/v1.0/teams/{team_id}"
RECENT_MESSAGES_PATH: Final = "/v1.0/teams/{team_id}/channels/messages"
POST_MESSAGE_PATH: Final = "/v1.0/teams/{team_id}/channels/messages"
...
class MicrosoftTeamsClient(IntegrationClient):
    """Microsoft Teams, reached through the credential proxy."""
    ...
    async def recent_messages(
        ...
        *,
        ...
        """Return what Microsoft Teams holds for this request, bounded.
        ...
        """
        # Microsoft Teams narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"$filter": channel}
            asked.update(parameters)
            answer = (await self.get(RECENT_MESSAGES_PATH, params=asked)).json()
            return Page(
                items=records(answer, "value"), cursor=text(answer, "@odata", "nextLink") or None
            )
    ...
    async def post_message(self, channel: str, text: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Microsoft Teams said."""
        body: dict[str, Any] = {
            "body": {"content": text},
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Microsoft Teams offers.
