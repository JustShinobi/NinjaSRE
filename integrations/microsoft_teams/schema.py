"""Microsoft Teams's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "microsoft_teams"


#: Every host Microsoft Teams serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="graph.microsoft.com", name="global")
...
SCHEMA: Final = credential_schema(
    ...
    secret(
        "token",
        "Microsoft Graph access token for an application with channel permissions",
        min_length=8,
    ),
    public("team_id", "Teams group id the channel belongs to", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="team_id", field="team_id"),
    ),
)
