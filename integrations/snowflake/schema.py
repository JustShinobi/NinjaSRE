"""Snowflake's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "snowflake"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "acme-prod.snowflakecomputing.com"

#: Every host Snowflake serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(
    INTEGRATION, host="acme-prod.snowflakecomputing.com", name="account"
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Snowflake key-pair JWT or OAuth access token", min_length=8),
    public("account", "Snowflake account identifier", required=True),
    public("warehouse", "Warehouse the statements run on"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Snowflake is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="snowflake.acme.example")``.
    """
    named = {"account": DEFAULT_HOST, **endpoints}
    return RegionMap(
        integration=INTEGRATION,
