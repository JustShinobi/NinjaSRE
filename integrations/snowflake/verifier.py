"""Proves a Snowflake credential works, and proves what it is allowed to do.
...
from integrations.snowflake.client import SnowflakeClient
from integrations.snowflake.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Snowflake, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Snowflake rejected the credential. Re-issue it: Snowflake → Admin → Users, with a key pair registered, exchanged for a JWT"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Snowflake credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Snowflake → Admin → Users, with a key pair registered, exchanged for a JWT"
...
_NO_INTROSPECTION: Final = (
    "Snowflake has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="MONITOR on the account",
    grants="read query history",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="USAGE on the warehouse",
    grants="run the statements that read it",
    capabilities=(
        "snowflake_session_statistics",
        "snowflake_slow_queries",
    ),
...
class SnowflakeVerifier:
    """Checks a stored Snowflake credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v2/statements — the cheapest read Snowflake offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Snowflake accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Snowflake accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Snowflake said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> SnowflakeClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a snowflake probe needs a proxy transport and a request context")
        return SnowflakeClient(transport=transport, context=context, region=self.region)
