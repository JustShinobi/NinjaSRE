"""Counting Snowflake's sessions before reading any of them.

The first call of an investigation that reaches Snowflake, and the one
...
Source of truth: Snowflake's ``/api/v2/statements``, read through the
...
TOOL_NAME = "snowflake_session_statistics"
...
DEFAULT_GROUP_BY = "EXECUTION_STATUS"
...
@tool(
    name=TOOL_NAME,
    display_name="Snowflake session statistics",
    description=(
...
async def snowflake_session_statistics(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Snowflake's sessions across ``group_by``.

    ``start`` and ``end`` are Snowflake's own time expressions. Both are
    ...
    client = access.client(SnowflakeClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"snowflake:session_statistics:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no sessions were returned by Snowflake for this request"
