"""Reading a capped number of Snowflake's statements, after something narrowed the question.
...
Source of truth: Snowflake's ``/api/v2/statements``.
"""
...
TOOL_NAME = "snowflake_slow_queries"
...
@tool(
    name=TOOL_NAME,
    display_name="Snowflake slow queries",
    description=(
...
async def snowflake_slow_queries(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Snowflake's statements, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.ANALYSIS,
                summary=f"{len(entries)} statements from Snowflake{tail}",
                reference=f"snowflake:slow_queries:{scope or 'all'}",
            ),
        ),
    )
