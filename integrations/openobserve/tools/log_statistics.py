"""Counting OpenObserve's log lines before reading any of them.

The first call of an investigation that reaches OpenObserve, and the one
...
Source of truth: OpenObserve's ``/api/default/_search``, read through the
...
TOOL_NAME = "openobserve_log_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="OpenObserve log statistics",
    description=(
...
async def openobserve_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of OpenObserve's log lines across ``group_by``.

    ``start`` and ``end`` are OpenObserve's own time expressions. Both are
    ...
    client = access.client(OpenobserveClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"openobserve:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by OpenObserve for this request"
