"""Reading a capped number of OpenObserve's log lines, after something narrowed the question.
...
Source of truth: OpenObserve's ``/api/default/_search``.
"""
...
TOOL_NAME = "openobserve_sample_logs"
...
@tool(
    name=TOOL_NAME,
    display_name="OpenObserve log samples",
    description=(
...
async def openobserve_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of OpenObserve's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from OpenObserve{tail}",
                reference=f"openobserve:sample_logs:{query or 'all'}",
            ),
        ),
    )
