"""OpenObserve's API, over the proxy, holding no credential.
...
OpenObserve's authentication changed tomorrow, because the authentication is
...
from integrations.openobserve.schema import base_url as _region_url

PING_PATH: Final = "/api/default/_health"
SEARCH_LOGS_PATH: Final = "/api/default/_search"
RECENT_LOGS_PATH: Final = "/api/default/_search"
...
class OpenobserveClient(IntegrationClient):
    """OpenObserve, reached through the credential proxy."""
    ...
    def __init__(
        ...
        *,
        ...
        # OpenObserve is. The egress allow-list still decides whether the host may
        # be reached.
        super().__init__(
            transport=transport,
            context=context,
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what OpenObserve holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(items=records(answer, "hits"), cursor="" or None)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call OpenObserve offers.
