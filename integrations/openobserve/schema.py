"""OpenObserve's credential shape, hosts, and how its secret enters a request.
...
DEFAULT_HOST: Final = "openobserve.example.com"

#: Every host OpenObserve serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="openobserve.example.com", name="self-hosted")
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    OpenObserve is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="openobserve.acme.example")``.
    """
