"""MongoDB Atlas's API, over the proxy, holding no credential.
...
MongoDB Atlas's authentication changed tomorrow, because the authentication is
...
from integrations.mongodb_atlas.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/atlas/v2/groups/{group}"
LIST_SESSIONS_PATH: Final = "/api/atlas/v2/groups/{group}/processes"
SLOW_QUERIES_PATH: Final = "/api/atlas/v2/groups/{group}/clusters"
...
PAGINATION: Final[tuple[EndpointPagination, ...]] = (
    EndpointPagination(
        ...
        parameter="pageNum",
        page_size_parameter="itemsPerPage",
        page_size=100,
    ),
    EndpointPagination(
...
class MongodbAtlasClient(IntegrationClient):
    """MongoDB Atlas, reached through the credential proxy."""
    ...
    async def list_sessions(
        ...
        *,
        ...
        """Return what MongoDB Atlas holds for this request, bounded.
        ...
        """
        # MongoDB Atlas narrows this endpoint by other means; the argument stays in the
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call MongoDB Atlas offers.
        ...
        """
        return await self.get(PING_PATH, params={"itemsPerPage": "1"})
