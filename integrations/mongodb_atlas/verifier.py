"""Proves a MongoDB Atlas credential works, and proves what it is allowed to do.
...
#: What each failure means for MongoDB Atlas, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "MongoDB Atlas rejected the credential. Re-issue it: Atlas → Organization Access Manager → API Keys, with Project Read Only"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No MongoDB Atlas credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Atlas → Organization Access Manager → API Keys, with Project Read Only"
...
_NO_INTROSPECTION: Final = (
    "MongoDB Atlas has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="Project Read Only",
    grants="read processes and clusters in the project",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="Project Monitoring Admin",
    grants="read process measurements and the performance advisor",
    capabilities=("mongodb_atlas_slow_queries",),
    where=_GRANTED_AT,
)
...
class MongodbAtlasVerifier:
    """Checks a stored MongoDB Atlas credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/atlas/v2/groups/{group} — the cheapest read MongoDB Atlas offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether MongoDB Atlas accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="MongoDB Atlas accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what MongoDB Atlas said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> MongodbAtlasClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a mongodb_atlas probe needs a proxy transport and a request context")
        return MongodbAtlasClient(transport=transport, context=context, region=self.region)
