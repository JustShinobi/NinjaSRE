"""Counting MongoDB Atlas's sessions before reading any of them.

The first call of an investigation that reaches MongoDB Atlas, and the one
...
Source of truth: MongoDB Atlas's ``/api/atlas/v2/groups/{group}/processes``, read through the
...
TOOL_NAME = "mongodb_atlas_session_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="MongoDB Atlas session statistics",
    description=(
...
async def mongodb_atlas_session_statistics(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of MongoDB Atlas's sessions across ``group_by``.

    ``start`` and ``end`` are MongoDB Atlas's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"mongodb_atlas:session_statistics:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no sessions were returned by MongoDB Atlas for this request"
