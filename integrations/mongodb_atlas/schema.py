"""MongoDB Atlas's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "mongodb_atlas"


#: Every host MongoDB Atlas serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="cloud.mongodb.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "Atlas API public key", min_length=8),
    secret("password", "Atlas API private key", min_length=8),
    public("group", "Atlas project (group) id", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="group", field="group"),
    ),
)
