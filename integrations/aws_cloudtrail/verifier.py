"""Proves a AWS CloudTrail credential works, and proves what it is allowed to do.
...
from integrations.aws_cloudtrail.client import AwsCloudtrailClient
from integrations.aws_cloudtrail.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for AWS CloudTrail, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS CloudTrail rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS CloudTrail credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "AWS CloudTrail has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="cloudtrail:LookupEvents",
    grants="read the management-event history",
    capabilities=(
...
class AwsCloudtrailVerifier:
    """Checks a stored AWS CloudTrail credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls / — the cheapest read AWS CloudTrail offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS CloudTrail accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS CloudTrail accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS CloudTrail said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsCloudtrailClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_cloudtrail probe needs a proxy transport and a request context")
        return AwsCloudtrailClient(transport=transport, context=context, region=self.region)


__all__ = [
    "PERMISSION_1",
    "PERMISSIONS",
    "AwsCloudtrailVerifier",
]
