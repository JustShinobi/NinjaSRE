"""AWS CloudTrail's API, over the proxy, holding no credential.
...
AWS CloudTrail's authentication changed tomorrow, because the authentication is
...
from integrations.aws_cloudtrail.schema import INTEGRATION, base_url
...
class AwsCloudtrailClient(IntegrationClient):
    """AWS CloudTrail, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what AWS CloudTrail holds for this request, bounded.
        ...
        """
        # AWS CloudTrail narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {"MaxResults": 50, "StartTime": start, "EndTime": end}
            ...
            return Page(items=records(answer, "Events"), cursor=text(answer, "NextToken") or None)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call AWS CloudTrail offers.
