"""Counting AWS CloudTrail's resources before reading any of them.

The first call of an investigation that reaches AWS CloudTrail, and the one
...
Source of truth: AWS CloudTrail's ``/``, read through the
...
TOOL_NAME = "aws_cloudtrail_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS CloudTrail resource inventory",
    description=(
...
async def aws_cloudtrail_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of AWS CloudTrail's resources across ``group_by``.

    ``start`` and ``end`` are AWS CloudTrail's own time expressions. Both are
    ...
    client = access.client(AwsCloudtrailClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"aws_cloudtrail:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by AWS CloudTrail for this request"
