"""Reading a capped number of AWS CloudTrail's changes, after something narrowed the question.
...
Source of truth: AWS CloudTrail's ``/``.
"""
...
TOOL_NAME = "aws_cloudtrail_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS CloudTrail recent changes",
    description=(
...
async def aws_cloudtrail_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of AWS CloudTrail's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from AWS CloudTrail{tail}",
                reference=f"aws_cloudtrail:recent_changes:{kind or 'all'}",
            ),
        ),
    )
