"""PagerDuty: the one change this integration is allowed to make.
...
TOOL_NAME = "pagerduty_acknowledge_incident"
...
class PagerdutyAcknowledgeIncidentRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the PagerDuty change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what PagerDuty returned for {target}, including its identifier.",
                "Undo the change in PagerDuty using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="PagerDuty acknowledge incident",
    description=(
    ...
    rollback_planner=PagerdutyAcknowledgeIncidentRollback(),
    tags=(
        "incident",
        "pagerduty",
        "write",
...
async def pagerduty_acknowledge_incident(incident: str, note: str) -> CapabilityResult:
    """Make the change in PagerDuty and return what it said."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"PagerDuty accepted the change to {incident!r}",
                reference=f"pagerduty:acknowledge_incident:{incident}",
            ),
        ),
    )
