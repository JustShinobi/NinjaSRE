"""Counting PagerDuty's incidents before reading any of them.

The first call of an investigation that reaches PagerDuty, and the one
...
Source of truth: PagerDuty's ``/incidents``, read through the
...
TOOL_NAME = "pagerduty_incident_statistics"
...
DEFAULT_GROUP_BY = "service.summary"
...
@tool(
    name=TOOL_NAME,
    display_name="PagerDuty incident statistics",
    description=(
...
async def pagerduty_incident_statistics(
    status: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of PagerDuty's incidents across ``group_by``.

    ``start`` and ``end`` are PagerDuty's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"pagerduty:incident_statistics:{status or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no incidents were returned by PagerDuty for this request"
