"""Proves a PagerDuty credential works, and proves what it is allowed to do.
...
#: What each failure means for PagerDuty, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "PagerDuty rejected the credential. Re-issue it: PagerDuty → Integrations → API Access Keys, or a user token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No PagerDuty credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "PagerDuty → Integrations → API Access Keys, or a user token"
...
_NO_INTROSPECTION: Final = (
    "PagerDuty has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="incidents.read",
    grants="read incidents and their log entries",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="acknowledge an incident",
    capabilities=("pagerduty_acknowledge_incident",),
    where=_GRANTED_AT,
)
...
class PagerdutyVerifier:
    """Checks a stored PagerDuty credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /abilities — the cheapest read PagerDuty offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether PagerDuty accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="PagerDuty accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what PagerDuty said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> PagerdutyClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a pagerduty probe needs a proxy transport and a request context")
        return PagerdutyClient(transport=transport, context=context, region=self.region)
