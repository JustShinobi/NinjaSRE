"""PagerDuty's API, over the proxy, holding no credential.
...
PagerDuty's authentication changed tomorrow, because the authentication is
...
from integrations.pagerduty.schema import INTEGRATION, base_url

PING_PATH: Final = "/abilities"
LIST_INCIDENTS_PATH: Final = "/incidents"
INCIDENT_TIMELINE_PATH: Final = "/incidents"
ACKNOWLEDGE_PATH: Final = "/incidents"
...
class PagerdutyClient(IntegrationClient):
    """PagerDuty, reached through the credential proxy."""
    ...
    async def list_incidents(
        ...
        *,
        ...
        """Return what PagerDuty holds for this request, bounded.
        ...
        """
        # PagerDuty narrows this endpoint by other means; the argument stays in the
    ...
    async def incident_timeline(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"statuses[]": "triggered", "include[]": "acknowledgers"}
            asked.update(parameters)
    ...
    async def acknowledge(self, incident: str, note: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what PagerDuty said."""
        body: dict[str, Any] = {
            "incidents": [{"id": incident, "type": "incident_reference", "status": "acknowledged"}],
            "note": note,
        }
        answer = (await self.put(ACKNOWLEDGE_PATH, json_body=body)).json()
        return dict(answer) if isinstance(answer, dict) else {"result": answer}

    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call PagerDuty offers.
