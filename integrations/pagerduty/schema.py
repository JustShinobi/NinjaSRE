"""PagerDuty's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "pagerduty"


#: Every host PagerDuty serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.pagerduty.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "PagerDuty REST API token, user-scoped or account-scoped", min_length=8),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        HeaderInjection(header="Authorization", field="token", template="Token token={value}"),
    ),
)
