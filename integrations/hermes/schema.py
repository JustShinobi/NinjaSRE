"""Hermes's credential shape, hosts, and how its secret enters a request.
...
DEFAULT_HOST: Final = "hermes.example.com"

#: Every host Hermes serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="hermes.example.com", name="self-hosted")
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(HeaderInjection(header="X-Hermes-Token", field="api_key", template="{value}"),),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Hermes is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="hermes.acme.example")``.
    """
