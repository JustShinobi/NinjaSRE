"""Proves a Hermes credential works, and proves what it is allowed to do.
...
#: What each failure means for Hermes, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Hermes rejected the credential. Re-issue it: the Hermes console → Access tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Hermes credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "the Hermes console → Access tokens"
...
_NO_INTROSPECTION: Final = (
    "Hermes has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="logs:read",
    grants="search and tail the workspace's streams",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="streams:list",
    grants="list streams, which the connectivity probe uses",
    capabilities=("hermes_log_statistics",),
    where=_GRANTED_AT,
)
...
class HermesVerifier:
    """Checks a stored Hermes credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v1/streams — the cheapest read Hermes offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Hermes accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Hermes accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Hermes said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> HermesClient:
