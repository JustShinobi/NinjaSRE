"""Counting Hermes's log lines before reading any of them.

The first call of an investigation that reaches Hermes, and the one
...
Source of truth: Hermes's ``/v1/logs/search``, read through the
...
TOOL_NAME = "hermes_log_statistics"
...
DEFAULT_GROUP_BY = "classification"
...
@tool(
    name=TOOL_NAME,
    display_name="Hermes log statistics",
    description=(
...
async def hermes_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Hermes's log lines across ``group_by``.

    ``start`` and ``end`` are Hermes's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"hermes:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by Hermes for this request"
