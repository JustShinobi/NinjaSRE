"""Hermes's API, over the proxy, holding no credential.
...
Hermes's authentication changed tomorrow, because the authentication is
...
from integrations.hermes.schema import base_url as _region_url

PING_PATH: Final = "/v1/streams"
SEARCH_LOGS_PATH: Final = "/v1/logs/search"
RECENT_LOGS_PATH: Final = "/v1/logs/search"
...
class HermesClient(IntegrationClient):
    """Hermes, reached through the credential proxy."""
    ...
    def __init__(
        ...
        *,
        ...
        # Hermes is. The egress allow-list still decides whether the host may
        # be reached.
        super().__init__(
            transport=transport,
            context=context,
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what Hermes holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {"query": query, "from": start, "to": end}
            body.update(parameters)
            answer = (await self.post(SEARCH_LOGS_PATH, json_body=body)).json()
            return Page(
                items=records(answer, "entries"), cursor=text(answer, "next_cursor") or None
            )
    ...
    async def recent_logs(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {"query": query, "from": start, "to": end, "order": "desc"}
            body.update(parameters)
            answer = (await self.post(RECENT_LOGS_PATH, json_body=body)).json()
            return Page(
                items=records(answer, "entries"), cursor=text(answer, "next_cursor") or None
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Hermes offers.
