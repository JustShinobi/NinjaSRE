"""Dagster's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "dagster"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "dagster.example.com"

#: Every host Dagster serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="dagster.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    ...
    secret(
        "token",
        "Dagster Cloud user token, or the token your self-hosted webserver accepts",
        min_length=8,
    ),
    public("deployment", "Dagster Cloud deployment name"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Dagster is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="dagster.acme.example")``.
    """
