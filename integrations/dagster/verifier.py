"""Proves a Dagster credential works, and proves what it is allowed to do.
...
from integrations.dagster.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Dagster, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Dagster rejected the credential. Re-issue it: Dagster Cloud → Organization Settings → Tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Dagster credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Dagster Cloud → Organization Settings → Tokens"
...
_NO_INTROSPECTION: Final = (
    "Dagster has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="runs:read",
    grants="read run status through the GraphQL API",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="version:read",
    grants="read the version, which the probe uses",
    capabilities=("dagster_pipeline_health",),
    where=_GRANTED_AT,
)
...
class DagsterVerifier:
    """Checks a stored Dagster credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /graphql — the cheapest read Dagster offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Dagster accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Dagster accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Dagster said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> DagsterClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a dagster probe needs a proxy transport and a request context")
        return DagsterClient(transport=transport, context=context, region=self.region)
