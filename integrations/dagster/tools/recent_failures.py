"""Reading a capped number of Dagster's failures, after something narrowed the question.
...
Source of truth: Dagster's ``/graphql``.
"""
...
TOOL_NAME = "dagster_recent_failures"
...
@tool(
    name=TOOL_NAME,
    display_name="Dagster recent failures",
    description=(
...
async def dagster_recent_failures(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Dagster's failures, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} failures from Dagster{tail}",
                reference=f"dagster:recent_failures:{scope or 'all'}",
            ),
        ),
    )
