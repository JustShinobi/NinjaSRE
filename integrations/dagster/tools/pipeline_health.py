"""Counting Dagster's tasks before reading any of them.

The first call of an investigation that reaches Dagster, and the one
...
Source of truth: Dagster's ``/graphql``, read through the
...
TOOL_NAME = "dagster_pipeline_health"
...
@tool(
    name=TOOL_NAME,
    display_name="Dagster pipeline health",
    description=(
...
async def dagster_pipeline_health(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Dagster's tasks across ``group_by``.

    ``start`` and ``end`` are Dagster's own time expressions. Both are
    ...
    client = access.client(DagsterClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"dagster:pipeline_health:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no tasks were returned by Dagster for this request"
