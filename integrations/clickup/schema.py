"""ClickUp's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "clickup"


#: Every host ClickUp serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.clickup.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("api_key", "ClickUp personal API token", min_length=8),
    public("list_id", "ClickUp list id tasks are read from", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        HeaderInjection(header="Authorization", field="api_key", template="{value}"),
        PathSegmentInjection(placeholder="list_id", field="list_id"),
    ),
)
