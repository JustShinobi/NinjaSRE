"""Proves a ClickUp credential works, and proves what it is allowed to do.
...
from integrations.clickup.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for ClickUp, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "ClickUp rejected the credential. Re-issue it: ClickUp → Settings → Apps → API token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No ClickUp credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "ClickUp → Settings → Apps → API token"
...
_NO_INTROSPECTION: Final = (
    "ClickUp has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="tasks:read",
    grants="read tasks in the configured list",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="list:read",
    grants="read the list itself, which the probe uses",
    capabilities=("clickup_issue_statistics",),
    where=_GRANTED_AT,
)
...
class ClickupVerifier:
    """Checks a stored ClickUp credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v2/list/{list_id} — the cheapest read ClickUp offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether ClickUp accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="ClickUp accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what ClickUp said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> ClickupClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a clickup probe needs a proxy transport and a request context")
        return ClickupClient(transport=transport, context=context, region=self.region)
