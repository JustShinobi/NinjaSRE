"""Reading a capped number of ClickUp's issues, after something narrowed the question.
...
Source of truth: ClickUp's ``/api/v2/list/{list_id}/task``.
"""
...
TOOL_NAME = "clickup_recent_issues"
...
@tool(
    name=TOOL_NAME,
    display_name="ClickUp recent issues",
    description=(
...
async def clickup_recent_issues(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of ClickUp's issues, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} issues from ClickUp{tail}",
                reference=f"clickup:recent_issues:{query or 'all'}",
            ),
        ),
    )
