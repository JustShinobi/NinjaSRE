"""Counting ClickUp's issues before reading any of them.

The first call of an investigation that reaches ClickUp, and the one
...
Source of truth: ClickUp's ``/api/v2/list/{list_id}/task``, read through the
...
TOOL_NAME = "clickup_issue_statistics"
...
DEFAULT_GROUP_BY = "status.status"
...
@tool(
    name=TOOL_NAME,
    display_name="ClickUp issue statistics",
    description=(
...
async def clickup_issue_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of ClickUp's issues across ``group_by``.

    ``start`` and ``end`` are ClickUp's own time expressions. Both are
    ...
    client = access.client(ClickupClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"clickup:issue_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no issues were returned by ClickUp for this request"
