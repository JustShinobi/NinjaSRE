"""ClickUp's API, over the proxy, holding no credential.
...
ClickUp's authentication changed tomorrow, because the authentication is
...
from integrations.clickup.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v2/list/{list_id}"
SEARCH_ISSUES_PATH: Final = "/api/v2/list/{list_id}/task"
RECENT_ISSUES_PATH: Final = "/api/v2/list/{list_id}/task"
...
class ClickupClient(IntegrationClient):
    """ClickUp, reached through the credential proxy."""
    ...
    async def search_issues(
        ...
        *,
        ...
        """Return what ClickUp holds for this request, bounded.
        ...
        """
        # ClickUp narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"subtasks": "true"}
            asked.update(parameters)
            answer = (await self.get(SEARCH_ISSUES_PATH, params=asked)).json()
            return Page(items=records(answer, "tasks"), cursor="" or None)
    ...
    async def recent_issues(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"subtasks": "true", "order_by": "updated"}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call ClickUp offers.
