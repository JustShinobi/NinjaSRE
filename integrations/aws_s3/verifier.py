"""Proves a AWS S3 credential works, and proves what it is allowed to do.
...
from integrations.aws_s3.client import AwsS3Client
from integrations.aws_s3.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for AWS S3, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS S3 rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS S3 credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "AWS S3 has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="s3:ListBucket",
    grants="list the objects in the configured bucket",
    capabilities=("aws_s3_resource_inventory",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="s3:ListBucketVersions",
    grants="list object versions, which is what makes a change history",
    capabilities=("aws_s3_recent_changes",),
    where=_GRANTED_AT,
)
...
class AwsS3Verifier:
    """Checks a stored AWS S3 credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /{bucket} — the cheapest read AWS S3 offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS S3 accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS S3 accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS S3 said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsS3Client:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_s3 probe needs a proxy transport and a request context")
        return AwsS3Client(transport=transport, context=context, region=self.region)
