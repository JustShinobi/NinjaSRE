"""AWS S3: What is in the bucket this team configured: the objects and their storage class, and the version history, which is the closest S3 has to a change log.

AWS signs a request rather than attaching a token to it, so a client that signed would be a client that holds a key. This one sends the request unsigned and the proxy signs it at the network edge, which is why the largest integration family in the catalogue is not the one exception to Article IV. The bucket is part of the path and is substituted by the proxy from the stored configuration, so a capability cannot be pointed at another bucket by an argument.
...
from integrations.aws_s3.client import PAGINATION, AwsS3Client
from integrations.aws_s3.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.aws_s3.verifier import PERMISSIONS, AwsS3Verifier
...
DESCRIPTOR: Final = IntegrationDescriptor(
    ...
    schema=SCHEMA,
    rule=RULE,
    verifier=AwsS3Verifier(),
    client_class=AwsS3Client,
