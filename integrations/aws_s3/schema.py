"""AWS S3's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "aws_s3"
DEFAULT_REGION: Final = "us-east-1"

#: Every host AWS S3 serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.from_template(
    INTEGRATION,
    template="s3.{region}.amazonaws.com",
    names=(
        "us-east-1",
        "us-east-2",
        "us-west-1",
...
SCHEMA: Final = credential_schema(
    ...
    public("bucket", "The bucket this integration reads", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        SignatureInjection(signer=SigV4Signer(service="s3", default_region=DEFAULT_REGION)),
        PathSegmentInjection(placeholder="bucket", field="bucket"),
    ),
)
