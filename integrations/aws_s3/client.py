"""AWS S3's API, over the proxy, holding no credential.
...
AWS S3's authentication changed tomorrow, because the authentication is
...
from integrations.aws_s3.schema import INTEGRATION, base_url

PING_PATH: Final = "/{bucket}"
LIST_RESOURCES_PATH: Final = "/{bucket}"
LIST_CHANGES_PATH: Final = "/{bucket}"
...
PAGINATION: Final[tuple[EndpointPagination, ...]] = (
    EndpointPagination(
        ...
        page_size_parameter="max-keys",
        page_size=100,
    ),
    EndpointPagination(
        endpoint="list_changes",
        style=PaginationStyle.CURSOR,
        parameter="key-marker",
        page_size_parameter="max-keys",
        page_size=50,
    ),
)
...
class AwsS3Client(IntegrationClient):
    """AWS S3, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what AWS S3 holds for this request, bounded.
        ...
        """
        # AWS S3 narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"list-type": "2", "prefix": kind}
            asked.update(parameters)
            answer = (await self.get(LIST_RESOURCES_PATH, params=asked)).text
            return Page(
                items=xml_records(answer, "Contents"),
                cursor=xml_text(answer, "NextContinuationToken") or None,
            )
    ...
    async def ping(self) -> ClientResponse:
        ...
        """
        return await self.get(PING_PATH, params={"list-type": "2", "max-keys": "1"})
