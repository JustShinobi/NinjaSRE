"""Counting AWS S3's resources before reading any of them.

The first call of an investigation that reaches AWS S3, and the one
...
Source of truth: AWS S3's ``/{bucket}``, read through the
...
TOOL_NAME = "aws_s3_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS S3 resource inventory",
    description=(
...
async def aws_s3_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of AWS S3's resources across ``group_by``.

    ``start`` and ``end`` are AWS S3's own time expressions. Both are
    ...
    client = access.client(AwsS3Client, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"aws_s3:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by AWS S3 for this request"
