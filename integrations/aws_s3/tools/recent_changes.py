"""Reading a capped number of AWS S3's changes, after something narrowed the question.
...
Source of truth: AWS S3's ``/{bucket}``.
"""
...
TOOL_NAME = "aws_s3_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS S3 recent changes",
    description=(
...
async def aws_s3_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of AWS S3's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from AWS S3{tail}",
                reference=f"aws_s3:recent_changes:{kind or 'all'}",
            ),
        ),
    )
