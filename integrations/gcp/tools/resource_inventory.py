"""Counting Google Cloud's resources before reading any of them.

The first call of an investigation that reaches Google Cloud, and the one
...
Source of truth: Google Cloud's ``/v1/projects/{project}/assets``, read through the
...
TOOL_NAME = "gcp_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="Google Cloud resource inventory",
    description=(
...
async def gcp_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Google Cloud's resources across ``group_by``.

    ``start`` and ``end`` are Google Cloud's own time expressions. Both are
    ...
    client = access.client(GcpClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"gcp:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by Google Cloud for this request"
