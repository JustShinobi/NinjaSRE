"""Reading a capped number of Google Cloud's changes, after something narrowed the question.
...
Source of truth: Google Cloud's ``/v1/projects/{project}/assets``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Google Cloud recent changes",
    description=(
...
async def gcp_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Google Cloud's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from Google Cloud{tail}",
                reference=f"gcp:recent_changes:{kind or 'all'}",
            ),
        ),
    )
