"""Google Cloud's API, over the proxy, holding no credential.
...
Google Cloud's authentication changed tomorrow, because the authentication is
...
from integrations.gcp.schema import INTEGRATION, base_url

PING_PATH: Final = "/v1/projects/{project}/assets"
LIST_RESOURCES_PATH: Final = "/v1/projects/{project}/assets"
LIST_CHANGES_PATH: Final = "/v1/projects/{project}/assets"
...
class GcpClient(IntegrationClient):
    """Google Cloud, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what Google Cloud holds for this request, bounded.
        ...
        """
        # Google Cloud narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"contentType": "RESOURCE", "assetTypes": kind}
            asked.update(parameters)
            answer = (await self.get(LIST_RESOURCES_PATH, params=asked)).json()
            return Page(
                items=records(answer, "assets"), cursor=text(answer, "nextPageToken") or None
            )
    ...
    async def list_changes(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"contentType": "RESOURCE", "assetTypes": kind, "readTime": end}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Google Cloud offers.
        ...
        """
        return await self.get(PING_PATH, params={"pageSize": "1", "contentType": "RESOURCE"})
