"""Google Cloud's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "gcp"


#: Every host Google Cloud serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap(
    integration=INTEGRATION,
    regions=(
        Region(
            name="global", host="cloudasset.googleapis.com", display_name="Cloud Asset Inventory"
        ),
    ),
    default="global",
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "OAuth access token for a service account with the viewer roles", min_length=8),
