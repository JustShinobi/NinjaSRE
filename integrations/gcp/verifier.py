"""Proves a Google Cloud credential works, and proves what it is allowed to do.
...
from integrations.gcp.client import GcpClient
from integrations.gcp.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Google Cloud, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Google Cloud rejected the credential. Re-issue it: IAM & Admin → Service Accounts → Keys, exchanged for an access token against oauth2.googleapis.com"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Google Cloud credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "Google Cloud has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_1: Final = RequiredPermission(
    ...
    grants="list the resources in the configured project",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read a resource's state at a point in time",
    capabilities=("gcp_recent_changes",),
    where=_GRANTED_AT,
)
...
class GcpVerifier:
    """Checks a stored Google Cloud credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v1/projects/{project}/assets — the cheapest read Google Cloud offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Google Cloud accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Google Cloud accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Google Cloud said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> GcpClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a gcp probe needs a proxy transport and a request context")
        return GcpClient(transport=transport, context=context, region=self.region)
