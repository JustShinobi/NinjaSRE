"""Rocket.Chat's API, over the proxy, holding no credential.
...
Rocket.Chat's authentication changed tomorrow, because the authentication is
...
from integrations.rocket_chat.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v1/me"
RECENT_MESSAGES_PATH: Final = "/api/v1/channels.history"
POST_MESSAGE_PATH: Final = "/api/v1/chat.postMessage"
...
class RocketChatClient(IntegrationClient):
    """Rocket.Chat, reached through the credential proxy."""
    ...
    async def recent_messages(
        ...
        *,
        ...
        """Return what Rocket.Chat holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"roomName": channel, "oldest": start, "latest": end}
    ...
    async def post_message(self, channel: str, text: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Rocket.Chat said."""
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Rocket.Chat offers.
