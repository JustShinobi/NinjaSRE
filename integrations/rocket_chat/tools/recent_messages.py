"""Reading a capped number of Rocket.Chat's messages, after something narrowed the question.
...
Source of truth: Rocket.Chat's ``/api/v1/channels.history``.
"""
...
TOOL_NAME = "rocket_chat_recent_messages"
...
@tool(
    name=TOOL_NAME,
    display_name="Rocket.Chat recent messages",
    description=(
...
async def rocket_chat_recent_messages(
    channel: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Rocket.Chat's messages, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} messages from Rocket.Chat{tail}",
                reference=f"rocket_chat:recent_messages:{channel or 'all'}",
            ),
        ),
    )
