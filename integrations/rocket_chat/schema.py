"""Rocket.Chat's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "rocket_chat"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "rocketchat.example.com"

#: Every host Rocket.Chat serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="rocketchat.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Rocket.Chat personal access token", min_length=8),
    secret("user_id", "The user id the token belongs to", min_length=8),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        HeaderInjection(header="X-Auth-Token", field="token", template="{value}"),
        HeaderInjection(header="X-User-Id", field="user_id"),
    ),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Rocket.Chat is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="rocket_chat.acme.example")``.
    """
