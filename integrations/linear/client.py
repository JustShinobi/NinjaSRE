"""Linear's API, over the proxy, holding no credential.
...
Linear's authentication changed tomorrow, because the authentication is
...
from integrations.linear.schema import INTEGRATION, base_url

PING_PATH: Final = "/graphql"
SEARCH_ISSUES_PATH: Final = "/graphql"
RECENT_ISSUES_PATH: Final = "/graphql"
...
class LinearClient(IntegrationClient):
    """Linear, reached through the credential proxy."""
    ...
    async def search_issues(
        ...
        *,
        ...
        """Return what Linear holds for this request, bounded.
        ...
        """
        # Linear narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                "query": "{ issues(first: 100) { nodes { identifier title state { name } } } }"
            }
            body.update(parameters)
            answer = (await self.post(SEARCH_ISSUES_PATH, json_body=body)).json()
            return Page(
                items=records(answer, "data", "issues", "nodes"),
                cursor=text(answer, "data", "issues", "pageInfo", "endCursor") or None,
            )
    ...
    async def recent_issues(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                "query": "{ issues(first: 20, orderBy: updatedAt) { nodes { identifier title state { name } } } }"
            }
            body.update(parameters)
            answer = (await self.post(RECENT_ISSUES_PATH, json_body=body)).json()
            return Page(
                items=records(answer, "data", "issues", "nodes"),
                cursor=text(answer, "data", "issues", "pageInfo", "endCursor") or None,
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Linear offers.
        ...
        """
        return await self.post(PING_PATH, json_body={"query": "{ viewer { id } }"})
