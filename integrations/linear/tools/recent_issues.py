"""Reading a capped number of Linear's issues, after something narrowed the question.
...
Source of truth: Linear's ``/graphql``.
"""
...
TOOL_NAME = "linear_recent_issues"
...
@tool(
    name=TOOL_NAME,
    display_name="Linear recent issues",
    description=(
...
async def linear_recent_issues(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Linear's issues, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} issues from Linear{tail}",
                reference=f"linear:recent_issues:{query or 'all'}",
            ),
        ),
    )
