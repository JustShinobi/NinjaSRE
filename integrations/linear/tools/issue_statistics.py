"""Counting Linear's issues before reading any of them.

The first call of an investigation that reaches Linear, and the one
...
Source of truth: Linear's ``/graphql``, read through the
...
TOOL_NAME = "linear_issue_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Linear issue statistics",
    description=(
...
async def linear_issue_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Linear's issues across ``group_by``.

    ``start`` and ``end`` are Linear's own time expressions. Both are
    ...
    client = access.client(LinearClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"linear:issue_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no issues were returned by Linear for this request"
