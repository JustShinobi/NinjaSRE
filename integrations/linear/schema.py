"""Linear's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "linear"


#: Every host Linear serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.linear.app", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Linear API key", min_length=8),
)

RULE: Final = InjectionRule(
    integration=INTEGRATION,
    hosts=HOSTS,
    injections=(HeaderInjection(header="Authorization", field="token", template="{value}"),),
)
