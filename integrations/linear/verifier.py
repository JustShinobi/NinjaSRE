"""Proves a Linear credential works, and proves what it is allowed to do.
...
from integrations.linear.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Linear, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Linear rejected the credential. Re-issue it: Linear → Settings → API → Personal API keys"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Linear credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Linear → Settings → API → Personal API keys"
...
_NO_INTROSPECTION: Final = (
    "Linear has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="read",
    grants="read issues and their state",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="viewer",
    grants="read the key's own identity, which the probe uses",
    capabilities=("linear_issue_statistics",),
    where=_GRANTED_AT,
)
...
class LinearVerifier:
    """Checks a stored Linear credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /graphql — the cheapest read Linear offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Linear accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Linear accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Linear said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> LinearClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a linear probe needs a proxy transport and a request context")
        return LinearClient(transport=transport, context=context, region=self.region)
