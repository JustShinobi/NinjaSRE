"""Proves a Redis Cloud credential works, and proves what it is allowed to do.
...
from integrations.redis.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Redis Cloud, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Redis Cloud rejected the credential. Re-issue it: Redis Cloud → Access Management → API Keys"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Redis Cloud credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Redis Cloud → Access Management → API Keys"
...
_NO_INTROSPECTION: Final = (
    "Redis Cloud has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="subscriptions:read",
    grants="list subscriptions and their status",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="databases:read",
    grants="read database state within a subscription",
    capabilities=("redis_slow_queries",),
    where=_GRANTED_AT,
)
...
class RedisVerifier:
    """Checks a stored Redis Cloud credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v1/subscriptions — the cheapest read Redis Cloud offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Redis Cloud accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Redis Cloud accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Redis Cloud said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> RedisClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a redis probe needs a proxy transport and a request context")
        return RedisClient(transport=transport, context=context, region=self.region)
