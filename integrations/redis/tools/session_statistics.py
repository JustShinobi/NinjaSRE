"""Counting Redis Cloud's sessions before reading any of them.

The first call of an investigation that reaches Redis Cloud, and the one
...
Source of truth: Redis Cloud's ``/v1/subscriptions``, read through the
...
TOOL_NAME = "redis_session_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Redis Cloud session statistics",
    description=(
...
async def redis_session_statistics(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Redis Cloud's sessions across ``group_by``.

    ``start`` and ``end`` are Redis Cloud's own time expressions. Both are
    ...
    client = access.client(RedisClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"redis:session_statistics:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no sessions were returned by Redis Cloud for this request"
