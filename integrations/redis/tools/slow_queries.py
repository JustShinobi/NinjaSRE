"""Reading a capped number of Redis Cloud's statements, after something narrowed the question.
...
Source of truth: Redis Cloud's ``/v1/subscriptions``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Redis Cloud slow queries",
    description=(
...
async def redis_slow_queries(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Redis Cloud's statements, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.ANALYSIS,
                summary=f"{len(entries)} statements from Redis Cloud{tail}",
                reference=f"redis:slow_queries:{scope or 'all'}",
            ),
        ),
    )
