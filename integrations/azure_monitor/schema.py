"""Azure Monitor's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "azure_monitor"


#: Every host Azure Monitor serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.loganalytics.io", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Azure AD access token for the Log Analytics API audience", min_length=8),
    public("workspace", "Log Analytics workspace id", required=True),
)
