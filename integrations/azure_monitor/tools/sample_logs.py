"""Reading a capped number of Azure Monitor's log lines, after something narrowed the question.
...
Source of truth: Azure Monitor's ``/v1/workspaces/{workspace}/query``.
"""
...
TOOL_NAME = "azure_monitor_sample_logs"
...
@tool(
    name=TOOL_NAME,
    display_name="Azure Monitor log samples",
    description=(
...
async def azure_monitor_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Azure Monitor's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from Azure Monitor{tail}",
                reference=f"azure_monitor:sample_logs:{query or 'all'}",
            ),
        ),
    )
