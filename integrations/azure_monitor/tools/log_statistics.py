"""Counting Azure Monitor's log lines before reading any of them.

The first call of an investigation that reaches Azure Monitor, and the one
...
Source of truth: Azure Monitor's ``/v1/workspaces/{workspace}/query``, read through the
...
TOOL_NAME = "azure_monitor_log_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Azure Monitor log statistics",
    description=(
...
async def azure_monitor_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Azure Monitor's log lines across ``group_by``.

    ``start`` and ``end`` are Azure Monitor's own time expressions. Both are
    ...
    client = access.client(AzureMonitorClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"azure_monitor:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by Azure Monitor for this request"
