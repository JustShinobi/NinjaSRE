"""Azure Monitor's API, over the proxy, holding no credential.
...
Azure Monitor's authentication changed tomorrow, because the authentication is
...
from integrations.azure_monitor.schema import INTEGRATION, base_url

PING_PATH: Final = "/v1/workspaces/{workspace}/metadata"
SEARCH_LOGS_PATH: Final = "/v1/workspaces/{workspace}/query"
RECENT_LOGS_PATH: Final = "/v1/workspaces/{workspace}/query"
...
class AzureMonitorClient(IntegrationClient):
    """Azure Monitor, reached through the credential proxy."""
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what Azure Monitor holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                "query": query or "AppTraces | take 100",
                "timespan": start + "/" + end,
            }
            body.update(parameters)
            answer = (await self.post(SEARCH_LOGS_PATH, json_body=body)).json()
            return Page(items=records(answer, "tables"), cursor="" or None)
    ...
    async def recent_logs(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                "query": query or "AppTraces | take 20",
                "timespan": start + "/" + end,
            }
            body.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Azure Monitor offers.
