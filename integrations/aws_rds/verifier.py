"""Proves a AWS RDS credential works, and proves what it is allowed to do.
...
from integrations.aws_rds.client import AwsRdsClient
from integrations.aws_rds.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for AWS RDS, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS RDS rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS RDS credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "AWS RDS has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="rds:DescribeDBInstances",
    grants="read instance state and configuration",
    capabilities=("aws_rds_resource_inventory",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="rds:DescribeEvents",
    grants="read the failover, restart, and parameter-change history",
    capabilities=("aws_rds_recent_changes",),
    where=_GRANTED_AT,
)
...
class AwsRdsVerifier:
    """Checks a stored AWS RDS credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls / — the cheapest read AWS RDS offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS RDS accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS RDS accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS RDS said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsRdsClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_rds probe needs a proxy transport and a request context")
        return AwsRdsClient(transport=transport, context=context, region=self.region)
