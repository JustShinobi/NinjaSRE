"""AWS RDS: The RDS control plane: which database instances exist and in what state, and the events RDS recorded against them — failovers, restarts, parameter changes.
...
from integrations.aws_rds.client import PAGINATION, AwsRdsClient
from integrations.aws_rds.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.aws_rds.verifier import PERMISSIONS, AwsRdsVerifier
...
DESCRIPTOR: Final = IntegrationDescriptor(
    ...
    schema=SCHEMA,
    rule=RULE,
    verifier=AwsRdsVerifier(),
    client_class=AwsRdsClient,
