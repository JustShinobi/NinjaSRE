"""Counting AWS RDS's resources before reading any of them.

The first call of an investigation that reaches AWS RDS, and the one
...
Source of truth: AWS RDS's ``/``, read through the
...
TOOL_NAME = "aws_rds_resource_inventory"
...
DEFAULT_GROUP_BY = "DBInstanceStatus"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS RDS resource inventory",
    description=(
...
async def aws_rds_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of AWS RDS's resources across ``group_by``.

    ``start`` and ``end`` are AWS RDS's own time expressions. Both are
    ...
    client = access.client(AwsRdsClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"aws_rds:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by AWS RDS for this request"
