"""AWS RDS's credential shape, hosts, and how its secret enters a request.
...
#: Every host AWS RDS serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.from_template(
    INTEGRATION,
    template="rds.{region}.amazonaws.com",
    names=(
        "us-east-1",
        "us-east-2",
        "us-west-1",
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        SignatureInjection(signer=SigV4Signer(service="rds", default_region=DEFAULT_REGION)),
    ),
)
