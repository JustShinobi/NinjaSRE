"""Proves a groundcover credential works, and proves what it is allowed to do.
...
from integrations.groundcover.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for groundcover, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "groundcover rejected the credential. Re-issue it: the groundcover portal → Settings → API keys"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No groundcover credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "the groundcover portal → Settings → API keys"
...
_NO_INTROSPECTION: Final = (
    "groundcover has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="metrics:read",
    grants="evaluate PromQL against the ingested series",
    capabilities=("groundcover_metric_statistics",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="monitors:read",
    grants="list monitors and their current state",
    capabilities=("groundcover_active_alerts",),
    where=_GRANTED_AT,
)
...
class GroundcoverVerifier:
    """Checks a stored groundcover credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/monitors/list — the cheapest read groundcover offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether groundcover accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="groundcover accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what groundcover said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> GroundcoverClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a groundcover probe needs a proxy transport and a request context")
        return GroundcoverClient(transport=transport, context=context, region=self.region)
