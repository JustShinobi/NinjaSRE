"""Counting groundcover's series before reading any of them.

The first call of an investigation that reaches groundcover, and the one
...
Source of truth: groundcover's ``/api/promql/query_range``, read through the
...
TOOL_NAME = "groundcover_metric_statistics"
...
DEFAULT_GROUP_BY = "metric.workload"
...
@tool(
    name=TOOL_NAME,
    display_name="groundcover metric statistics",
    description=(
...
async def groundcover_metric_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of groundcover's series across ``group_by``.

    ``start`` and ``end`` are groundcover's own time expressions. Both are
    ...
    client = access.client(GroundcoverClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"groundcover:metric_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no series were returned by groundcover for this request"
