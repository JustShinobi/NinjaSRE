"""Reading a capped number of groundcover's alerts, after something narrowed the question.
...
Source of truth: groundcover's ``/api/monitors/list``.
"""
...
TOOL_NAME = "groundcover_active_alerts"
...
@tool(
    name=TOOL_NAME,
    display_name="groundcover active alerts",
    description=(
...
async def groundcover_active_alerts(
    state: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of groundcover's alerts, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} alerts from groundcover{tail}",
                reference=f"groundcover:active_alerts:{state or 'all'}",
            ),
        ),
    )
