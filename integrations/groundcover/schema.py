"""groundcover's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "groundcover"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "api.groundcover.com"

#: Every host groundcover serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.groundcover.com", name="saas")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("api_key", "groundcover API key for the backend this cluster reports to", min_length=8),
)

RULE: Final = InjectionRule(
    integration=INTEGRATION,
    hosts=HOSTS,
    injections=(HeaderInjection(header="apikey", field="api_key", template="{value}"),),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    groundcover is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="groundcover.acme.example")``.
    """
    named = {"saas": DEFAULT_HOST, **endpoints}
    return RegionMap(
        integration=INTEGRATION,
