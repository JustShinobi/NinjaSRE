"""PostHog's API, over the proxy, holding no credential.
...
PostHog's authentication changed tomorrow, because the authentication is
...
from integrations.posthog.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/projects/{project_id}/"
QUERY_METRIC_PATH: Final = "/api/projects/{project_id}/events/"
LIST_ALERTS_PATH: Final = "/api/projects/{project_id}/feature_flags/"
...
class PosthogClient(IntegrationClient):
    """PostHog, reached through the credential proxy."""
    ...
    async def query_metric(
        ...
        *,
        ...
        """Return what PostHog holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"event": query, "after": start, "before": end}
            asked.update(parameters)
            answer = (await self.get(QUERY_METRIC_PATH, params=asked)).json()
            return Page(items=records(answer, "results"), cursor=text(answer, "next") or None)
    ...
    async def list_alerts(
        ...
        *,
        ...
        "there is no more" when what happened was "we stopped looking".
        """
        # PostHog narrows this endpoint by other means; the argument stays in the
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call PostHog offers.
