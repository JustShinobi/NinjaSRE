"""Counting PostHog's series before reading any of them.

The first call of an investigation that reaches PostHog, and the one
...
Source of truth: PostHog's ``/api/projects/{project_id}/events/``, read through the
...
TOOL_NAME = "posthog_metric_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="PostHog metric statistics",
    description=(
...
async def posthog_metric_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of PostHog's series across ``group_by``.

    ``start`` and ``end`` are PostHog's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"posthog:metric_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no series were returned by PostHog for this request"
