"""Proves a PostHog credential works, and proves what it is allowed to do.
...
#: What each failure means for PostHog, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "PostHog rejected the credential. Re-issue it: PostHog → Settings → Personal API keys"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No PostHog credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "PostHog → Settings → Personal API keys"
...
_NO_INTROSPECTION: Final = (
    "PostHog has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="event:read",
    grants="read the project's events",
    capabilities=("posthog_metric_statistics",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="feature_flag:read",
    grants="read feature flags and their state",
    capabilities=("posthog_active_alerts",),
    where=_GRANTED_AT,
)
...
class PosthogVerifier:
    """Checks a stored PostHog credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/projects/{project_id}/ — the cheapest read PostHog offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether PostHog accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="PostHog accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what PostHog said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> PosthogClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a posthog probe needs a proxy transport and a request context")
        return PosthogClient(transport=transport, context=context, region=self.region)
