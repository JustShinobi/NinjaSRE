"""PostHog's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "posthog"


#: Every host PostHog serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap(
    integration=INTEGRATION,
    regions=(
        Region(name="us", host="us.posthog.com", display_name="United States"),
        Region(name="eu", host="eu.posthog.com", display_name="Europe"),
        Region(name="self-hosted", host="posthog.example.com", display_name="Self-hosted"),
    ),
    default="us",
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "PostHog personal API key", min_length=8),
    public("project_id", "PostHog project id", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="project_id", field="project_id"),
    ),
)
