"""Reading a capped number of Spark's failures, after something narrowed the question.
...
Source of truth: Spark's ``/api/v1/applications``.
"""
...
TOOL_NAME = "spark_recent_failures"
...
@tool(
    name=TOOL_NAME,
    display_name="Spark recent failures",
    description=(
...
async def spark_recent_failures(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Spark's failures, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} failures from Spark{tail}",
                reference=f"spark:recent_failures:{scope or 'all'}",
            ),
        ),
    )
