"""Counting Spark's tasks before reading any of them.

The first call of an investigation that reaches Spark, and the one
...
Source of truth: Spark's ``/api/v1/applications``, read through the
...
TOOL_NAME = "spark_pipeline_health"
...
DEFAULT_GROUP_BY = "attempts.completed"
...
@tool(
    name=TOOL_NAME,
    display_name="Spark pipeline health",
    description=(
...
async def spark_pipeline_health(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Spark's tasks across ``group_by``.

    ``start`` and ``end`` are Spark's own time expressions. Both are
    ...
    client = access.client(SparkClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"spark:pipeline_health:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no tasks were returned by Spark for this request"
