"""Spark's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "spark"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "spark-history.example.com"

#: Every host Spark serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="spark-history.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Bearer token accepted by whatever fronts the Spark UI", min_length=8),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Spark is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="spark.acme.example")``.
    """
