"""Spark's API, over the proxy, holding no credential.
...
Spark's authentication changed tomorrow, because the authentication is
...
from integrations.spark.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v1/version"
LIST_TASKS_PATH: Final = "/api/v1/applications"
LIST_FAILURES_PATH: Final = "/api/v1/applications"
...
class SparkClient(IntegrationClient):
    """Spark, reached through the credential proxy."""
    ...
    async def list_tasks(
        ...
        *,
        ...
        """Return what Spark holds for this request, bounded.
        ...
        """
        # Spark narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"status": scope}
    ...
    async def list_failures(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"status": "completed", "minDate": start}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Spark offers.
