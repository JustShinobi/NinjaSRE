"""Proves a Spark credential works, and proves what it is allowed to do.
...
from integrations.spark.client import SparkClient
from integrations.spark.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Spark, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Spark rejected the credential. Re-issue it: your reverse proxy or Knox gateway — the Spark UI has no authentication of its own"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Spark credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "Spark has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="applications:read",
    grants="list applications and their attempts",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    capabilities=("spark_pipeline_health",),
    where=_GRANTED_AT,
)
...
class SparkVerifier:
    """Checks a stored Spark credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v1/version — the cheapest read Spark offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Spark accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Spark accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Spark said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> SparkClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a spark probe needs a proxy transport and a request context")
        return SparkClient(transport=transport, context=context, region=self.region)
