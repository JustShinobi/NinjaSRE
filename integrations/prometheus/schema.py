"""Prometheus's credential shape, hosts, and how its secret enters a request.
...
DEFAULT_HOST: Final = "prometheus.example.com"

#: Every host Prometheus serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="prometheus.example.com", name="self-hosted")
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Prometheus is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="prometheus.acme.example")``.
    """
