"""Counting Prometheus's series before reading any of them.

The first call of an investigation that reaches Prometheus, and the one
...
Source of truth: Prometheus's ``/api/v1/query_range``, read through the
...
TOOL_NAME = "prometheus_metric_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Prometheus metric statistics",
    description=(
...
async def prometheus_metric_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Prometheus's series across ``group_by``.

    ``start`` and ``end`` are Prometheus's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"prometheus:metric_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no series were returned by Prometheus for this request"
