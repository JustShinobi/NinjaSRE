"""Reading a capped number of Prometheus's alerts, after something narrowed the question.
...
Source of truth: Prometheus's ``/api/v1/alerts``.
"""
...
TOOL_NAME = "prometheus_active_alerts"
...
@tool(
    name=TOOL_NAME,
    display_name="Prometheus active alerts",
    description=(
...
async def prometheus_active_alerts(
    ...
    """Return up to ``limit`` of Prometheus's alerts, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} alerts from Prometheus{tail}",
                reference=f"prometheus:active_alerts:{state or 'all'}",
            ),
        ),
    )
