"""Elasticsearch's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "elasticsearch"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "elasticsearch.example.com"

#: Every host Elasticsearch serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="elasticsearch.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    ...
    secret(
        "api_key",
        "Elasticsearch API key, base64 of id:api_key as Elasticsearch issues it",
        min_length=8,
    ),
    public("index", "Default index or data stream pattern to search"),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        HeaderInjection(header="Authorization", field="api_key", template="ApiKey {value}"),
    ),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Elasticsearch is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="elasticsearch.acme.example")``.
    """
