"""Elasticsearch's API, over the proxy, holding no credential.
...
Elasticsearch's authentication changed tomorrow, because the authentication is
...
from integrations.elasticsearch.schema import INTEGRATION, base_url

PING_PATH: Final = "/_cluster/health"
SEARCH_LOGS_PATH: Final = "/_search"
RECENT_LOGS_PATH: Final = "/_search"
...
class ElasticsearchClient(IntegrationClient):
    """Elasticsearch, reached through the credential proxy."""
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what Elasticsearch holds for this request, bounded.
        ...
        """
        # Elasticsearch narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(items=records(answer, "hits", "hits"), cursor="" or None)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Elasticsearch offers.
