"""Counting Elasticsearch's log lines before reading any of them.

The first call of an investigation that reaches Elasticsearch, and the one
...
Source of truth: Elasticsearch's ``/_search``, read through the
...
from integrations.elasticsearch.schema import INTEGRATION

TOOL_NAME = "elasticsearch_log_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Elasticsearch log statistics",
    description=(
...
async def elasticsearch_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Elasticsearch's log lines across ``group_by``.

    ``start`` and ``end`` are Elasticsearch's own time expressions. Both are
    ...
    client = access.client(ElasticsearchClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"elasticsearch:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by Elasticsearch for this request"
