"""Reading a capped number of Elasticsearch's log lines, after something narrowed the question.
...
Source of truth: Elasticsearch's ``/_search``.
"""
...
TOOL_NAME = "elasticsearch_sample_logs"
...
@tool(
    name=TOOL_NAME,
    display_name="Elasticsearch log samples",
    description=(
...
async def elasticsearch_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Elasticsearch's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from Elasticsearch{tail}",
                reference=f"elasticsearch:sample_logs:{query or 'all'}",
            ),
        ),
    )
