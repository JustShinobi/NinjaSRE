"""Counting Alertmanager's incidents before reading any of them.

The first call of an investigation that reaches Alertmanager, and the one
...
Source of truth: Alertmanager's ``/api/v2/alerts``, read through the
...
TOOL_NAME = "alertmanager_incident_statistics"
...
DEFAULT_GROUP_BY = "labels.alertname"
...
@tool(
    name=TOOL_NAME,
    display_name="Alertmanager incident statistics",
    description=(
...
async def alertmanager_incident_statistics(
    status: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Alertmanager's incidents across ``group_by``.

    ``start`` and ``end`` are Alertmanager's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"alertmanager:incident_statistics:{status or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no incidents were returned by Alertmanager for this request"
