"""Proves a Airflow credential works, and proves what it is allowed to do.
...
from integrations.airflow.client import AirflowClient
from integrations.airflow.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Airflow, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Airflow rejected the credential. Re-issue it: Airflow → Security → List Users, or whichever auth backend your webserver uses"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Airflow credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "Airflow has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="can_read on DAG Runs",
    grants="list DAG runs and their state",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="can_read on Task Instances",
    grants="read the tasks behind a failed run",
    capabilities=("airflow_recent_failures",),
    where=_GRANTED_AT,
)
...
class AirflowVerifier:
    """Checks a stored Airflow credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v1/health — the cheapest read Airflow offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Airflow accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Airflow accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Airflow said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AirflowClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a airflow probe needs a proxy transport and a request context")
        return AirflowClient(transport=transport, context=context, region=self.region)
