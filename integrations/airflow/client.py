"""Airflow's API, over the proxy, holding no credential.
...
Airflow's authentication changed tomorrow, because the authentication is
...
from integrations.airflow.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v1/health"
LIST_TASKS_PATH: Final = "/api/v1/dags/~/dagRuns"
LIST_FAILURES_PATH: Final = "/api/v1/dags/~/dagRuns"
...
class AirflowClient(IntegrationClient):
    """Airflow, reached through the credential proxy."""
    ...
    async def list_tasks(
        ...
        *,
        ...
        """Return what Airflow holds for this request, bounded.
        ...
        """
        # Airflow narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"dag_id_pattern": scope, "start_date_gte": start}
            asked.update(parameters)
            answer = (await self.get(LIST_TASKS_PATH, params=asked)).json()
            return Page(items=records(answer, "dag_runs"), cursor="" or None)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Airflow offers.
