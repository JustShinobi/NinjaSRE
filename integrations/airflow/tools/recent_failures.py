"""Reading a capped number of Airflow's failures, after something narrowed the question.
...
Source of truth: Airflow's ``/api/v1/dags/~/dagRuns``.
"""
...
TOOL_NAME = "airflow_recent_failures"
...
@tool(
    name=TOOL_NAME,
    display_name="Airflow recent failures",
    description=(
...
async def airflow_recent_failures(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Airflow's failures, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} failures from Airflow{tail}",
                reference=f"airflow:recent_failures:{scope or 'all'}",
            ),
        ),
    )
