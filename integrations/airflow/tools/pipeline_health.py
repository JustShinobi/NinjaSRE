"""Counting Airflow's tasks before reading any of them.

The first call of an investigation that reaches Airflow, and the one
...
Source of truth: Airflow's ``/api/v1/dags/~/dagRuns``, read through the
...
TOOL_NAME = "airflow_pipeline_health"
...
@tool(
    name=TOOL_NAME,
    display_name="Airflow pipeline health",
    description=(
...
async def airflow_pipeline_health(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Airflow's tasks across ``group_by``.

    ``start`` and ``end`` are Airflow's own time expressions. Both are
    ...
    client = access.client(AirflowClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"airflow:pipeline_health:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no tasks were returned by Airflow for this request"
