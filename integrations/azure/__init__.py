"""Azure: The Azure Resource Manager control plane: what exists in a subscription, and the activity log entries that changed it.



Two declarations, and discovery walks for both. ``DESCRIPTOR`` is the credential
half — what a credential is made of, which hosts it may reach, how the secret
enters a request, and how to check that it works. ``PROFILE`` is the operational
half — what class of system this is, where it can be reached, what permissions
its capabilities need, and how its endpoints page.
"""

from __future__ import annotations

from typing import Final

from integrations._catalogue.entry import IntegrationCategory, IntegrationProfile
from integrations.azure.client import PAGINATION, AzureClient
from integrations.azure.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.azure.verifier import PERMISSIONS, AzureVerifier
from platform.credentials.descriptor import IntegrationDescriptor, SdkStrategy

DESCRIPTOR: Final = IntegrationDescriptor(
    name=INTEGRATION,
    schema=SCHEMA,
    rule=RULE,
    verifier=AzureVerifier(),
    client_class=AzureClient,
    sdk_strategy=SdkStrategy.DIRECT_CLIENT,
    strategy_note=(
        "The Azure SDK resolves credentials through DefaultAzureCredential, which reads the environment, a managed identity endpoint, and a shared token cache."
    ),
)

PROFILE: Final = IntegrationProfile(
    integration=INTEGRATION,
    category=IntegrationCategory.CLOUD_CONTROL_PLANE,
    summary="The Azure Resource Manager control plane: what exists in a subscription, and the activity log entries that changed it.",
    regions=REGIONS,
    permissions=PERMISSIONS,
    pagination=PAGINATION,
)

__all__ = [
    "DESCRIPTOR",
    "HOSTS",
    "INTEGRATION",
    "PAGINATION",
    "PERMISSIONS",
    "PROFILE",
    "REGIONS",
    "RULE",
    "SCHEMA",
    "AzureClient",
    "AzureVerifier",
    "base_url",
]
