"""Proves a Azure credential works, and proves what it is allowed to do.
...
from integrations.azure.client import AzureClient
from integrations.azure.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Azure, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Azure rejected the credential. Re-issue it: Azure Portal → App registrations → your service principal, exchanged for a token against https://management.azure.com"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Azure credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "Azure has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_1: Final = RequiredPermission(
    ...
    grants="list resources",
    capabilities=("azure_resource_inventory",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="Microsoft.Insights/eventtypes/values/read",
    grants="read the activity log, which is the change history",
    capabilities=("azure_recent_changes",),
    where=_GRANTED_AT,
)
...
class AzureVerifier:
    """Checks a stored Azure credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /subscriptions/{subscription}/resources — the cheapest read Azure offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Azure accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Azure accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Azure said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AzureClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a azure probe needs a proxy transport and a request context")
        return AzureClient(transport=transport, context=context, region=self.region)
