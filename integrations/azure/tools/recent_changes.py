"""Reading a capped number of Azure's changes, after something narrowed the question.
...
Source of truth: Azure's ``/subscriptions/{subscription}/providers/microsoft.insights/eventtypes/management/values``.
"""
...
TOOL_NAME = "azure_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="Azure recent changes",
    description=(
...
async def azure_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Azure's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from Azure{tail}",
                reference=f"azure:recent_changes:{kind or 'all'}",
            ),
        ),
    )
