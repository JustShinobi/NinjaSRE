"""Counting Azure's resources before reading any of them.

The first call of an investigation that reaches Azure, and the one
...
Source of truth: Azure's ``/subscriptions/{subscription}/resources``, read through the
...
@tool(
    name=TOOL_NAME,
    display_name="Azure resource inventory",
    description=(
...
async def azure_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Azure's resources across ``group_by``.

    ``start`` and ``end`` are Azure's own time expressions. Both are
    ...
    client = access.client(AzureClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"azure:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by Azure for this request"
