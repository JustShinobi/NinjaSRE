"""Telegram's API, over the proxy, holding no credential.
...
Telegram's authentication changed tomorrow, because the authentication is
...
from integrations.telegram.schema import INTEGRATION, base_url

PING_PATH: Final = "/bot{token}/getMe"
RECENT_MESSAGES_PATH: Final = "/bot{token}/getUpdates"
POST_MESSAGE_PATH: Final = "/bot{token}/sendMessage"
...
class TelegramClient(IntegrationClient):
    """Telegram, reached through the credential proxy."""
    ...
    async def recent_messages(
        ...
        *,
        ...
        """Return what Telegram holds for this request, bounded.
        ...
        """
        # Telegram narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"allowed_updates": channel}
    ...
    async def post_message(self, channel: str, text: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Telegram said."""
        body: dict[str, Any] = {"chat_id": channel, "text": text}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Telegram offers.
