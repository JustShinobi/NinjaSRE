"""Telegram: the one change this integration is allowed to make.
...
Source of truth: Telegram's ``/bot{token}/sendMessage``.
"""
...
class TelegramPostMessageRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the Telegram change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what Telegram returned for {target}, including its identifier.",
                "Undo the change in Telegram using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="Telegram post message",
    description=(
    ...
    rollback_planner=TelegramPostMessageRollback(),
    tags=(
        "communication",
        "telegram",
        "write",
...
async def telegram_post_message(channel: str, text: str) -> CapabilityResult:
    """Make the change in Telegram and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(TelegramClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"Telegram accepted the change to {channel!r}",
                reference=f"telegram:post_message:{channel}",
            ),
        ),
    )
