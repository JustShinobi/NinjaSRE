"""Reading a capped number of Telegram's messages, after something narrowed the question.
...
Source of truth: Telegram's ``/bot{token}/getUpdates``.
"""
...
TOOL_NAME = "telegram_recent_messages"
...
@tool(
    name=TOOL_NAME,
    display_name="Telegram recent messages",
    description=(
...
async def telegram_recent_messages(
    channel: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Telegram's messages, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} messages from Telegram{tail}",
                reference=f"telegram:recent_messages:{channel or 'all'}",
            ),
        ),
    )
