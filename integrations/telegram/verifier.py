"""Proves a Telegram credential works, and proves what it is allowed to do.
...
from integrations.telegram.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Telegram, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Telegram rejected the credential. Re-issue it: Telegram → @BotFather → /mybots → API Token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Telegram credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Telegram → @BotFather → /mybots → API Token"
...
_NO_INTROSPECTION: Final = (
    "Telegram has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="getUpdates",
    grants="read updates addressed to the bot",
    capabilities=("telegram_recent_messages",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="sendMessage",
    grants="send a message to a chat the bot is in",
...
class TelegramVerifier:
    """Checks a stored Telegram credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /bot{token}/getMe — the cheapest read Telegram offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Telegram accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Telegram accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Telegram said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> TelegramClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a telegram probe needs a proxy transport and a request context")
        return TelegramClient(transport=transport, context=context, region=self.region)
