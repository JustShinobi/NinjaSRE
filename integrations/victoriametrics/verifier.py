"""Proves a VictoriaMetrics credential works, and proves what it is allowed to do.
...
from integrations.victoriametrics.client import VictoriametricsClient
from integrations.victoriametrics.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for VictoriaMetrics, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "VictoriaMetrics rejected the credential. Re-issue it: your vmauth configuration — VictoriaMetrics ships no authentication of its own"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No VictoriaMetrics credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "VictoriaMetrics has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="select",
    grants="evaluate MetricsQL over the stored series",
    capabilities=("victoriametrics_metric_statistics",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="alerts:read",
    grants="read the alerts vmalert is holding",
    capabilities=("victoriametrics_active_alerts",),
    where=_GRANTED_AT,
)
...
class VictoriametricsVerifier:
    """Checks a stored VictoriaMetrics credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v1/status/buildinfo — the cheapest read VictoriaMetrics offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether VictoriaMetrics accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="VictoriaMetrics accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what VictoriaMetrics said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> VictoriametricsClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a victoriametrics probe needs a proxy transport and a request context")
        return VictoriametricsClient(transport=transport, context=context, region=self.region)
