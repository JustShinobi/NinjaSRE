"""VictoriaMetrics's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "victoriametrics"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "victoriametrics.example.com"

#: Every host VictoriaMetrics serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(
    INTEGRATION, host="victoriametrics.example.com", name="self-hosted"
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret(
        "token", "Bearer token accepted by vmauth or whatever fronts VictoriaMetrics", min_length=8
    ),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    VictoriaMetrics is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="victoriametrics.acme.example")``.
    """
