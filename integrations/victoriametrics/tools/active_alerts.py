"""Reading a capped number of VictoriaMetrics's alerts, after something narrowed the question.
...
Source of truth: VictoriaMetrics's ``/api/v1/alerts``.
"""
...
TOOL_NAME = "victoriametrics_active_alerts"
...
@tool(
    name=TOOL_NAME,
    display_name="VictoriaMetrics active alerts",
    description=(
...
async def victoriametrics_active_alerts(
    state: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of VictoriaMetrics's alerts, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} alerts from VictoriaMetrics{tail}",
                reference=f"victoriametrics:active_alerts:{state or 'all'}",
            ),
        ),
    )
