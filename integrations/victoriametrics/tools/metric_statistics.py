"""Counting VictoriaMetrics's series before reading any of them.

The first call of an investigation that reaches VictoriaMetrics, and the one
...
Source of truth: VictoriaMetrics's ``/api/v1/query_range``, read through the
...
TOOL_NAME = "victoriametrics_metric_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="VictoriaMetrics metric statistics",
    description=(
...
async def victoriametrics_metric_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of VictoriaMetrics's series across ``group_by``.

    ``start`` and ``end`` are VictoriaMetrics's own time expressions. Both are
    ...
    client = access.client(VictoriametricsClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"victoriametrics:metric_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no series were returned by VictoriaMetrics for this request"
