"""Blameless's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "blameless"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "acme.blameless.io"

#: Every host Blameless serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="acme.blameless.io", name="tenant")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Blameless API token for a service account", min_length=8),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Blameless is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="blameless.acme.example")``.
    """
    named = {"tenant": DEFAULT_HOST, **endpoints}
    return RegionMap(
        integration=INTEGRATION,
