"""Blameless: the one change this integration is allowed to make.
...
Source of truth: Blameless's ``/api/v1/incidents/events``.
"""
...
TOOL_NAME = "blameless_acknowledge_incident"
...
class BlamelessAcknowledgeIncidentRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the Blameless change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what Blameless returned for {target}, including its identifier.",
                "Undo the change in Blameless using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="Blameless acknowledge incident",
    description=(
    ...
    rollback_planner=BlamelessAcknowledgeIncidentRollback(),
    tags=(
        "incident",
        "blameless",
        "write",
...
async def blameless_acknowledge_incident(incident: str, note: str) -> CapabilityResult:
    """Make the change in Blameless and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(BlamelessClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"Blameless accepted the change to {incident!r}",
                reference=f"blameless:acknowledge_incident:{incident}",
            ),
        ),
    )
