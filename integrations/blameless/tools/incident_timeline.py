"""Reading a capped number of Blameless's timeline entries, after something narrowed the question.
...
Source of truth: Blameless's ``/api/v1/incidents``.
"""
...
TOOL_NAME = "blameless_incident_timeline"
...
@tool(
    name=TOOL_NAME,
    display_name="Blameless incident timeline",
    description=(
...
async def blameless_incident_timeline(
    incident: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Blameless's timeline entries, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"{len(entries)} timeline entries from Blameless{tail}",
                reference=f"blameless:incident_timeline:{incident or 'all'}",
            ),
        ),
    )
