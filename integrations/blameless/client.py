"""Blameless's API, over the proxy, holding no credential.
...
Blameless's authentication changed tomorrow, because the authentication is
...
from integrations.blameless.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v1/incidents"
LIST_INCIDENTS_PATH: Final = "/api/v1/incidents"
INCIDENT_TIMELINE_PATH: Final = "/api/v1/incidents"
ACKNOWLEDGE_PATH: Final = "/api/v1/incidents/events"
...
class BlamelessClient(IntegrationClient):
    """Blameless, reached through the credential proxy."""
    ...
    async def list_incidents(
        ...
        *,
        ...
        """Return what Blameless holds for this request, bounded.
        ...
        """
        # Blameless narrows this endpoint by other means; the argument stays in the
    ...
    async def incident_timeline(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"status": "active"}
            asked.update(parameters)
    ...
    async def acknowledge(self, incident: str, note: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Blameless said."""
        body: dict[str, Any] = {"incident_id": incident, "message": note, "source": "ninjasre"}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Blameless offers.
