"""Proves a Blameless credential works, and proves what it is allowed to do.
...
from integrations.blameless.client import BlamelessClient
from integrations.blameless.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Blameless, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Blameless rejected the credential. Re-issue it: Blameless → Settings → API tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Blameless credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Blameless → Settings → API tokens"
...
_NO_INTROSPECTION: Final = (
    "Blameless has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="post an event onto an incident",
    capabilities=("blameless_acknowledge_incident",),
    where=_GRANTED_AT,
)
...
class BlamelessVerifier:
    """Checks a stored Blameless credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v1/incidents — the cheapest read Blameless offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Blameless accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Blameless accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Blameless said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> BlamelessClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a blameless probe needs a proxy transport and a request context")
        return BlamelessClient(transport=transport, context=context, region=self.region)
