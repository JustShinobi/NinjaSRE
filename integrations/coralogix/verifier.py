"""Proves a Coralogix credential works, and proves what it is allowed to do.
...
from integrations.coralogix.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Coralogix, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Coralogix rejected the credential. Re-issue it: Data Flow → API Keys, on the team whose data you investigate"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Coralogix credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Data Flow → API Keys, on the team whose data you investigate"
...
_NO_INTROSPECTION: Final = (
    "Coralogix has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="DataQuerying",
    grants="run DataPrime and Lucene queries",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="LogsQuerying",
    grants="read the log entries a query matched",
    capabilities=("coralogix_sample_logs",),
    where=_GRANTED_AT,
)
...
class CoralogixVerifier:
    """Checks a stored Coralogix credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v1/dataprime/query — the cheapest read Coralogix offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Coralogix accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Coralogix accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Coralogix said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> CoralogixClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a coralogix probe needs a proxy transport and a request context")
        return CoralogixClient(transport=transport, context=context, region=self.region)
