"""Coralogix's API, over the proxy, holding no credential.
...
Coralogix's authentication changed tomorrow, because the authentication is
...
from integrations.coralogix.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v1/dataprime/query"
SEARCH_LOGS_PATH: Final = "/api/v1/dataprime/query"
RECENT_LOGS_PATH: Final = "/api/v1/dataprime/query"
...
class CoralogixClient(IntegrationClient):
    """Coralogix, reached through the credential proxy."""
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what Coralogix holds for this request, bounded.
    ...
    async def recent_logs(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                "query": query or "source logs | limit 20",
                "metadata": {"startDate": start, "endDate": end},
            }
            body.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Coralogix offers.
        ...
        """
        return await self.post(PING_PATH, json_body={"query": "limit 1"})
