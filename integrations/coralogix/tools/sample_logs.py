"""Reading a capped number of Coralogix's log lines, after something narrowed the question.
...
Source of truth: Coralogix's ``/api/v1/dataprime/query``.
"""
...
TOOL_NAME = "coralogix_sample_logs"
...
@tool(
    name=TOOL_NAME,
    display_name="Coralogix log samples",
    description=(
...
async def coralogix_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Coralogix's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from Coralogix{tail}",
                reference=f"coralogix:sample_logs:{query or 'all'}",
            ),
        ),
    )
