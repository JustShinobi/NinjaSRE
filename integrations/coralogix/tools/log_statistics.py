"""Counting Coralogix's log lines before reading any of them.

The first call of an investigation that reaches Coralogix, and the one
...
Source of truth: Coralogix's ``/api/v1/dataprime/query``, read through the
...
TOOL_NAME = "coralogix_log_statistics"
...
DEFAULT_GROUP_BY = "userData.severity"
...
@tool(
    name=TOOL_NAME,
    display_name="Coralogix log statistics",
    description=(
...
async def coralogix_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Coralogix's log lines across ``group_by``.

    ``start`` and ``end`` are Coralogix's own time expressions. Both are
    ...
    client = access.client(CoralogixClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"coralogix:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by Coralogix for this request"
