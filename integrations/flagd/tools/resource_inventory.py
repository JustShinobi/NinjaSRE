"""Counting flagd's resources before reading any of them.

The first call of an investigation that reaches flagd, and the one
...
Source of truth: flagd's ``/flagd.evaluation.v1.Service/ResolveAll``, read through the
...
TOOL_NAME = "flagd_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="flagd resource inventory",
    description=(
...
async def flagd_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of flagd's resources across ``group_by``.

    ``start`` and ``end`` are flagd's own time expressions. Both are
    ...
    client = access.client(FlagdClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"flagd:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by flagd for this request"
