"""Reading a capped number of flagd's changes, after something narrowed the question.
...
Source of truth: flagd's ``/flagd.evaluation.v1.Service/ResolveAll``.
"""
...
TOOL_NAME = "flagd_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="flagd recent changes",
    description=(
...
async def flagd_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of flagd's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from flagd{tail}",
                reference=f"flagd:recent_changes:{kind or 'all'}",
            ),
        ),
    )
