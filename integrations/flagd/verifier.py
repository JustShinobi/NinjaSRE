"""Proves a flagd credential works, and proves what it is allowed to do.
...
from integrations.flagd.client import FlagdClient
from integrations.flagd.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for flagd, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "flagd rejected the credential. Re-issue it: your reverse proxy or service mesh — flagd's evaluation API has no authentication of its own"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No flagd credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "flagd has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="evaluation:resolve",
    grants="resolve every flag for a context",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="healthz",
    grants="read liveness, which the connectivity probe uses",
    capabilities=("flagd_resource_inventory",),
    where=_GRANTED_AT,
)
...
class FlagdVerifier:
    """Checks a stored flagd credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /healthz — the cheapest read flagd offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether flagd accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="flagd accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what flagd said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> FlagdClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a flagd probe needs a proxy transport and a request context")
        return FlagdClient(transport=transport, context=context, region=self.region)
