"""flagd's API, over the proxy, holding no credential.
...
flagd's authentication changed tomorrow, because the authentication is
...
from integrations.flagd.schema import INTEGRATION, base_url

PING_PATH: Final = "/healthz"
LIST_RESOURCES_PATH: Final = "/flagd.evaluation.v1.Service/ResolveAll"
LIST_CHANGES_PATH: Final = "/flagd.evaluation.v1.Service/ResolveAll"
...
class FlagdClient(IntegrationClient):
    """flagd, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what flagd holds for this request, bounded.
        ...
        """
        # flagd narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            answer = (await self.post(LIST_RESOURCES_PATH, json_body=body)).json()
            return Page(items=records(answer, "flags"), cursor="" or None)
