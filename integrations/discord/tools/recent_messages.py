"""Reading a capped number of Discord's messages, after something narrowed the question.
...
TOOL_NAME = "discord_recent_messages"
...
@tool(
    name=TOOL_NAME,
    display_name="Discord recent messages",
    description=(
...
async def discord_recent_messages(
    channel: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Discord's messages, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} messages from Discord{tail}",
                reference=f"discord:recent_messages:{channel or 'all'}",
            ),
        ),
    )
