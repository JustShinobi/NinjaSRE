"""Discord's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "discord"


#: Every host Discord serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="discord.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Discord bot token", min_length=8),
    public("channel_id", "Default channel id", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        HeaderInjection(header="Authorization", field="token", template="Bot {value}"),
        PathSegmentInjection(placeholder="channel_id", field="channel_id"),
    ),
)
