"""Discord's API, over the proxy, holding no credential.
...
Discord's authentication changed tomorrow, because the authentication is
...
from integrations.discord.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v10/channels/{channel_id}"
RECENT_MESSAGES_PATH: Final = "/api/v10/channels/{channel_id}/messages"
POST_MESSAGE_PATH: Final = "/api/v10/channels/{channel_id}/messages"
...
class DiscordClient(IntegrationClient):
    """Discord, reached through the credential proxy."""
    ...
    async def recent_messages(
        ...
        *,
        ...
        """Return what Discord holds for this request, bounded.
        ...
        """
        # Discord narrows this endpoint by other means; the argument stays in the
        # signature so every read in the catalogue is asked for the same way.
        del channel, start, end
    ...
    async def post_message(self, channel: str, text: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Discord said."""
        # The proxy supplies this from the stored configuration, so the client
        # never carries it and a caller cannot point the change somewhere else.
        del channel
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Discord offers.
