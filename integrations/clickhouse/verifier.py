"""Proves a ClickHouse credential works, and proves what it is allowed to do.
...
#: What each failure means for ClickHouse, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "ClickHouse rejected the credential. Re-issue it: your ClickHouse users.xml or the cloud console's SQL console user"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No ClickHouse credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "your ClickHouse users.xml or the cloud console's SQL console user"
...
_NO_INTROSPECTION: Final = (
    "ClickHouse has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="SELECT on system.processes",
    grants="read what is executing now",
    capabilities=("clickhouse_session_statistics",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="SELECT on system.query_log",
    grants="read the completed-query log",
    capabilities=("clickhouse_slow_queries",),
    where=_GRANTED_AT,
)
...
class ClickhouseVerifier:
    """Checks a stored ClickHouse credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /ping — the cheapest read ClickHouse offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether ClickHouse accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="ClickHouse accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what ClickHouse said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> ClickhouseClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a clickhouse probe needs a proxy transport and a request context")
        return ClickhouseClient(transport=transport, context=context, region=self.region)
