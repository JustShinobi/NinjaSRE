"""ClickHouse's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "clickhouse"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "clickhouse.example.com"

#: Every host ClickHouse serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="clickhouse.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "ClickHouse user with access to the system tables", min_length=8),
    secret("password", "That user's password", min_length=8),
    public("database", "Default database for queries"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    ClickHouse is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="clickhouse.acme.example")``.
    """
