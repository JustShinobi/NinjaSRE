"""Counting ClickHouse's sessions before reading any of them.

The first call of an investigation that reaches ClickHouse, and the one
...
Source of truth: ClickHouse's ``/``, read through the
...
from integrations.clickhouse.client import ClickhouseClient
from integrations.clickhouse.schema import INTEGRATION

TOOL_NAME = "clickhouse_session_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="ClickHouse session statistics",
    description=(
...
async def clickhouse_session_statistics(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of ClickHouse's sessions across ``group_by``.

    ``start`` and ``end`` are ClickHouse's own time expressions. Both are
    ...
    client = access.client(ClickhouseClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"clickhouse:session_statistics:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no sessions were returned by ClickHouse for this request"
