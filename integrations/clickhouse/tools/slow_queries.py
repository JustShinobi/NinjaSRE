"""Reading a capped number of ClickHouse's statements, after something narrowed the question.
...
TOOL_NAME = "clickhouse_slow_queries"
...
@tool(
    name=TOOL_NAME,
    display_name="ClickHouse slow queries",
    description=(
...
async def clickhouse_slow_queries(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of ClickHouse's statements, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.ANALYSIS,
                summary=f"{len(entries)} statements from ClickHouse{tail}",
                reference=f"clickhouse:slow_queries:{scope or 'all'}",
            ),
        ),
    )
