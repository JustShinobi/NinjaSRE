"""ClickHouse's API, over the proxy, holding no credential.
...
ClickHouse's authentication changed tomorrow, because the authentication is
...
from integrations.clickhouse.schema import INTEGRATION, base_url

PING_PATH: Final = "/ping"
LIST_SESSIONS_PATH: Final = "/"
SLOW_QUERIES_PATH: Final = "/"
...
PAGINATION: Final[tuple[EndpointPagination, ...]] = (
    EndpointPagination(
        ...
        parameter="offset",
        page_size_parameter="max_result_rows",
        page_size=100,
    ),
    EndpointPagination(
...
class ClickhouseClient(IntegrationClient):
    """ClickHouse, reached through the credential proxy."""
    ...
    async def list_sessions(
        ...
        *,
        ...
        """Return what ClickHouse holds for this request, bounded.
        ...
        """
        # ClickHouse narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {
                "query": "SELECT query_id, user, elapsed, read_rows FROM system.processes FORMAT JSON",
                "default_format": "JSON",
            }
    ...
    async def slow_queries(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {
                "query": "SELECT query, query_duration_ms, type FROM system.query_log WHERE type != 'QueryStart' ORDER BY query_duration_ms DESC LIMIT 20 FORMAT JSON",
                "default_format": "JSON",
            }
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call ClickHouse offers.
