"""Proves a Better Stack credential works, and proves what it is allowed to do.
...
from integrations.better_stack.client import BetterStackClient
from integrations.better_stack.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Better Stack, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Better Stack rejected the credential. Re-issue it: Better Stack → Settings → API tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Better Stack credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Better Stack → Settings → API tokens"
...
_NO_INTROSPECTION: Final = (
    "Better Stack has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="logs:read",
    grants="query the sources this token can see",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="sources:read",
    grants="list the sources a query may name",
    capabilities=("better_stack_log_statistics",),
    where=_GRANTED_AT,
)
...
class BetterStackVerifier:
    """Checks a stored Better Stack credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v2/query/live-tail — the cheapest read Better Stack offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Better Stack accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Better Stack accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Better Stack said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> BetterStackClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a better_stack probe needs a proxy transport and a request context")
        return BetterStackClient(transport=transport, context=context, region=self.region)
