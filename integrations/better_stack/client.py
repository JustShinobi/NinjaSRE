"""Better Stack's API, over the proxy, holding no credential.
...
Better Stack's authentication changed tomorrow, because the authentication is
...
from integrations.better_stack.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v2/query/live-tail"
SEARCH_LOGS_PATH: Final = "/api/v2/query/live-tail"
RECENT_LOGS_PATH: Final = "/api/v2/query/live-tail"
...
class BetterStackClient(IntegrationClient):
    """Better Stack, reached through the credential proxy."""
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what Better Stack holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"query": query, "from": start, "to": end}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Better Stack offers.
        ...
        """
        return await self.get(PING_PATH, params={"batch": "1"})
