"""Counting Better Stack's log lines before reading any of them.

The first call of an investigation that reaches Better Stack, and the one
...
Source of truth: Better Stack's ``/api/v2/query/live-tail``, read through the
...
TOOL_NAME = "better_stack_log_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Better Stack log statistics",
    description=(
...
async def better_stack_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Better Stack's log lines across ``group_by``.

    ``start`` and ``end`` are Better Stack's own time expressions. Both are
    ...
    client = access.client(BetterStackClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"better_stack:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by Better Stack for this request"
