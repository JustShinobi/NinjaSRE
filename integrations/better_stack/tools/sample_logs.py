"""Reading a capped number of Better Stack's log lines, after something narrowed the question.
...
Source of truth: Better Stack's ``/api/v2/query/live-tail``.
"""
...
TOOL_NAME = "better_stack_sample_logs"
...
@tool(
    name=TOOL_NAME,
    display_name="Better Stack log samples",
    description=(
...
async def better_stack_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Better Stack's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from Better Stack{tail}",
                reference=f"better_stack:sample_logs:{query or 'all'}",
            ),
        ),
    )
