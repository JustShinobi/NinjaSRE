"""Counting Notion's issues before reading any of them.

The first call of an investigation that reaches Notion, and the one
...
Source of truth: Notion's ``/v1/search``, read through the
...
TOOL_NAME = "notion_issue_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Notion issue statistics",
    description=(
...
async def notion_issue_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Notion's issues across ``group_by``.

    ``start`` and ``end`` are Notion's own time expressions. Both are
    ...
    client = access.client(NotionClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"notion:issue_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no issues were returned by Notion for this request"
