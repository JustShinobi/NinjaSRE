"""Reading a capped number of Notion's issues, after something narrowed the question.
...
Source of truth: Notion's ``/v1/search``.
"""
...
TOOL_NAME = "notion_recent_issues"
...
@tool(
    name=TOOL_NAME,
    display_name="Notion recent issues",
    description=(
...
async def notion_recent_issues(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Notion's issues, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} issues from Notion{tail}",
                reference=f"notion:recent_issues:{query or 'all'}",
            ),
        ),
    )
