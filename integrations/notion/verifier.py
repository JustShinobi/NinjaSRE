"""Proves a Notion credential works, and proves what it is allowed to do.
...
from integrations.notion.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Notion, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Notion rejected the credential. Re-issue it: notion.so/my-integrations → your integration → Internal Integration Secret"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Notion credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "notion.so/my-integrations → your integration → Internal Integration Secret"
...
_NO_INTROSPECTION: Final = (
    "Notion has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="read content",
    grants="search and read pages the integration is shared with",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read the bot user, which the probe uses",
    capabilities=("notion_issue_statistics",),
    where=_GRANTED_AT,
)
...
class NotionVerifier:
    """Checks a stored Notion credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v1/users/me — the cheapest read Notion offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Notion accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Notion accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Notion said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> NotionClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a notion probe needs a proxy transport and a request context")
        return NotionClient(transport=transport, context=context, region=self.region)
