"""Notion's API, over the proxy, holding no credential.
...
Notion's authentication changed tomorrow, because the authentication is
...
from integrations.notion.schema import INTEGRATION, base_url

PING_PATH: Final = "/v1/users/me"
SEARCH_ISSUES_PATH: Final = "/v1/search"
RECENT_ISSUES_PATH: Final = "/v1/search"
...
class NotionClient(IntegrationClient):
    """Notion, reached through the credential proxy."""
    ...
    async def search_issues(
        ...
        *,
        ...
        """Return what Notion holds for this request, bounded.
        ...
        """
        # Notion narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(
                items=records(answer, "results"), cursor=text(answer, "next_cursor") or None
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Notion offers.
