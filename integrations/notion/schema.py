"""Notion's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "notion"


#: Every host Notion serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.notion.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Notion internal integration secret", min_length=8),
    secret("api_version", "Notion API version, sent on every call", min_length=8),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        HeaderInjection(header="Authorization", field="token", template="Bearer {value}"),
        HeaderInjection(header="Notion-Version", field="api_version"),
    ),
)
