"""Twilio's API, over the proxy, holding no credential.
...
Twilio's authentication changed tomorrow, because the authentication is
...
from integrations.twilio.schema import INTEGRATION, base_url

PING_PATH: Final = "/2010-04-01/Accounts/{account_sid}.json"
RECENT_MESSAGES_PATH: Final = "/2010-04-01/Accounts/{account_sid}/Messages.json"
POST_MESSAGE_PATH: Final = "/2010-04-01/Accounts/{account_sid}/Messages.json"
...
class TwilioClient(IntegrationClient):
    """Twilio, reached through the credential proxy."""
    ...
    async def recent_messages(
        ...
        *,
        ...
        """Return what Twilio holds for this request, bounded.
        ...
        """
        # Twilio narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"To": channel}
            asked.update(parameters)
            answer = (await self.get(RECENT_MESSAGES_PATH, params=asked)).json()
            return Page(items=records(answer, "messages"), cursor="" or None)
    ...
    async def post_message(self, channel: str, text: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Twilio said."""
        body: dict[str, Any] = {"To": channel, "Body": text}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Twilio offers.
