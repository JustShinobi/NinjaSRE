"""Proves a Twilio credential works, and proves what it is allowed to do.
...
from integrations.twilio.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Twilio, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Twilio rejected the credential. Re-issue it: console.twilio.com → Account → API keys & tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Twilio credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "console.twilio.com → Account → API keys & tokens"
...
_NO_INTROSPECTION: Final = (
    "Twilio has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="messages:read",
    grants="read the account's message history",
    capabilities=("twilio_recent_messages",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="messages:create",
    grants="send a message from the account",
    capabilities=("twilio_post_message",),
    where=_GRANTED_AT,
)
...
class TwilioVerifier:
    """Checks a stored Twilio credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /2010-04-01/Accounts/{account_sid}.json — the cheapest read Twilio offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Twilio accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Twilio accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Twilio said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> TwilioClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a twilio probe needs a proxy transport and a request context")
        return TwilioClient(transport=transport, context=context, region=self.region)
