"""Reading a capped number of Twilio's messages, after something narrowed the question.
...
TOOL_NAME = "twilio_recent_messages"
...
@tool(
    name=TOOL_NAME,
    display_name="Twilio recent messages",
    description=(
...
async def twilio_recent_messages(
    channel: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Twilio's messages, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} messages from Twilio{tail}",
                reference=f"twilio:recent_messages:{channel or 'all'}",
            ),
        ),
    )
