"""Twilio: the one change this integration is allowed to make.
...
Source of truth: Twilio's ``/2010-04-01/Accounts/{account_sid}/Messages.json``.
"""
...
class TwilioPostMessageRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the Twilio change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what Twilio returned for {target}, including its identifier.",
                "Undo the change in Twilio using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="Twilio post message",
    description=(
    ...
    rollback_planner=TwilioPostMessageRollback(),
    tags=(
        "communication",
        "twilio",
        "write",
...
async def twilio_post_message(channel: str, text: str) -> CapabilityResult:
    """Make the change in Twilio and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(TwilioClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"Twilio accepted the change to {channel!r}",
                reference=f"twilio:post_message:{channel}",
            ),
        ),
    )
