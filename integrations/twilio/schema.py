"""Twilio's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "twilio"


#: Every host Twilio serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.twilio.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret(
        "username", "Twilio account SID, which doubles as the basic-auth username", min_length=8
    ),
    secret("password", "Twilio auth token, or an API key secret", min_length=8),
    public("account_sid", "Twilio account SID used in the path", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="account_sid", field="account_sid"),
    ),
)
