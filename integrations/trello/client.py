"""Trello's API, over the proxy, holding no credential.
...
Trello's authentication changed tomorrow, because the authentication is
...
from integrations.trello.schema import INTEGRATION, base_url

PING_PATH: Final = "/1/boards/{board}"
SEARCH_ISSUES_PATH: Final = "/1/boards/{board}/cards"
RECENT_ISSUES_PATH: Final = "/1/boards/{board}/cards"
...
class TrelloClient(IntegrationClient):
    """Trello, reached through the credential proxy."""
    ...
    async def search_issues(
        ...
        *,
        ...
        """Return what Trello holds for this request, bounded.
        ...
        """
        # Trello narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"fields": "name,idList,dateLastActivity"}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Trello offers.
        ...
        """
        return await self.get(PING_PATH, params={"fields": "id"})
