"""Trello's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "trello"


#: Every host Trello serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.trello.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Trello API token for the member whose boards are read", min_length=8),
    public("board", "Trello board id", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        QueryParameterInjection(parameter="token", field="token"),
        PathSegmentInjection(placeholder="board", field="board"),
    ),
)
