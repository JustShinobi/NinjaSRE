"""Reading a capped number of Trello's issues, after something narrowed the question.
...
Source of truth: Trello's ``/1/boards/{board}/cards``.
"""
...
TOOL_NAME = "trello_recent_issues"
...
@tool(
    name=TOOL_NAME,
    display_name="Trello recent issues",
    description=(
...
async def trello_recent_issues(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Trello's issues, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} issues from Trello{tail}",
                reference=f"trello:recent_issues:{query or 'all'}",
            ),
        ),
    )
