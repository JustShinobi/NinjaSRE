"""Counting Trello's issues before reading any of them.

The first call of an investigation that reaches Trello, and the one
...
Source of truth: Trello's ``/1/boards/{board}/cards``, read through the
...
TOOL_NAME = "trello_issue_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Trello issue statistics",
    description=(
...
async def trello_issue_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Trello's issues across ``group_by``.

    ``start`` and ``end`` are Trello's own time expressions. Both are
    ...
    client = access.client(TrelloClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"trello:issue_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no issues were returned by Trello for this request"
