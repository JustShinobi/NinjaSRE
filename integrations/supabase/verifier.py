"""Proves a Supabase credential works, and proves what it is allowed to do.
...
#: What each failure means for Supabase, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Supabase rejected the credential. Re-issue it: Supabase → Account → Access Tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Supabase credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Supabase → Account → Access Tokens"
...
_NO_INTROSPECTION: Final = (
    "Supabase has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="projects:read",
    grants="list projects and their status",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read organisations, which the probe uses",
    capabilities=("supabase_session_statistics",),
    where=_GRANTED_AT,
)
...
class SupabaseVerifier:
    """Checks a stored Supabase credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v1/organizations — the cheapest read Supabase offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Supabase accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Supabase accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Supabase said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> SupabaseClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a supabase probe needs a proxy transport and a request context")
        return SupabaseClient(transport=transport, context=context, region=self.region)
