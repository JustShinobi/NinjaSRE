"""Reading a capped number of Supabase's statements, after something narrowed the question.
...
Source of truth: Supabase's ``/v1/projects``.
"""
...
TOOL_NAME = "supabase_slow_queries"
...
@tool(
    name=TOOL_NAME,
    display_name="Supabase slow queries",
    description=(
...
async def supabase_slow_queries(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Supabase's statements, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.ANALYSIS,
                summary=f"{len(entries)} statements from Supabase{tail}",
                reference=f"supabase:slow_queries:{scope or 'all'}",
            ),
        ),
    )
