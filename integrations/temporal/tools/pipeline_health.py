"""Counting Temporal's tasks before reading any of them.

The first call of an investigation that reaches Temporal, and the one
...
Source of truth: Temporal's ``/api/v1/namespaces/{namespace}/workflows``, read through the
...
TOOL_NAME = "temporal_pipeline_health"
...
@tool(
    name=TOOL_NAME,
    display_name="Temporal pipeline health",
    description=(
...
async def temporal_pipeline_health(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Temporal's tasks across ``group_by``.

    ``start`` and ``end`` are Temporal's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"temporal:pipeline_health:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no tasks were returned by Temporal for this request"
