"""Temporal's API, over the proxy, holding no credential.
...
Temporal's authentication changed tomorrow, because the authentication is
...
from integrations.temporal.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v1/namespaces/{namespace}"
LIST_TASKS_PATH: Final = "/api/v1/namespaces/{namespace}/workflows"
LIST_FAILURES_PATH: Final = "/api/v1/namespaces/{namespace}/workflows"
...
class TemporalClient(IntegrationClient):
    """Temporal, reached through the credential proxy."""
    ...
    async def list_tasks(
        ...
        *,
        ...
        """Return what Temporal holds for this request, bounded.
        ...
        """
        # Temporal narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"query": scope}
            asked.update(parameters)
            answer = (await self.get(LIST_TASKS_PATH, params=asked)).json()
            return Page(
                items=records(answer, "executions"), cursor=text(answer, "nextPageToken") or None
            )
    ...
    async def list_failures(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"query": "ExecutionStatus = 'Failed'"}
            asked.update(parameters)
            answer = (await self.get(LIST_FAILURES_PATH, params=asked)).json()
            return Page(
                items=records(answer, "executions"), cursor=text(answer, "nextPageToken") or None
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Temporal offers.
