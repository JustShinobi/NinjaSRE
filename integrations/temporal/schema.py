"""Temporal's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "temporal"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "temporal.example.com"

#: Every host Temporal serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="temporal.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    ...
    secret(
        "token",
        "Temporal Cloud API key, or the token your self-hosted frontend accepts",
        min_length=8,
    ),
    public("namespace", "Temporal namespace", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="namespace", field="namespace"),
    ),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Temporal is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="temporal.acme.example")``.
    """
