"""GitHub's API, over the proxy, holding no credential.
...
GitHub's authentication changed tomorrow, because the authentication is
...
LIST_PULL_REQUESTS_PATH: Final = "/search/issues"
...
class GithubClient(IntegrationClient):
    """GitHub, reached through the credential proxy."""
    ...
    async def list_commits(
        ...
        *,
        ...
        """Return what GitHub holds for this request, bounded.
        ...
        """
        # GitHub narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"q": repository, "sort": "committer-date", "order": "desc"}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call GitHub offers.
