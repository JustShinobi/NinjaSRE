"""Reading a capped number of Google Docs's issues, after something narrowed the question.
...
Source of truth: Google Docs's ``/drive/v3/files``.
"""
...
TOOL_NAME = "google_docs_recent_issues"
...
@tool(
    name=TOOL_NAME,
    display_name="Google Docs recent issues",
    description=(
...
async def google_docs_recent_issues(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Google Docs's issues, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} issues from Google Docs{tail}",
                reference=f"google_docs:recent_issues:{query or 'all'}",
            ),
        ),
    )
