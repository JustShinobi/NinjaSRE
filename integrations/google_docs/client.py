"""Google Docs's API, over the proxy, holding no credential.
...
Google Docs's authentication changed tomorrow, because the authentication is
...
from integrations.google_docs.schema import INTEGRATION, base_url

PING_PATH: Final = "/drive/v3/about"
SEARCH_ISSUES_PATH: Final = "/drive/v3/files"
RECENT_ISSUES_PATH: Final = "/drive/v3/files"
...
class GoogleDocsClient(IntegrationClient):
    """Google Docs, reached through the credential proxy."""
    ...
    async def search_issues(
        ...
        *,
        ...
        """Return what Google Docs holds for this request, bounded.
        ...
        """
        # Google Docs narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(
                items=records(answer, "files"), cursor=text(answer, "nextPageToken") or None
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Google Docs offers.
        ...
        """
        return await self.get(PING_PATH, params={"fields": "user"})
