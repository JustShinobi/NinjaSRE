"""Proves a Google Docs credential works, and proves what it is allowed to do.
...
#: What each failure means for Google Docs, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Google Docs rejected the credential. Re-issue it: IAM & Admin → Service Accounts → Keys, exchanged for an access token with the drive.readonly scope"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Google Docs credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "IAM & Admin → Service Accounts → Keys, exchanged for an access token with the drive.readonly scope"
...
_NO_INTROSPECTION: Final = (
    "Google Docs has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="drive.readonly",
    grants="list and read document metadata",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read the token's own account, which the probe uses",
    capabilities=("google_docs_issue_statistics",),
    where=_GRANTED_AT,
)
...
class GoogleDocsVerifier:
    """Checks a stored Google Docs credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /drive/v3/about — the cheapest read Google Docs offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Google Docs accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Google Docs accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Google Docs said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> GoogleDocsClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a google_docs probe needs a proxy transport and a request context")
        return GoogleDocsClient(transport=transport, context=context, region=self.region)
