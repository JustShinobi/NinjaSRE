"""Google Docs's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "google_docs"


#: Every host Google Docs serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="www.googleapis.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret(
        "token", "OAuth access token for a service account with Drive read access", min_length=8
    ),
)
