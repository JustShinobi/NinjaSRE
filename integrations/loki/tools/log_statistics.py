"""Counting Loki's log lines before reading any of them.

The first call of an investigation that reaches Loki, and the one
...
Source of truth: Loki's ``/loki/api/v1/query_range``, read through the
...
@tool(
    name=TOOL_NAME,
    display_name="Loki log statistics",
    description=(
...
async def loki_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Loki's log lines across ``group_by``.

    ``start`` and ``end`` are Loki's own time expressions. Both are
    ...
    client = access.client(LokiClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"loki:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by Loki for this request"
