"""Reading a capped number of Loki's log lines, after something narrowed the question.
...
Source of truth: Loki's ``/loki/api/v1/query_range``.
"""
...
async def loki_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Loki's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from Loki{tail}",
                reference=f"loki:sample_logs:{query or 'all'}",
            ),
        ),
    )
