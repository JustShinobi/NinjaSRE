"""Proves a WhatsApp credential works, and proves what it is allowed to do.
...
from integrations.whatsapp.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for WhatsApp, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "WhatsApp rejected the credential. Re-issue it: developers.facebook.com → your app → WhatsApp → API Setup → access token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No WhatsApp credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "developers.facebook.com → your app → WhatsApp → API Setup → access token"
...
_NO_INTROSPECTION: Final = (
    "WhatsApp has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_1: Final = RequiredPermission(
    ...
    grants="read the number's message templates",
    capabilities=("whatsapp_recent_messages",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="whatsapp_business_messaging",
    grants="send a message from the number",
    capabilities=("whatsapp_post_message",),
    where=_GRANTED_AT,
)
...
class WhatsappVerifier:
    """Checks a stored WhatsApp credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v21.0/{phone_number_id} — the cheapest read WhatsApp offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether WhatsApp accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="WhatsApp accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what WhatsApp said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> WhatsappClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a whatsapp probe needs a proxy transport and a request context")
        return WhatsappClient(transport=transport, context=context, region=self.region)
