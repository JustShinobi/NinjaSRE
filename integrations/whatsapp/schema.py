"""WhatsApp's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "whatsapp"


#: Every host WhatsApp serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="graph.facebook.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Meta system user access token with whatsapp_business_messaging", min_length=8),
    public("phone_number_id", "WhatsApp Business phone number id", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="phone_number_id", field="phone_number_id"),
    ),
)
