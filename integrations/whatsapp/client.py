"""WhatsApp's API, over the proxy, holding no credential.
...
WhatsApp's authentication changed tomorrow, because the authentication is
...
from integrations.whatsapp.schema import INTEGRATION, base_url

PING_PATH: Final = "/v21.0/{phone_number_id}"
RECENT_MESSAGES_PATH: Final = "/v21.0/{phone_number_id}/message_templates"
POST_MESSAGE_PATH: Final = "/v21.0/{phone_number_id}/messages"
...
class WhatsappClient(IntegrationClient):
    """WhatsApp, reached through the credential proxy."""
    ...
    async def recent_messages(
        ...
        *,
        ...
        """Return what WhatsApp holds for this request, bounded.
        ...
        """
        # WhatsApp narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(
                items=records(answer, "data"),
                cursor=text(answer, "paging", "cursors", "after") or None,
            )
    ...
    async def post_message(self, channel: str, text: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what WhatsApp said."""
        body: dict[str, Any] = {
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call WhatsApp offers.
