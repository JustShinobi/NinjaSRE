"""Reading a capped number of WhatsApp's messages, after something narrowed the question.
...
Source of truth: WhatsApp's ``/v21.0/{phone_number_id}/message_templates``.
"""
...
TOOL_NAME = "whatsapp_recent_messages"
...
@tool(
    name=TOOL_NAME,
    display_name="WhatsApp recent messages",
    description=(
...
async def whatsapp_recent_messages(
    channel: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of WhatsApp's messages, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} messages from WhatsApp{tail}",
                reference=f"whatsapp:recent_messages:{channel or 'all'}",
            ),
        ),
    )
