"""WhatsApp: the one change this integration is allowed to make.
...
Source of truth: WhatsApp's ``/v21.0/{phone_number_id}/messages``.
"""
...
TOOL_NAME = "whatsapp_post_message"
...
class WhatsappPostMessageRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the WhatsApp change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what WhatsApp returned for {target}, including its identifier.",
                "Undo the change in WhatsApp using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="WhatsApp post message",
    description=(
    ...
    rollback_planner=WhatsappPostMessageRollback(),
    tags=(
        "communication",
        "whatsapp",
        "write",
...
async def whatsapp_post_message(channel: str, text: str) -> CapabilityResult:
    """Make the change in WhatsApp and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(WhatsappClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"WhatsApp accepted the change to {channel!r}",
                reference=f"whatsapp:post_message:{channel}",
            ),
        ),
    )
