"""New Relic's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "new_relic"


#: Every host New Relic serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap(
    integration=INTEGRATION,
    regions=(
        Region(name="us", host="api.newrelic.com", display_name="United States"),
        Region(name="eu", host="api.eu.newrelic.com", display_name="Europe"),
    ),
    default="us",
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("api_key", "New Relic user API key, which is the one that can run NRQL", min_length=8),
    public("account", "New Relic account id, required by every NRQL query"),
)

RULE: Final = InjectionRule(
    integration=INTEGRATION,
    hosts=HOSTS,
    injections=(HeaderInjection(header="API-Key", field="api_key", template="{value}"),),
)
