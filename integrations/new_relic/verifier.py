"""Proves a New Relic credential works, and proves what it is allowed to do.
...
#: What each failure means for New Relic, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "New Relic rejected the credential. Re-issue it: one.newrelic.com → API keys → Create a key, of type User"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No New Relic credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "one.newrelic.com → API keys → Create a key, of type User"
...
_NO_INTROSPECTION: Final = (
    "New Relic has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="apm:read",
    grants="read application summaries and their health",
    capabilities=("new_relic_metric_statistics",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="alerts:read",
    grants="read open alert violations",
    capabilities=("new_relic_active_alerts",),
    where=_GRANTED_AT,
)
...
class NewRelicVerifier:
    """Checks a stored New Relic credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v2/applications.json — the cheapest read New Relic offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether New Relic accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="New Relic accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what New Relic said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> NewRelicClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a new_relic probe needs a proxy transport and a request context")
        return NewRelicClient(transport=transport, context=context, region=self.region)
