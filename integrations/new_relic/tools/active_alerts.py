"""Reading a capped number of New Relic's alerts, after something narrowed the question.
...
Source of truth: New Relic's ``/v2/alerts_violations.json``.
"""
...
TOOL_NAME = "new_relic_active_alerts"
...
@tool(
    name=TOOL_NAME,
    display_name="New Relic active alerts",
    description=(
...
async def new_relic_active_alerts(
    state: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of New Relic's alerts, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} alerts from New Relic{tail}",
                reference=f"new_relic:active_alerts:{state or 'all'}",
            ),
        ),
    )
