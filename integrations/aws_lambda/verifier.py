"""Proves a AWS Lambda credential works, and proves what it is allowed to do.
...
from integrations.aws_lambda.client import AwsLambdaClient
from integrations.aws_lambda.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for AWS Lambda, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS Lambda rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS Lambda credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "IAM → Users → Security credentials, or the role your deployment assumes"
...
_NO_INTROSPECTION: Final = (
    "AWS Lambda has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="lambda:ListFunctions",
    grants="list the functions in this region",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read a function's runtime, memory, and last modification",
    capabilities=("aws_lambda_recent_changes",),
    where=_GRANTED_AT,
)
...
class AwsLambdaVerifier:
    """Checks a stored AWS Lambda credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /2015-03-31/functions/ — the cheapest read AWS Lambda offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS Lambda accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS Lambda accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS Lambda said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsLambdaClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_lambda probe needs a proxy transport and a request context")
        return AwsLambdaClient(transport=transport, context=context, region=self.region)
