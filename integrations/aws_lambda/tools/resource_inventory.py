"""Counting AWS Lambda's resources before reading any of them.

The first call of an investigation that reaches AWS Lambda, and the one
...
Source of truth: AWS Lambda's ``/2015-03-31/functions/``, read through the
...
TOOL_NAME = "aws_lambda_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS Lambda resource inventory",
    description=(
...
async def aws_lambda_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of AWS Lambda's resources across ``group_by``.

    ``start`` and ``end`` are AWS Lambda's own time expressions. Both are
    ...
    client = access.client(AwsLambdaClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"aws_lambda:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by AWS Lambda for this request"
