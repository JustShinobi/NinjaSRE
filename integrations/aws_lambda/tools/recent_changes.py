"""Reading a capped number of AWS Lambda's changes, after something narrowed the question.
...
Source of truth: AWS Lambda's ``/2015-03-31/functions/``.
"""
...
TOOL_NAME = "aws_lambda_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS Lambda recent changes",
    description=(
...
async def aws_lambda_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of AWS Lambda's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from AWS Lambda{tail}",
                reference=f"aws_lambda:recent_changes:{kind or 'all'}",
            ),
        ),
    )
