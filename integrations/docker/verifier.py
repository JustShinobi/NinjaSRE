"""Proves a Docker credential works, and proves what it is allowed to do.
...
from integrations.docker.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Docker, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Docker rejected the credential. Re-issue it: your TLS-terminating proxy — the Engine API has no authentication of its own and must never be exposed without one"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Docker credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "your TLS-terminating proxy — the Engine API has no authentication of its own and must never be exposed without one"
...
_NO_INTROSPECTION: Final = (
    "Docker has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="containers:list",
    grants="list containers and their state",
    capabilities=("docker_resource_inventory",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="events:read",
    grants="read the engine event stream",
    capabilities=("docker_recent_changes",),
    where=_GRANTED_AT,
)
...
class DockerVerifier:
    """Checks a stored Docker credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /version — the cheapest read Docker offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Docker accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Docker accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Docker said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> DockerClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a docker probe needs a proxy transport and a request context")
        return DockerClient(transport=transport, context=context, region=self.region)
