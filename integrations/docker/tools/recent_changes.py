"""Reading a capped number of Docker's changes, after something narrowed the question.
...
Source of truth: Docker's ``/events``.
"""
...
TOOL_NAME = "docker_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="Docker recent changes",
    description=(
...
async def docker_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Docker's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from Docker{tail}",
                reference=f"docker:recent_changes:{kind or 'all'}",
            ),
        ),
    )
