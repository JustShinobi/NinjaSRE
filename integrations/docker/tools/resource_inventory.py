"""Counting Docker's resources before reading any of them.

The first call of an investigation that reaches Docker, and the one
...
Source of truth: Docker's ``/containers/json``, read through the
...
TOOL_NAME = "docker_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="Docker resource inventory",
    description=(
...
async def docker_resource_inventory(
    ...
    """Return the distribution of Docker's resources across ``group_by``.

    ``start`` and ``end`` are Docker's own time expressions. Both are
    ...
    client = access.client(DockerClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"docker:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by Docker for this request"
