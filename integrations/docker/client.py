"""Docker's API, over the proxy, holding no credential.
...
Docker's authentication changed tomorrow, because the authentication is
...
from integrations.docker.schema import INTEGRATION, base_url

PING_PATH: Final = "/version"
LIST_RESOURCES_PATH: Final = "/containers/json"
LIST_CHANGES_PATH: Final = "/events"
...
class DockerClient(IntegrationClient):
    """Docker, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what Docker holds for this request, bounded.
        ...
        """
        # Docker narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"all": "true", "filters": kind}
    ...
    async def list_changes(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"since": start, "until": end, "filters": kind}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Docker offers.
