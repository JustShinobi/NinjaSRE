"""Docker's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "docker"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "docker.example.com"

#: Every host Docker serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="docker.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Bearer token accepted by whatever fronts the Docker Engine API", min_length=8),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Docker is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="docker.acme.example")``.
    """
