"""Jenkins's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "jenkins"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "jenkins.example.com"

#: Every host Jenkins serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="jenkins.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "Jenkins user id", min_length=8),
    secret("password", "That user's API token, not their password", min_length=8),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Jenkins is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="jenkins.acme.example")``.
    """
