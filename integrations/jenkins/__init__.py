"""Jenkins: Jenkins build history: how a job has been doing lately, and the builds that failed, for the estates whose pipelines still run there.
...
from integrations.jenkins.client import PAGINATION, JenkinsClient
from integrations.jenkins.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.jenkins.verifier import PERMISSIONS, JenkinsVerifier
...
DESCRIPTOR: Final = IntegrationDescriptor(
    ...
    schema=SCHEMA,
    rule=RULE,
    verifier=JenkinsVerifier(),
    client_class=JenkinsClient,
