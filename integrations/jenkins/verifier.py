"""Proves a Jenkins credential works, and proves what it is allowed to do.
...
from integrations.jenkins.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Jenkins, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Jenkins rejected the credential. Re-issue it: your Jenkins user → Configure → API Token → Add new Token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Jenkins credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "your Jenkins user → Configure → API Token → Add new Token"
...
_NO_INTROSPECTION: Final = (
    "Jenkins has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="Overall/Read",
    grants="read the Jenkins instance at all",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="Job/Read",
    grants="read job configuration and build history",
    capabilities=(
        "jenkins_pipeline_statistics",
        "jenkins_failed_runs",
    ),
...
class JenkinsVerifier:
    """Checks a stored Jenkins credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/json — the cheapest read Jenkins offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Jenkins accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Jenkins accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Jenkins said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> JenkinsClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a jenkins probe needs a proxy transport and a request context")
        return JenkinsClient(transport=transport, context=context, region=self.region)
