"""Reading a capped number of Jenkins's runs, after something narrowed the question.
...
Source of truth: Jenkins's ``/api/json``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Jenkins failed runs",
    description=(
...
async def jenkins_failed_runs(
    project: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Jenkins's runs, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} runs from Jenkins{tail}",
                reference=f"jenkins:failed_runs:{project or 'all'}",
            ),
        ),
    )
