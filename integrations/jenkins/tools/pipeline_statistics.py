"""Counting Jenkins's runs before reading any of them.

The first call of an investigation that reaches Jenkins, and the one
...
Source of truth: Jenkins's ``/api/json``, read through the
...
TOOL_NAME = "jenkins_pipeline_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Jenkins pipeline statistics",
    description=(
...
async def jenkins_pipeline_statistics(
    project: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Jenkins's runs across ``group_by``.

    ``start`` and ``end`` are Jenkins's own time expressions. Both are
    ...
    client = access.client(JenkinsClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"jenkins:pipeline_statistics:{project or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no runs were returned by Jenkins for this request"
