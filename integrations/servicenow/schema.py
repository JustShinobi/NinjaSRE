"""ServiceNow's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "servicenow"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "acme.service-now.com"

#: Every host ServiceNow serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="acme.service-now.com", name="instance")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "ServiceNow integration user id", min_length=8),
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    ServiceNow is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="servicenow.acme.example")``.
    """
    named = {"instance": DEFAULT_HOST, **endpoints}
    return RegionMap(
        integration=INTEGRATION,
