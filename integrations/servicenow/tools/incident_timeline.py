"""Reading a capped number of ServiceNow's timeline entries, after something narrowed the question.
...
TOOL_NAME = "servicenow_incident_timeline"
...
@tool(
    name=TOOL_NAME,
    display_name="ServiceNow incident timeline",
    description=(
...
async def servicenow_incident_timeline(
    incident: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of ServiceNow's timeline entries, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"{len(entries)} timeline entries from ServiceNow{tail}",
                reference=f"servicenow:incident_timeline:{incident or 'all'}",
            ),
        ),
    )
