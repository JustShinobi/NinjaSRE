"""Counting ServiceNow's incidents before reading any of them.

The first call of an investigation that reaches ServiceNow, and the one
...
Source of truth: ServiceNow's ``/api/now/table/incident``, read through the
...
TOOL_NAME = "servicenow_incident_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="ServiceNow incident statistics",
    description=(
...
async def servicenow_incident_statistics(
    status: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of ServiceNow's incidents across ``group_by``.

    ``start`` and ``end`` are ServiceNow's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"servicenow:incident_statistics:{status or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no incidents were returned by ServiceNow for this request"
