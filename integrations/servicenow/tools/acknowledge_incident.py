"""ServiceNow: the one change this integration is allowed to make.
...
Source of truth: ServiceNow's ``/api/now/table/incident``.
"""
...
TOOL_NAME = "servicenow_acknowledge_incident"
...
class ServicenowAcknowledgeIncidentRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the ServiceNow change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what ServiceNow returned for {target}, including its identifier.",
                "Undo the change in ServiceNow using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="ServiceNow acknowledge incident",
    description=(
    ...
    rollback_planner=ServicenowAcknowledgeIncidentRollback(),
    tags=(
        "incident",
        "servicenow",
        "write",
...
async def servicenow_acknowledge_incident(incident: str, note: str) -> CapabilityResult:
    """Make the change in ServiceNow and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(ServicenowClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"ServiceNow accepted the change to {incident!r}",
                reference=f"servicenow:acknowledge_incident:{incident}",
            ),
        ),
    )
