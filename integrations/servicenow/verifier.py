"""Proves a ServiceNow credential works, and proves what it is allowed to do.
...
from integrations.servicenow.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for ServiceNow, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "ServiceNow rejected the credential. Re-issue it: ServiceNow → User Administration → Users, on the integration account"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No ServiceNow credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "ServiceNow → User Administration → Users, on the integration account"
...
_NO_INTROSPECTION: Final = (
    "ServiceNow has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="itil",
    grants="read and update incident records",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="rest_service",
    grants="reach the Table API at all",
    capabilities=(
        "servicenow_incident_statistics",
        "servicenow_incident_timeline",
        "servicenow_acknowledge_incident",
...
class ServicenowVerifier:
    """Checks a stored ServiceNow credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/now/table/incident — the cheapest read ServiceNow offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether ServiceNow accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="ServiceNow accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what ServiceNow said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> ServicenowClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a servicenow probe needs a proxy transport and a request context")
        return ServicenowClient(transport=transport, context=context, region=self.region)
