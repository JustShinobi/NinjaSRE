"""Azure SQL: Azure SQL through Resource Manager: which databases exist in a subscription and in what state, and their recent service-level events.
...
from integrations.azure_sql.client import PAGINATION, AzureSqlClient
from integrations.azure_sql.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.azure_sql.verifier import PERMISSIONS, AzureSqlVerifier
...
DESCRIPTOR: Final = IntegrationDescriptor(
    ...
    schema=SCHEMA,
    rule=RULE,
    verifier=AzureSqlVerifier(),
    client_class=AzureSqlClient,
    sdk_strategy=SdkStrategy.DIRECT_CLIENT,
    strategy_note=(
        "The Azure SDK resolves credentials through DefaultAzureCredential, which reads the environment, a managed identity endpoint, and a shared token cache."
    ),
)
