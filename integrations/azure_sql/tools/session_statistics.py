"""Counting Azure SQL's sessions before reading any of them.

The first call of an investigation that reaches Azure SQL, and the one
...
Source of truth: Azure SQL's ``/subscriptions/{subscription}/providers/Microsoft.Sql/servers``, read through the
...
TOOL_NAME = "azure_sql_session_statistics"
...
DEFAULT_GROUP_BY = "properties.state"
...
@tool(
    name=TOOL_NAME,
    display_name="Azure SQL session statistics",
    description=(
...
async def azure_sql_session_statistics(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Azure SQL's sessions across ``group_by``.

    ``start`` and ``end`` are Azure SQL's own time expressions. Both are
    ...
    client = access.client(AzureSqlClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"azure_sql:session_statistics:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no sessions were returned by Azure SQL for this request"
