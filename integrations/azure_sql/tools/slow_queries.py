"""Reading a capped number of Azure SQL's statements, after something narrowed the question.
...
Source of truth: Azure SQL's ``/subscriptions/{subscription}/providers/Microsoft.Sql/servers``.
"""
...
TOOL_NAME = "azure_sql_slow_queries"
...
@tool(
    name=TOOL_NAME,
    display_name="Azure SQL slow queries",
    description=(
...
async def azure_sql_slow_queries(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Azure SQL's statements, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.ANALYSIS,
                summary=f"{len(entries)} statements from Azure SQL{tail}",
                reference=f"azure_sql:slow_queries:{scope or 'all'}",
            ),
        ),
    )
