"""Proves a Azure SQL credential works, and proves what it is allowed to do.
...
from integrations.azure_sql.client import AzureSqlClient
from integrations.azure_sql.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Azure SQL, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Azure SQL rejected the credential. Re-issue it: Azure Portal → App registrations → your service principal, exchanged for a token against https://management.azure.com"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Azure SQL credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Azure Portal → App registrations → your service principal, exchanged for a token against https://management.azure.com"
...
_NO_INTROSPECTION: Final = (
    "Azure SQL has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_1: Final = RequiredPermission(
    ...
    grants="list logical servers",
    capabilities=("azure_sql_session_statistics",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="Microsoft.Sql/servers/databases/read",
    grants="read database state and service tier",
    capabilities=("azure_sql_slow_queries",),
    where=_GRANTED_AT,
)
...
class AzureSqlVerifier:
    """Checks a stored Azure SQL credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /subscriptions/{subscription}/providers/Microsoft.Sql/servers — the cheapest read Azure SQL offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Azure SQL accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Azure SQL accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Azure SQL said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AzureSqlClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a azure_sql probe needs a proxy transport and a request context")
        return AzureSqlClient(transport=transport, context=context, region=self.region)
