"""Counting Prefect's tasks before reading any of them.

The first call of an investigation that reaches Prefect, and the one
...
Source of truth: Prefect's ``/api/flow_runs/filter``, read through the
...
TOOL_NAME = "prefect_pipeline_health"
...
@tool(
    name=TOOL_NAME,
    display_name="Prefect pipeline health",
    description=(
...
async def prefect_pipeline_health(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Prefect's tasks across ``group_by``.

    ``start`` and ``end`` are Prefect's own time expressions. Both are
    ...
    client = access.client(PrefectClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"prefect:pipeline_health:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no tasks were returned by Prefect for this request"
