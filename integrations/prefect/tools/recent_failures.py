"""Reading a capped number of Prefect's failures, after something narrowed the question.
...
Source of truth: Prefect's ``/api/flow_runs/filter``.
"""
...
TOOL_NAME = "prefect_recent_failures"
...
@tool(
    name=TOOL_NAME,
    display_name="Prefect recent failures",
    description=(
...
async def prefect_recent_failures(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Prefect's failures, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} failures from Prefect{tail}",
                reference=f"prefect:recent_failures:{scope or 'all'}",
            ),
        ),
    )
