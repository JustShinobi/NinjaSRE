"""Proves a Prefect credential works, and proves what it is allowed to do.
...
from integrations.prefect.client import PrefectClient
from integrations.prefect.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Prefect, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Prefect rejected the credential. Re-issue it: Prefect Cloud → Account → API keys"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Prefect credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Prefect Cloud → Account → API keys"
...
_NO_INTROSPECTION: Final = (
    "Prefect has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="flow_runs:read",
    grants="read flow runs and their state",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="health:read",
    grants="read server health, which the probe uses",
    capabilities=("prefect_pipeline_health",),
    where=_GRANTED_AT,
)
...
class PrefectVerifier:
    """Checks a stored Prefect credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/health — the cheapest read Prefect offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Prefect accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Prefect accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Prefect said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> PrefectClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a prefect probe needs a proxy transport and a request context")
        return PrefectClient(transport=transport, context=context, region=self.region)
