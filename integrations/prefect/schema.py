"""Prefect's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "prefect"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "api.prefect.cloud"

#: Every host Prefect serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.prefect.cloud", name="cloud")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret(
        "token", "Prefect Cloud API key, or the token your self-hosted server accepts", min_length=8
    ),
    public("workspace", "Prefect Cloud workspace path, account/workspace"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Prefect is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="prefect.acme.example")``.
    """
    named = {"cloud": DEFAULT_HOST, **endpoints}
    return RegionMap(
        integration=INTEGRATION,
