"""Reading a capped number of Sentry's log lines, after something narrowed the question.
...
Source of truth: Sentry's ``/api/0/organizations/{organisation}/issues/``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Sentry log samples",
    description=(
...
async def sentry_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Sentry's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from Sentry{tail}",
                reference=f"sentry:sample_logs:{query or 'all'}",
            ),
        ),
    )
