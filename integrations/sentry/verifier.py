"""Proves a Sentry credential works, and proves what it is allowed to do.
...
#: What each failure means for Sentry, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Sentry rejected the credential. Re-issue it: Settings → Developer Settings → Auth Tokens, or an organisation token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Sentry credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Settings → Developer Settings → Auth Tokens, or an organisation token"
...
_NO_INTROSPECTION: Final = (
    "Sentry has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="project:read",
    grants="list projects and their issues",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="event:read",
    grants="read the events behind an issue",
    capabilities=("sentry_sample_logs",),
    where=_GRANTED_AT,
)
...
class SentryVerifier:
    """Checks a stored Sentry credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/0/organizations/{organisation}/ — the cheapest read Sentry offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Sentry accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Sentry accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Sentry said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> SentryClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a sentry probe needs a proxy transport and a request context")
        return SentryClient(transport=transport, context=context, region=self.region)
