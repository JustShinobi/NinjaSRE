"""Sentry's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "sentry"


#: Every host Sentry serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap(
    integration=INTEGRATION,
    regions=(
        Region(name="us", host="sentry.io", display_name="United States"),
        Region(name="de", host="de.sentry.io", display_name="Germany"),
        Region(name="self-hosted", host="sentry.example.com", display_name="Self-hosted"),
    ),
    default="us",
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Sentry auth token with project:read and event:read", min_length=8),
    public("organisation", "Sentry organisation slug", required=True),
    public("project", "Default project slug to search"),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="organisation", field="organisation"),
    ),
)
