"""Reading a capped number of Honeycomb's traces, after something narrowed the question.
...
Source of truth: Honeycomb's ``/1/events/{dataset}``.
"""
...
TOOL_NAME = "honeycomb_slow_traces"
...
@tool(
    name=TOOL_NAME,
    display_name="Honeycomb slow traces",
    description=(
...
async def honeycomb_slow_traces(
    service: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Honeycomb's traces, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.TRACE,
                summary=f"{len(entries)} traces from Honeycomb{tail}",
                reference=f"honeycomb:slow_traces:{service or 'all'}",
            ),
        ),
    )
