"""Counting Honeycomb's traces before reading any of them.

The first call of an investigation that reaches Honeycomb, and the one
...
Source of truth: Honeycomb's ``/1/events/{dataset}``, read through the
...
TOOL_NAME = "honeycomb_trace_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Honeycomb trace statistics",
    description=(
...
async def honeycomb_trace_statistics(
    service: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Honeycomb's traces across ``group_by``.

    ``start`` and ``end`` are Honeycomb's own time expressions. Both are
    ...
    client = access.client(HoneycombClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"honeycomb:trace_statistics:{service or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no traces were returned by Honeycomb for this request"
