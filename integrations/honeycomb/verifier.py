"""Proves a Honeycomb credential works, and proves what it is allowed to do.
...
from integrations.honeycomb.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Honeycomb, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Honeycomb rejected the credential. Re-issue it: Environment settings → API keys, on the environment holding the dataset"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Honeycomb credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Environment settings → API keys, on the environment holding the dataset"
...
_NO_INTROSPECTION: Final = (
    "Honeycomb has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="query",
    grants="run queries against a dataset's events",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="auth:read",
    grants="read what the key is allowed to do, which the probe uses",
    capabilities=("honeycomb_trace_statistics",),
    where=_GRANTED_AT,
)
...
class HoneycombVerifier:
    """Checks a stored Honeycomb credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /1/auth/{dataset} — the cheapest read Honeycomb offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Honeycomb accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Honeycomb accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Honeycomb said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> HoneycombClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a honeycomb probe needs a proxy transport and a request context")
        return HoneycombClient(transport=transport, context=context, region=self.region)
