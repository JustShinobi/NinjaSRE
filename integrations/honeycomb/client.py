"""Honeycomb's API, over the proxy, holding no credential.
...
Honeycomb's authentication changed tomorrow, because the authentication is
...
from integrations.honeycomb.schema import INTEGRATION, base_url

PING_PATH: Final = "/1/auth/{dataset}"
SEARCH_TRACES_PATH: Final = "/1/events/{dataset}"
SLOW_TRACES_PATH: Final = "/1/events/{dataset}"
...
class HoneycombClient(IntegrationClient):
    """Honeycomb, reached through the credential proxy."""
    ...
    async def search_traces(
        ...
        *,
        ...
        """Return what Honeycomb holds for this request, bounded.
        ...
        """
        # Honeycomb narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"service": service}
    ...
    async def slow_traces(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"service": service, "sort": "duration_ms"}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Honeycomb offers.
