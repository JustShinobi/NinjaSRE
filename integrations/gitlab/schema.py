"""GitLab's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "gitlab"


#: Every host GitLab serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap(
    integration=INTEGRATION,
    regions=(
        Region(name="gitlab.com", host="gitlab.com", display_name="gitlab.com"),
        Region(name="self-managed", host="gitlab.example.com", display_name="Self-managed"),
    ),
    default="gitlab.com",
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret(
        "api_key", "GitLab personal, project, or group access token with read_api", min_length=8
    ),
    public("project_id", "Default project id or URL-encoded path"),
)

RULE: Final = InjectionRule(
    integration=INTEGRATION,
    hosts=HOSTS,
    injections=(HeaderInjection(header="PRIVATE-TOKEN", field="api_key", template="{value}"),),
)
