"""GitLab's API, over the proxy, holding no credential.
...
GitLab's authentication changed tomorrow, because the authentication is
...
from integrations.gitlab.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/v4/user"
LIST_COMMITS_PATH: Final = "/api/v4/projects"
LIST_PULL_REQUESTS_PATH: Final = "/api/v4/merge_requests"
...
class GitlabClient(IntegrationClient):
    """GitLab, reached through the credential proxy."""
    ...
    async def list_commits(
        ...
        *,
        ...
        """Return what GitLab holds for this request, bounded.
        ...
        """
        # GitLab narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"search": repository, "order_by": "last_activity_at"}
    ...
    async def list_pull_requests(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"scope": "all", "state": "merged", "updated_after": start}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call GitLab offers.
