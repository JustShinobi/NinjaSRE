"""RabbitMQ's API, over the proxy, holding no credential.
...
RabbitMQ's authentication changed tomorrow, because the authentication is
...
from integrations.rabbitmq.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/overview"
LIST_TASKS_PATH: Final = "/api/queues"
LIST_FAILURES_PATH: Final = "/api/queues"
...
class RabbitmqClient(IntegrationClient):
    """RabbitMQ, reached through the credential proxy."""
    ...
    async def list_tasks(
        ...
        *,
        ...
        """Return what RabbitMQ holds for this request, bounded.
        ...
        """
        # RabbitMQ narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"columns": "name,vhost,state,messages,consumers"}
    ...
    async def list_failures(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {
                "columns": "name,vhost,state,messages,consumers",
                "sort": "messages",
                "sort_reverse": "true",
            }
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call RabbitMQ offers.
