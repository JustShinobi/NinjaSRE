"""RabbitMQ's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "rabbitmq"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "rabbitmq.example.com"

#: Every host RabbitMQ serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="rabbitmq.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "RabbitMQ management user with the monitoring tag", min_length=8),
    secret("password", "That user's password", min_length=8),
    public("vhost", "Virtual host to read, defaulting to /"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    RabbitMQ is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="rabbitmq.acme.example")``.
    """
