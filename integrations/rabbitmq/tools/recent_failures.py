"""Reading a capped number of RabbitMQ's failures, after something narrowed the question.
...
Source of truth: RabbitMQ's ``/api/queues``.
"""
...
TOOL_NAME = "rabbitmq_recent_failures"
...
@tool(
    name=TOOL_NAME,
    display_name="RabbitMQ recent failures",
    description=(
...
async def rabbitmq_recent_failures(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of RabbitMQ's failures, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} failures from RabbitMQ{tail}",
                reference=f"rabbitmq:recent_failures:{scope or 'all'}",
            ),
        ),
    )
