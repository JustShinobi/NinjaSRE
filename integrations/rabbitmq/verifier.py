"""Proves a RabbitMQ credential works, and proves what it is allowed to do.
...
#: What each failure means for RabbitMQ, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "RabbitMQ rejected the credential. Re-issue it: RabbitMQ management UI → Admin → Users, with the `monitoring` tag"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No RabbitMQ credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "RabbitMQ management UI → Admin → Users, with the `monitoring` tag"
...
_NO_INTROSPECTION: Final = (
    "RabbitMQ has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="monitoring",
    grants="read queues, overview, and node statistics",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="management",
    grants="reach the management plugin at all",
    capabilities=(
        "rabbitmq_pipeline_health",
        "rabbitmq_recent_failures",
    ),
...
class RabbitmqVerifier:
    """Checks a stored RabbitMQ credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/overview — the cheapest read RabbitMQ offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether RabbitMQ accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="RabbitMQ accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what RabbitMQ said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> RabbitmqClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a rabbitmq probe needs a proxy transport and a request context")
        return RabbitmqClient(transport=transport, context=context, region=self.region)
