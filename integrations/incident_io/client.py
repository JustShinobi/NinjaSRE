"""incident.io's API, over the proxy, holding no credential.
...
incident.io's authentication changed tomorrow, because the authentication is
...
from integrations.incident_io.schema import INTEGRATION, base_url

PING_PATH: Final = "/v2/incidents"
LIST_INCIDENTS_PATH: Final = "/v2/incidents"
INCIDENT_TIMELINE_PATH: Final = "/v2/incidents"
ACKNOWLEDGE_PATH: Final = "/v2/incident_updates"
...
class IncidentIoClient(IntegrationClient):
    """incident.io, reached through the credential proxy."""
    ...
    async def list_incidents(
        ...
        *,
        ...
        """Return what incident.io holds for this request, bounded.
        ...
        """
        # incident.io narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"status": status}
            asked.update(parameters)
            answer = (await self.get(LIST_INCIDENTS_PATH, params=asked)).json()
            return Page(
                items=records(answer, "incidents"),
                cursor=text(answer, "pagination_meta", "after") or None,
            )
    ...
    async def incident_timeline(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"status": "live"}
            asked.update(parameters)
    ...
    async def acknowledge(self, incident: str, note: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what incident.io said."""
        body: dict[str, Any] = {"incident_id": incident, "message": note}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call incident.io offers.
        ...
        """
        return await self.get(PING_PATH, params={"page_size": "1"})
