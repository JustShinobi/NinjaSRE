"""incident.io: incident.io's record of what is happening: the open incidents, one incident's timeline, and the acknowledgement that says somebody is on it.
...
from integrations.incident_io.client import PAGINATION, IncidentIoClient
from integrations.incident_io.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.incident_io.verifier import PERMISSIONS, IncidentIoVerifier
...
DESCRIPTOR: Final = IntegrationDescriptor(
    ...
    schema=SCHEMA,
    rule=RULE,
    verifier=IncidentIoVerifier(),
    client_class=IncidentIoClient,
