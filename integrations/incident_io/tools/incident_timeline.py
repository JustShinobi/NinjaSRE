"""Reading a capped number of incident.io's timeline entries, after something narrowed the question.
...
Source of truth: incident.io's ``/v2/incidents``.
"""
...
TOOL_NAME = "incident_io_incident_timeline"
...
@tool(
    name=TOOL_NAME,
    display_name="incident.io incident timeline",
    description=(
...
async def incident_io_incident_timeline(
    incident: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of incident.io's timeline entries, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"{len(entries)} timeline entries from incident.io{tail}",
                reference=f"incident_io:incident_timeline:{incident or 'all'}",
            ),
        ),
    )
