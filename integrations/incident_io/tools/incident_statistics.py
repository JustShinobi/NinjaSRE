"""Counting incident.io's incidents before reading any of them.

The first call of an investigation that reaches incident.io, and the one
...
Source of truth: incident.io's ``/v2/incidents``, read through the
...
TOOL_NAME = "incident_io_incident_statistics"
...
DEFAULT_GROUP_BY = "incident_status.category"
...
@tool(
    name=TOOL_NAME,
    display_name="incident.io incident statistics",
    description=(
...
async def incident_io_incident_statistics(
    status: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of incident.io's incidents across ``group_by``.

    ``start`` and ``end`` are incident.io's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"incident_io:incident_statistics:{status or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no incidents were returned by incident.io for this request"
