"""Splunk's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "splunk"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "splunk.example.com"

#: Every host Splunk serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="splunk.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Splunk authentication token, from a user with search access", min_length=8),
    public("app", "Splunk app context searches run in"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Splunk is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="splunk.acme.example")``.
    """
