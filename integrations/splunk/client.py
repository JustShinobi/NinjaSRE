"""Splunk's API, over the proxy, holding no credential.
...
Splunk's authentication changed tomorrow, because the authentication is
...
from integrations.splunk.schema import INTEGRATION, base_url

PING_PATH: Final = "/services/server/info"
SEARCH_LOGS_PATH: Final = "/services/search/jobs/export"
RECENT_LOGS_PATH: Final = "/services/search/jobs/export"
...
class SplunkClient(IntegrationClient):
    """Splunk, reached through the credential proxy."""
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what Splunk holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                "search": query or "search index=* | head 100",
                "output_mode": "json",
                "earliest_time": start,
                "latest_time": end,
            }
    ...
    async def recent_logs(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                "search": query or "search index=* | head 20",
                "output_mode": "json",
                "earliest_time": start,
                "latest_time": end,
            }
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Splunk offers.
        ...
        """
        return await self.get(PING_PATH, params={"output_mode": "json"})
