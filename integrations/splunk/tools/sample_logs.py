"""Reading a capped number of Splunk's log lines, after something narrowed the question.
...
Source of truth: Splunk's ``/services/search/jobs/export``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Splunk log samples",
    description=(
...
async def splunk_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Splunk's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from Splunk{tail}",
                reference=f"splunk:sample_logs:{query or 'all'}",
            ),
        ),
    )
