"""Counting Splunk's log lines before reading any of them.

The first call of an investigation that reaches Splunk, and the one
...
Source of truth: Splunk's ``/services/search/jobs/export``, read through the
...
TOOL_NAME = "splunk_log_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Splunk log statistics",
    description=(
...
async def splunk_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Splunk's log lines across ``group_by``.

    ``start`` and ``end`` are Splunk's own time expressions. Both are
    ...
    client = access.client(SplunkClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"splunk:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by Splunk for this request"
