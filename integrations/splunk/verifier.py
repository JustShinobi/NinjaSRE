"""Proves a Splunk credential works, and proves what it is allowed to do.
...
from integrations.splunk.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Splunk, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Splunk rejected the credential. Re-issue it: Settings → Tokens, or Splunk Cloud → Authentication tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Splunk credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Settings → Tokens, or Splunk Cloud → Authentication tokens"
...
_NO_INTROSPECTION: Final = (
    "Splunk has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="search",
    grants="run SPL searches in the app context",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read server info, which the probe uses",
    capabilities=("splunk_log_statistics",),
    where=_GRANTED_AT,
)
...
class SplunkVerifier:
    """Checks a stored Splunk credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /services/server/info — the cheapest read Splunk offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Splunk accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Splunk accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Splunk said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> SplunkClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a splunk probe needs a proxy transport and a request context")
        return SplunkClient(transport=transport, context=context, region=self.region)
