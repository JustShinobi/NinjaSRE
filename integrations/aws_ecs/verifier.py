"""Proves a AWS ECS credential works, and proves what it is allowed to do.
...
#: What each failure means for AWS ECS, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS ECS rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS ECS credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "AWS ECS has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_1: Final = RequiredPermission(
    ...
    capabilities=("aws_ecs_resource_inventory",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="ecs:ListTaskDefinitions",
    grants="list registered task definitions, newest first",
    capabilities=("aws_ecs_recent_changes",),
    where=_GRANTED_AT,
)
...
class AwsEcsVerifier:
    """Checks a stored AWS ECS credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls / — the cheapest read AWS ECS offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS ECS accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS ECS accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS ECS said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsEcsClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_ecs probe needs a proxy transport and a request context")
        return AwsEcsClient(transport=transport, context=context, region=self.region)
