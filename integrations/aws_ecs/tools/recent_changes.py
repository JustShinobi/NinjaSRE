"""Reading a capped number of AWS ECS's changes, after something narrowed the question.
...
TOOL_NAME = "aws_ecs_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS ECS recent changes",
    description=(
...
async def aws_ecs_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of AWS ECS's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from AWS ECS{tail}",
                reference=f"aws_ecs:recent_changes:{kind or 'all'}",
            ),
        ),
    )
