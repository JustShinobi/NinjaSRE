"""AWS ECS's API, over the proxy, holding no credential.
...
AWS ECS's authentication changed tomorrow, because the authentication is
...
from integrations.aws_ecs.schema import INTEGRATION, base_url
...
class AwsEcsClient(IntegrationClient):
    """AWS ECS, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what AWS ECS holds for this request, bounded.
        ...
        """
        # AWS ECS narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(
                items=named(answer, "clusterArns", key="cluster"),
    ...
    async def list_changes(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {"maxResults": 100, "sort": "DESC", "status": "ACTIVE"}
            body.update(parameters)
            answer = (
                await self.post(
                    LIST_CHANGES_PATH,
            ...
            return Page(
                items=named(answer, "taskDefinitionArns", key="task_definition"),
                cursor=text(answer, "nextToken") or None,
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call AWS ECS offers.
