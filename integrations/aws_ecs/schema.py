"""AWS ECS's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "aws_ecs"
DEFAULT_REGION: Final = "us-east-1"

#: Every host AWS ECS serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.from_template(
    INTEGRATION,
    template="ecs.{region}.amazonaws.com",
    names=(
        "us-east-1",
        "us-east-2",
        "us-west-1",
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        SignatureInjection(signer=SigV4Signer(service="ecs", default_region=DEFAULT_REGION)),
    ),
)
