"""AWS ELB's API, over the proxy, holding no credential.
...
AWS ELB's authentication changed tomorrow, because the authentication is
...
from integrations.aws_elb.schema import INTEGRATION, base_url
...
class AwsElbClient(IntegrationClient):
    """AWS ELB, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what AWS ELB holds for this request, bounded.
        ...
        """
        # AWS ELB narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"Action": "DescribeLoadBalancers", "Version": "2015-12-01"}
            asked.update(parameters)
            answer = (await self.get(LIST_RESOURCES_PATH, params=asked)).text
            return Page(
                items=xml_records(answer, "member"), cursor=xml_text(answer, "NextMarker") or None
            )
    ...
    async def list_changes(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"Action": "DescribeTargetGroups", "Version": "2015-12-01"}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call AWS ELB offers.
        ...
        return await self.get(
            PING_PATH,
            params={"Action": "DescribeLoadBalancers", "Version": "2015-12-01", "PageSize": "1"},
        )
