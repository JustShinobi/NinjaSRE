"""Proves a AWS ELB credential works, and proves what it is allowed to do.
...
from integrations.aws_elb.client import AwsElbClient
from integrations.aws_elb.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for AWS ELB, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS ELB rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS ELB credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "AWS ELB has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_1: Final = RequiredPermission(
    ...
    grants="read load balancer state and scheme",
    capabilities=("aws_elb_resource_inventory",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="elasticloadbalancing:DescribeTargetGroups",
    grants="read the target groups behind each load balancer",
    capabilities=("aws_elb_recent_changes",),
    where=_GRANTED_AT,
)
...
class AwsElbVerifier:
    """Checks a stored AWS ELB credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls / — the cheapest read AWS ELB offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS ELB accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS ELB accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS ELB said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsElbClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_elb probe needs a proxy transport and a request context")
        return AwsElbClient(transport=transport, context=context, region=self.region)
