"""Counting AWS ELB's resources before reading any of them.

The first call of an investigation that reaches AWS ELB, and the one
...
Source of truth: AWS ELB's ``/``, read through the
...
TOOL_NAME = "aws_elb_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS ELB resource inventory",
    description=(
...
async def aws_elb_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of AWS ELB's resources across ``group_by``.

    ``start`` and ``end`` are AWS ELB's own time expressions. Both are
    ...
    client = access.client(AwsElbClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"aws_elb:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by AWS ELB for this request"
