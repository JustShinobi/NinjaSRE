"""AWS ELB's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "aws_elb"
DEFAULT_REGION: Final = "us-east-1"

#: Every host AWS ELB serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.from_template(
    INTEGRATION,
    template="elasticloadbalancing.{region}.amazonaws.com",
    names=(
        "us-east-1",
        "us-east-2",
        "us-west-1",
...
RULE: Final = InjectionRule(
    ...
    injections=(
        SignatureInjection(
            signer=SigV4Signer(service="elasticloadbalancing", default_region=DEFAULT_REGION)
        ),
    ),
)
