"""Pushover: the one change this integration is allowed to make.
...
Source of truth: Pushover's ``/1/messages.json``.
"""
...
TOOL_NAME = "pushover_post_message"
...
class PushoverPostMessageRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the Pushover change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what Pushover returned for {target}, including its identifier.",
                "Undo the change in Pushover using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="Pushover post message",
    description=(
    ...
    rollback_planner=PushoverPostMessageRollback(),
    tags=(
        "communication",
        "pushover",
        "write",
...
async def pushover_post_message(channel: str, text: str) -> CapabilityResult:
    """Make the change in Pushover and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(PushoverClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"Pushover accepted the change to {channel!r}",
                reference=f"pushover:post_message:{channel}",
            ),
        ),
    )
