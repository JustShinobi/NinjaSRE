"""Reading a capped number of Pushover's messages, after something narrowed the question.
...
Source of truth: Pushover's ``/1/licenses.json``.
"""
...
TOOL_NAME = "pushover_recent_messages"
...
@tool(
    name=TOOL_NAME,
    display_name="Pushover recent messages",
    description=(
...
async def pushover_recent_messages(
    channel: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Pushover's messages, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} messages from Pushover{tail}",
                reference=f"pushover:recent_messages:{channel or 'all'}",
            ),
        ),
    )
