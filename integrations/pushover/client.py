"""Pushover's API, over the proxy, holding no credential.
...
Pushover's authentication changed tomorrow, because the authentication is
...
from integrations.pushover.schema import INTEGRATION, base_url

PING_PATH: Final = "/1/users/validate.json"
RECENT_MESSAGES_PATH: Final = "/1/licenses.json"
POST_MESSAGE_PATH: Final = "/1/messages.json"
...
class PushoverClient(IntegrationClient):
    """Pushover, reached through the credential proxy."""
    ...
    async def recent_messages(
        ...
        *,
        ...
        """Return what Pushover holds for this request, bounded.
        ...
        """
        # Pushover narrows this endpoint by other means; the argument stays in the
    ...
    async def post_message(self, channel: str, text: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Pushover said."""
        body: dict[str, Any] = {"message": text, "title": channel}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Pushover offers.
