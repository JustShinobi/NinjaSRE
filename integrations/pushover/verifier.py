"""Proves a Pushover credential works, and proves what it is allowed to do.
...
from integrations.pushover.client import PushoverClient
from integrations.pushover.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Pushover, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Pushover rejected the credential. Re-issue it: pushover.net → Your Applications → API Token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Pushover credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "pushover.net → Your Applications → API Token"
...
_NO_INTROSPECTION: Final = (
    "Pushover has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="validate",
    grants="confirm the token and user key are valid",
    capabilities=("pushover_recent_messages",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="messages:write",
    grants="push a message to the user or group",
    capabilities=("pushover_post_message",),
    where=_GRANTED_AT,
)
...
class PushoverVerifier:
    """Checks a stored Pushover credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /1/users/validate.json — the cheapest read Pushover offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Pushover accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Pushover accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Pushover said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> PushoverClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a pushover probe needs a proxy transport and a request context")
        return PushoverClient(transport=transport, context=context, region=self.region)
