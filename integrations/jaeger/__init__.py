"""Jaeger: Jaeger's trace store: where a service's operations concentrate latency, and the slowest traces behind that concentration.
...
from integrations.jaeger.client import PAGINATION, JaegerClient
from integrations.jaeger.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.jaeger.verifier import PERMISSIONS, JaegerVerifier
...
DESCRIPTOR: Final = IntegrationDescriptor(
    ...
    schema=SCHEMA,
    rule=RULE,
    verifier=JaegerVerifier(),
    client_class=JaegerClient,
