"""Jaeger's API, over the proxy, holding no credential.
...
Jaeger's authentication changed tomorrow, because the authentication is
...
from integrations.jaeger.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/services"
SEARCH_TRACES_PATH: Final = "/api/traces"
SLOW_TRACES_PATH: Final = "/api/traces"
...
class JaegerClient(IntegrationClient):
    """Jaeger, reached through the credential proxy."""
    ...
    async def search_traces(
        ...
        *,
        ...
        """Return what Jaeger holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"service": service, "start": start, "end": end}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Jaeger offers.
