"""Counting Jaeger's traces before reading any of them.

The first call of an investigation that reaches Jaeger, and the one
...
Source of truth: Jaeger's ``/api/traces``, read through the
...
TOOL_NAME = "jaeger_trace_statistics"
...
DEFAULT_GROUP_BY = "spans.operationName"
...
@tool(
    name=TOOL_NAME,
    display_name="Jaeger trace statistics",
    description=(
...
async def jaeger_trace_statistics(
    service: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Jaeger's traces across ``group_by``.

    ``start`` and ``end`` are Jaeger's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"jaeger:trace_statistics:{service or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no traces were returned by Jaeger for this request"
