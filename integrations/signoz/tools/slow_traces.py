"""Reading a capped number of SigNoz's traces, after something narrowed the question.
...
Source of truth: SigNoz's ``/api/v3/query_range``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="SigNoz slow traces",
    description=(
...
async def signoz_slow_traces(
    service: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of SigNoz's traces, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.TRACE,
                summary=f"{len(entries)} traces from SigNoz{tail}",
                reference=f"signoz:slow_traces:{service or 'all'}",
            ),
        ),
    )
