"""SigNoz's API, over the proxy, holding no credential.
...
SigNoz's authentication changed tomorrow, because the authentication is
...
from integrations.signoz.schema import base_url as _region_url

PING_PATH: Final = "/api/v1/version"
SEARCH_TRACES_PATH: Final = "/api/v3/query_range"
SLOW_TRACES_PATH: Final = "/api/v3/query_range"
...
class SignozClient(IntegrationClient):
    """SigNoz, reached through the credential proxy."""
    ...
    def __init__(
        ...
        *,
        ...
        # SigNoz is. The egress allow-list still decides whether the host may
        # be reached.
        super().__init__(
            transport=transport,
            context=context,
    ...
    async def search_traces(
        ...
        *,
        ...
        """Return what SigNoz holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            answer = (await self.post(SEARCH_TRACES_PATH, json_body=body)).json()
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            body: dict[str, Any] = {
                ...
                "dataSource": "traces",
                "serviceName": service,
                "orderBy": "durationNano desc",
            }
            body.update(parameters)
            answer = (await self.post(SLOW_TRACES_PATH, json_body=body)).json()
            return Page(items=records(answer, "data", "result"), cursor="" or None)

        return await walk(_pagination("slow_traces"), fetch, max_pages=max_pages, max_items=limit)

    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call SigNoz offers.
