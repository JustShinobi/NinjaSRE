"""VictoriaLogs's API, over the proxy, holding no credential.
...
VictoriaLogs's authentication changed tomorrow, because the authentication is
...
from integrations.victorialogs.schema import INTEGRATION, base_url

PING_PATH: Final = "/select/logsql/hits"
SEARCH_LOGS_PATH: Final = "/select/logsql/query"
RECENT_LOGS_PATH: Final = "/select/logsql/query"
...
class VictorialogsClient(IntegrationClient):
    """VictoriaLogs, reached through the credential proxy."""
    ...
    async def search_logs(
        ...
        *,
        ...
        """Return what VictoriaLogs holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"query": query or "*", "start": start, "end": end}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call VictoriaLogs offers.
        ...
        """
        return await self.get(PING_PATH, params={"query": "*", "step": "1h"})
