"""Counting VictoriaLogs's log lines before reading any of them.

The first call of an investigation that reaches VictoriaLogs, and the one
...
Source of truth: VictoriaLogs's ``/select/logsql/query``, read through the
...
from integrations.victorialogs.client import VictorialogsClient
from integrations.victorialogs.schema import INTEGRATION

TOOL_NAME = "victorialogs_log_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="VictoriaLogs log statistics",
    description=(
...
async def victorialogs_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of VictoriaLogs's log lines across ``group_by``.

    ``start`` and ``end`` are VictoriaLogs's own time expressions. Both are
    ...
    client = access.client(VictorialogsClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"victorialogs:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by VictoriaLogs for this request"
