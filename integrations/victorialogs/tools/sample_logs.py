"""Reading a capped number of VictoriaLogs's log lines, after something narrowed the question.
...
Source of truth: VictoriaLogs's ``/select/logsql/query``.
"""
...
TOOL_NAME = "victorialogs_sample_logs"
...
@tool(
    name=TOOL_NAME,
    display_name="VictoriaLogs log samples",
    description=(
...
async def victorialogs_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of VictoriaLogs's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from VictoriaLogs{tail}",
                reference=f"victorialogs:sample_logs:{query or 'all'}",
            ),
        ),
    )
