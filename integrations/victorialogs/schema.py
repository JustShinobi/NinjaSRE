"""VictoriaLogs's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "victorialogs"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "victorialogs.example.com"

#: Every host VictoriaLogs serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="victorialogs.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Bearer token accepted by whatever fronts VictoriaLogs", min_length=8),
    public("tenant", "AccountID:ProjectID for a multi-tenant install"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    VictoriaLogs is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="victorialogs.acme.example")``.
    """
