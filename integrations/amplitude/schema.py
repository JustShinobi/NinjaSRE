"""Amplitude's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "amplitude"


#: Every host Amplitude serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap(
    integration=INTEGRATION,
    regions=(
        Region(name="us", host="amplitude.com", display_name="United States"),
        Region(name="eu", host="analytics.eu.amplitude.com", display_name="Europe"),
    ),
    default="us",
)
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "Amplitude API key", min_length=8),
    secret("password", "Amplitude secret key", min_length=8),
)
