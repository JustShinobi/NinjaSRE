"""Counting Amplitude's series before reading any of them.

The first call of an investigation that reaches Amplitude, and the one
...
Source of truth: Amplitude's ``/api/2/events/segmentation``, read through the
...
TOOL_NAME = "amplitude_metric_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Amplitude metric statistics",
    description=(
...
async def amplitude_metric_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Amplitude's series across ``group_by``.

    ``start`` and ``end`` are Amplitude's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"amplitude:metric_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no series were returned by Amplitude for this request"
