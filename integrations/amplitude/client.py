"""Amplitude's API, over the proxy, holding no credential.
...
Amplitude's authentication changed tomorrow, because the authentication is
...
from integrations._base.payload import named, records
from integrations._base.retry import RetryPolicy
from integrations._base.transport import ProxyTransport, RequestContext
from integrations.amplitude.schema import INTEGRATION, base_url

PING_PATH: Final = "/api/2/annotations"
QUERY_METRIC_PATH: Final = "/api/2/events/segmentation"
LIST_ALERTS_PATH: Final = "/api/2/annotations"
...
class AmplitudeClient(IntegrationClient):
    """Amplitude, reached through the credential proxy."""
    ...
    async def query_metric(
        ...
        *,
        ...
        """Return what Amplitude holds for this request, bounded.
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"e": query, "start": start, "end": end}
            asked.update(parameters)
            answer = (await self.get(QUERY_METRIC_PATH, params=asked)).json()
            return Page(
                items=named(answer, "data", "seriesLabels", key="series"), cursor="" or None
            )
    ...
    async def list_alerts(
        ...
        *,
        ...
        "there is no more" when what happened was "we stopped looking".
        """
        # Amplitude narrows this endpoint by other means; the argument stays in the
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Amplitude offers.
