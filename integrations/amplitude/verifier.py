"""Proves a Amplitude credential works, and proves what it is allowed to do.
...
#: What each failure means for Amplitude, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Amplitude rejected the credential. Re-issue it: Amplitude → Settings → Projects → your project → General"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Amplitude credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Amplitude → Settings → Projects → your project → General"
...
_NO_INTROSPECTION: Final = (
    "Amplitude has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="events:read",
    grants="run event segmentation queries",
    capabilities=("amplitude_metric_statistics",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="annotations:read",
    grants="read chart annotations",
    capabilities=("amplitude_active_alerts",),
    where=_GRANTED_AT,
)
...
class AmplitudeVerifier:
    """Checks a stored Amplitude credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/2/annotations — the cheapest read Amplitude offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Amplitude accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Amplitude accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Amplitude said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AmplitudeClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a amplitude probe needs a proxy transport and a request context")
        return AmplitudeClient(transport=transport, context=context, region=self.region)
