"""Proves a Confluence credential works, and proves what it is allowed to do.
...
#: What each failure means for Confluence, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Confluence rejected the credential. Re-issue it: id.atlassian.com → Security → API tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Confluence credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "id.atlassian.com → Security → API tokens"
...
_NO_INTROSPECTION: Final = (
    "Confluence has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="list spaces, which the probe uses",
    capabilities=("confluence_issue_statistics",),
    where=_GRANTED_AT,
)
...
class ConfluenceVerifier:
    """Checks a stored Confluence credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /wiki/rest/api/space — the cheapest read Confluence offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Confluence accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Confluence accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Confluence said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> ConfluenceClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a confluence probe needs a proxy transport and a request context")
        return ConfluenceClient(transport=transport, context=context, region=self.region)
