"""Counting Confluence's issues before reading any of them.

The first call of an investigation that reaches Confluence, and the one
...
Source of truth: Confluence's ``/wiki/rest/api/content/search``, read through the
...
TOOL_NAME = "confluence_issue_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Confluence issue statistics",
    description=(
...
async def confluence_issue_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Confluence's issues across ``group_by``.

    ``start`` and ``end`` are Confluence's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"confluence:issue_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no issues were returned by Confluence for this request"
