"""Confluence's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "confluence"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "acme.atlassian.net"

#: Every host Confluence serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="acme.atlassian.net", name="site")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "Atlassian account email", min_length=8),
    secret("password", "Atlassian API token, not the account password", min_length=8),
    public("space", "Default space key searches are scoped to"),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Confluence is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="confluence.acme.example")``.
    """
    named = {"site": DEFAULT_HOST, **endpoints}
    return RegionMap(
        integration=INTEGRATION,
