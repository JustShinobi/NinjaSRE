"""BigQuery's API, over the proxy, holding no credential.
...
BigQuery's authentication changed tomorrow, because the authentication is
...
from integrations.bigquery.schema import INTEGRATION, base_url

PING_PATH: Final = "/bigquery/v2/projects/{project}/jobs"
LIST_SESSIONS_PATH: Final = "/bigquery/v2/projects/{project}/jobs"
SLOW_QUERIES_PATH: Final = "/bigquery/v2/projects/{project}/jobs"
...
class BigqueryClient(IntegrationClient):
    """BigQuery, reached through the credential proxy."""
    ...
    async def list_sessions(
        ...
        *,
        ...
        """Return what BigQuery holds for this request, bounded.
        ...
        """
        # BigQuery narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(items=records(answer, "jobs"), cursor=text(answer, "nextPageToken") or None)
    ...
    async def slow_queries(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"allUsers": "true", "projection": "full"}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call BigQuery offers.
        ...
        """
        return await self.get(PING_PATH, params={"maxResults": "1", "allUsers": "true"})
