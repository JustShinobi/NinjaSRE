"""Reading a capped number of BigQuery's statements, after something narrowed the question.
...
Source of truth: BigQuery's ``/bigquery/v2/projects/{project}/jobs``.
"""
...
TOOL_NAME = "bigquery_slow_queries"
...
@tool(
    name=TOOL_NAME,
    display_name="BigQuery slow queries",
    description=(
...
async def bigquery_slow_queries(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of BigQuery's statements, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.ANALYSIS,
                summary=f"{len(entries)} statements from BigQuery{tail}",
                reference=f"bigquery:slow_queries:{scope or 'all'}",
            ),
        ),
    )
