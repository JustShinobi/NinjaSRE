"""Counting BigQuery's sessions before reading any of them.

The first call of an investigation that reaches BigQuery, and the one
...
Source of truth: BigQuery's ``/bigquery/v2/projects/{project}/jobs``, read through the
...
TOOL_NAME = "bigquery_session_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="BigQuery session statistics",
    description=(
...
async def bigquery_session_statistics(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of BigQuery's sessions across ``group_by``.

    ``start`` and ``end`` are BigQuery's own time expressions. Both are
    ...
    client = access.client(BigqueryClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"bigquery:session_statistics:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no sessions were returned by BigQuery for this request"
