"""Grafana's API, over the proxy, holding no credential.
...
Grafana's authentication changed tomorrow, because the authentication is
...
from integrations.grafana.schema import base_url as _region_url

PING_PATH: Final = "/api/health"
LIST_RESOURCES_PATH: Final = "/api/search"
LIST_CHANGES_PATH: Final = "/api/annotations"
...
class GrafanaClient(IntegrationClient):
    """Grafana, reached through the credential proxy."""
    ...
    def __init__(
        ...
        *,
        ...
        # Grafana is. The egress allow-list still decides whether the host may
        # be reached.
        super().__init__(
            transport=transport,
            context=context,
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what Grafana holds for this request, bounded.
        ...
        """
        # Grafana narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"query": kind, "type": "dash-db"}
    ...
    async def list_changes(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"from": start, "to": end, "type": kind or "annotation"}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Grafana offers.
