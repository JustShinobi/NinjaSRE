"""Counting Grafana's resources before reading any of them.

The first call of an investigation that reaches Grafana, and the one
...
Source of truth: Grafana's ``/api/search``, read through the
...
TOOL_NAME = "grafana_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="Grafana resource inventory",
    description=(
...
async def grafana_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Grafana's resources across ``group_by``.

    ``start`` and ``end`` are Grafana's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"grafana:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by Grafana for this request"
