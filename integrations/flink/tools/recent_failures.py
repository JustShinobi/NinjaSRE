"""Reading a capped number of Flink's failures, after something narrowed the question.
...
Source of truth: Flink's ``/jobs/overview``.
"""
...
TOOL_NAME = "flink_recent_failures"
...
@tool(
    name=TOOL_NAME,
    display_name="Flink recent failures",
    description=(
...
async def flink_recent_failures(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Flink's failures, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} failures from Flink{tail}",
                reference=f"flink:recent_failures:{scope or 'all'}",
            ),
        ),
    )
