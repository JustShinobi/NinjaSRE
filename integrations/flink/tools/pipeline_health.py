"""Counting Flink's tasks before reading any of them.

The first call of an investigation that reaches Flink, and the one
...
Source of truth: Flink's ``/jobs/overview``, read through the
...
TOOL_NAME = "flink_pipeline_health"
...
@tool(
    name=TOOL_NAME,
    display_name="Flink pipeline health",
    description=(
...
async def flink_pipeline_health(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Flink's tasks across ``group_by``.

    ``start`` and ``end`` are Flink's own time expressions. Both are
    ...
    client = access.client(FlinkClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"flink:pipeline_health:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no tasks were returned by Flink for this request"
