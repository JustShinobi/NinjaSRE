"""Flink: Flink's JobManager REST API: which jobs are running, and the ones that failed or restarted, which is where a streaming backlog starts.
...
from integrations.flink.client import PAGINATION, FlinkClient
from integrations.flink.schema import HOSTS, INTEGRATION, REGIONS, RULE, SCHEMA, base_url
from integrations.flink.verifier import PERMISSIONS, FlinkVerifier
...
DESCRIPTOR: Final = IntegrationDescriptor(
    ...
    schema=SCHEMA,
    rule=RULE,
    verifier=FlinkVerifier(),
    client_class=FlinkClient,
