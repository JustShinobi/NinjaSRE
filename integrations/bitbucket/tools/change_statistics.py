"""Counting Bitbucket's commits before reading any of them.

The first call of an investigation that reaches Bitbucket, and the one
...
Source of truth: Bitbucket's ``/2.0/repositories/{workspace}``, read through the
...
TOOL_NAME = "bitbucket_change_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Bitbucket change statistics",
    description=(
...
async def bitbucket_change_statistics(
    repository: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Bitbucket's commits across ``group_by``.

    ``start`` and ``end`` are Bitbucket's own time expressions. Both are
    ...
    client = access.client(BitbucketClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"bitbucket:change_statistics:{repository or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no commits were returned by Bitbucket for this request"
