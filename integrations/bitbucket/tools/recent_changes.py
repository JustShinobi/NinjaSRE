"""Reading a capped number of Bitbucket's changes, after something narrowed the question.
...
Source of truth: Bitbucket's ``/2.0/repositories/{workspace}``.
"""
...
TOOL_NAME = "bitbucket_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="Bitbucket recent changes",
    description=(
...
async def bitbucket_recent_changes(
    repository: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Bitbucket's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from Bitbucket{tail}",
                reference=f"bitbucket:recent_changes:{repository or 'all'}",
            ),
        ),
    )
