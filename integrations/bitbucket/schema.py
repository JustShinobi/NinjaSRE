"""Bitbucket's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "bitbucket"


#: Every host Bitbucket serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.bitbucket.org", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "Bitbucket username, or the email on an Atlassian account", min_length=8),
    secret("password", "App password with repository and pull-request read access", min_length=8),
    public("workspace", "Bitbucket workspace slug", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    injections=(
        ...
        PathSegmentInjection(placeholder="workspace", field="workspace"),
    ),
)
