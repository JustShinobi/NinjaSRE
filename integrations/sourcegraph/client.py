"""Sourcegraph's API, over the proxy, holding no credential.
...
Sourcegraph's authentication changed tomorrow, because the authentication is
...
from integrations.sourcegraph.schema import INTEGRATION, base_url

PING_PATH: Final = "/.api/graphql"
LIST_COMMITS_PATH: Final = "/.api/search/stream"
LIST_PULL_REQUESTS_PATH: Final = "/.api/search/stream"
...
class SourcegraphClient(IntegrationClient):
    """Sourcegraph, reached through the credential proxy."""
    ...
    async def list_commits(
        ...
        *,
        ...
        """Return what Sourcegraph holds for this request, bounded.
        ...
        """
        # Sourcegraph narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"q": repository, "v": "V3", "t": "literal", "display": "100"}
    ...
    async def list_pull_requests(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"q": repository, "v": "V3", "t": "literal", "display": "20"}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Sourcegraph offers.
        ...
        """
        return await self.post(PING_PATH, json_body={"query": "{ currentUser { id } }"})
