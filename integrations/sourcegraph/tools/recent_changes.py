"""Reading a capped number of Sourcegraph's changes, after something narrowed the question.
...
Source of truth: Sourcegraph's ``/.api/search/stream``.
"""
...
TOOL_NAME = "sourcegraph_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="Sourcegraph recent changes",
    description=(
...
async def sourcegraph_recent_changes(
    repository: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Sourcegraph's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from Sourcegraph{tail}",
                reference=f"sourcegraph:recent_changes:{repository or 'all'}",
            ),
        ),
    )
