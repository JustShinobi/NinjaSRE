"""Counting Sourcegraph's commits before reading any of them.

The first call of an investigation that reaches Sourcegraph, and the one
...
Source of truth: Sourcegraph's ``/.api/search/stream``, read through the
...
from integrations.sourcegraph.client import SourcegraphClient
from integrations.sourcegraph.schema import INTEGRATION

TOOL_NAME = "sourcegraph_change_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Sourcegraph change statistics",
    description=(
...
async def sourcegraph_change_statistics(
    repository: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Sourcegraph's commits across ``group_by``.

    ``start`` and ``end`` are Sourcegraph's own time expressions. Both are
    ...
    client = access.client(SourcegraphClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"sourcegraph:change_statistics:{repository or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no commits were returned by Sourcegraph for this request"
