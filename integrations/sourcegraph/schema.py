"""Sourcegraph's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "sourcegraph"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "sourcegraph.example.com"

#: Every host Sourcegraph serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="sourcegraph.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Sourcegraph access token", min_length=8),
)

RULE: Final = InjectionRule(
    integration=INTEGRATION,
    hosts=HOSTS,
    injections=(HeaderInjection(header="Authorization", field="token", template="token {value}"),),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Sourcegraph is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="sourcegraph.acme.example")``.
    """
