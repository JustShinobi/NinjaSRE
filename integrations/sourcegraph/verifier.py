"""Proves a Sourcegraph credential works, and proves what it is allowed to do.
...
#: What each failure means for Sourcegraph, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Sourcegraph rejected the credential. Re-issue it: Sourcegraph → Settings → Access tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Sourcegraph credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Sourcegraph → Settings → Access tokens"
...
_NO_INTROSPECTION: Final = (
    "Sourcegraph has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="search",
    grants="run code searches across the indexed repositories",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    capabilities=("sourcegraph_change_statistics",),
    where=_GRANTED_AT,
)
...
class SourcegraphVerifier:
    """Checks a stored Sourcegraph credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /.api/graphql — the cheapest read Sourcegraph offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Sourcegraph accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Sourcegraph accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Sourcegraph said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> SourcegraphClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a sourcegraph probe needs a proxy transport and a request context")
        return SourcegraphClient(transport=transport, context=context, region=self.region)
