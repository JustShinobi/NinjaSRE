"""AWS EC2's API, over the proxy, holding no credential.
...
AWS EC2's authentication changed tomorrow, because the authentication is
...
from integrations.aws_ec2.schema import INTEGRATION, base_url
...
class AwsEc2Client(IntegrationClient):
    """AWS EC2, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what AWS EC2 holds for this request, bounded.
        ...
        """
        # AWS EC2 narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(
                items=xml_records(answer, "item"), cursor=xml_text(answer, "nextToken") or None
            )
    ...
    async def list_changes(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"Action": "DescribeInstances", "Version": "2016-11-15"}
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call AWS EC2 offers.
        ...
        return await self.get(
            PING_PATH, params={"Action": "DescribeAccountAttributes", "Version": "2016-11-15"}
        )
