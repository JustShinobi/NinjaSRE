"""Proves a AWS EC2 credential works, and proves what it is allowed to do.
...
from integrations.aws_ec2.client import AwsEc2Client
from integrations.aws_ec2.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for AWS EC2, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS EC2 rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS EC2 credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "AWS EC2 has no permission-introspection endpoint, so each permission is probed "
...
PERMISSION_1: Final = RequiredPermission(
    ...
    grants="read instance state and any scheduled events",
    capabilities=("aws_ec2_resource_inventory",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="ec2:DescribeInstances",
    grants="read instance detail and tags",
    capabilities=("aws_ec2_recent_changes",),
    where=_GRANTED_AT,
)
...
class AwsEc2Verifier:
    """Checks a stored AWS EC2 credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls / — the cheapest read AWS EC2 offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS EC2 accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS EC2 accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS EC2 said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsEc2Client:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_ec2 probe needs a proxy transport and a request context")
        return AwsEc2Client(transport=transport, context=context, region=self.region)
