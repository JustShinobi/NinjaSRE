"""Reading a capped number of AWS EC2's changes, after something narrowed the question.
...
TOOL_NAME = "aws_ec2_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS EC2 recent changes",
    description=(
...
async def aws_ec2_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of AWS EC2's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from AWS EC2{tail}",
                reference=f"aws_ec2:recent_changes:{kind or 'all'}",
            ),
        ),
    )
