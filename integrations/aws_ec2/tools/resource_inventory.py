"""Counting AWS EC2's resources before reading any of them.

The first call of an investigation that reaches AWS EC2, and the one
...
Source of truth: AWS EC2's ``/``, read through the
...
TOOL_NAME = "aws_ec2_resource_inventory"
...
DEFAULT_GROUP_BY = "instanceState.name"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS EC2 resource inventory",
    description=(
...
async def aws_ec2_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of AWS EC2's resources across ``group_by``.

    ``start`` and ``end`` are AWS EC2's own time expressions. Both are
    ...
    client = access.client(AwsEc2Client, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"aws_ec2:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by AWS EC2 for this request"
