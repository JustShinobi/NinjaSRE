"""Vercel's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "vercel"


#: Every host Vercel serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="api.vercel.com", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Vercel access token with read access to the team's projects", min_length=8),
    public("team", "Vercel team id, for a token that spans several"),
)
