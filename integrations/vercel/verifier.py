"""Proves a Vercel credential works, and proves what it is allowed to do.
...
from integrations.vercel.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Vercel, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Vercel rejected the credential. Re-issue it: Vercel → Account Settings → Tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Vercel credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Vercel → Account Settings → Tokens"
...
_NO_INTROSPECTION: Final = (
    "Vercel has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="deployments:read",
    grants="list deployments and their state",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    capabilities=("vercel_pipeline_statistics",),
    where=_GRANTED_AT,
)
...
class VercelVerifier:
    """Checks a stored Vercel credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v2/user — the cheapest read Vercel offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Vercel accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Vercel accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Vercel said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> VercelClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a vercel probe needs a proxy transport and a request context")
        return VercelClient(transport=transport, context=context, region=self.region)
