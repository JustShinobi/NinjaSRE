"""Reading a capped number of Vercel's runs, after something narrowed the question.
...
Source of truth: Vercel's ``/v6/deployments``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Vercel failed runs",
    description=(
...
async def vercel_failed_runs(
    project: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Vercel's runs, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} runs from Vercel{tail}",
                reference=f"vercel:failed_runs:{project or 'all'}",
            ),
        ),
    )
