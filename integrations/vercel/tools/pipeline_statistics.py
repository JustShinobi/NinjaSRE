"""Counting Vercel's runs before reading any of them.

The first call of an investigation that reaches Vercel, and the one
...
Source of truth: Vercel's ``/v6/deployments``, read through the
...
TOOL_NAME = "vercel_pipeline_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Vercel pipeline statistics",
    description=(
...
async def vercel_pipeline_statistics(
    project: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Vercel's runs across ``group_by``.

    ``start`` and ``end`` are Vercel's own time expressions. Both are
    ...
    client = access.client(VercelClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"vercel:pipeline_statistics:{project or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no runs were returned by Vercel for this request"
