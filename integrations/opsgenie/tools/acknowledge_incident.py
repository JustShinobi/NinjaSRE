"""Opsgenie: the one change this integration is allowed to make.
...
Source of truth: Opsgenie's ``/v2/alerts/acknowledge``.
"""
...
from integrations.opsgenie.client import OpsgenieClient
from integrations.opsgenie.schema import INTEGRATION

TOOL_NAME = "opsgenie_acknowledge_incident"
...
class OpsgenieAcknowledgeIncidentRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the Opsgenie change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what Opsgenie returned for {target}, including its identifier.",
                "Undo the change in Opsgenie using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="Opsgenie acknowledge incident",
    description=(
    ...
    rollback_planner=OpsgenieAcknowledgeIncidentRollback(),
    tags=(
        "incident",
        "opsgenie",
        "write",
...
async def opsgenie_acknowledge_incident(incident: str, note: str) -> CapabilityResult:
    """Make the change in Opsgenie and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(OpsgenieClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"Opsgenie accepted the change to {incident!r}",
                reference=f"opsgenie:acknowledge_incident:{incident}",
            ),
        ),
    )
