"""Counting Opsgenie's incidents before reading any of them.

The first call of an investigation that reaches Opsgenie, and the one
...
Source of truth: Opsgenie's ``/v2/alerts``, read through the
...
TOOL_NAME = "opsgenie_incident_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Opsgenie incident statistics",
    description=(
...
async def opsgenie_incident_statistics(
    status: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Opsgenie's incidents across ``group_by``.

    ``start`` and ``end`` are Opsgenie's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"opsgenie:incident_statistics:{status or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no incidents were returned by Opsgenie for this request"
