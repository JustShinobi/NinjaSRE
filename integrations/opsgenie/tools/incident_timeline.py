"""Reading a capped number of Opsgenie's timeline entries, after something narrowed the question.
...
Source of truth: Opsgenie's ``/v2/alerts``.
"""
...
TOOL_NAME = "opsgenie_incident_timeline"
...
@tool(
    name=TOOL_NAME,
    display_name="Opsgenie incident timeline",
    description=(
...
async def opsgenie_incident_timeline(
    incident: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Opsgenie's timeline entries, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"{len(entries)} timeline entries from Opsgenie{tail}",
                reference=f"opsgenie:incident_timeline:{incident or 'all'}",
            ),
        ),
    )
