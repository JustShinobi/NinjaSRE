"""Proves a Opsgenie credential works, and proves what it is allowed to do.
...
#: What each failure means for Opsgenie, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Opsgenie rejected the credential. Re-issue it: Opsgenie → Settings → API key management"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Opsgenie credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Opsgenie → Settings → API key management"
...
_NO_INTROSPECTION: Final = (
    "Opsgenie has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="read",
    grants="read alerts and their logs",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="acknowledge an alert",
    capabilities=("opsgenie_acknowledge_incident",),
    where=_GRANTED_AT,
)
...
class OpsgenieVerifier:
    """Checks a stored Opsgenie credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v2/alerts — the cheapest read Opsgenie offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Opsgenie accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Opsgenie accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Opsgenie said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> OpsgenieClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a opsgenie probe needs a proxy transport and a request context")
        return OpsgenieClient(transport=transport, context=context, region=self.region)
