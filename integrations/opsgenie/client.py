"""Opsgenie's API, over the proxy, holding no credential.
...
Opsgenie's authentication changed tomorrow, because the authentication is
...
from integrations.opsgenie.schema import INTEGRATION, base_url

PING_PATH: Final = "/v2/alerts"
LIST_INCIDENTS_PATH: Final = "/v2/alerts"
INCIDENT_TIMELINE_PATH: Final = "/v2/alerts"
ACKNOWLEDGE_PATH: Final = "/v2/alerts/acknowledge"
...
class OpsgenieClient(IntegrationClient):
    """Opsgenie, reached through the credential proxy."""
    ...
    async def list_incidents(
        ...
        *,
        ...
        """Return what Opsgenie holds for this request, bounded.
        ...
        """
        # Opsgenie narrows this endpoint by other means; the argument stays in the
    ...
    async def incident_timeline(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"query": "status:open", "sort": "createdAt", "order": "desc"}
            asked.update(parameters)
    ...
    async def acknowledge(self, incident: str, note: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what Opsgenie said."""
        body: dict[str, Any] = {"identifier": incident, "note": note, "source": "ninjasre"}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Opsgenie offers.
