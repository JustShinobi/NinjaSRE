"""Opsgenie's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "opsgenie"


#: Every host Opsgenie serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap(
    integration=INTEGRATION,
    regions=(
        Region(name="us", host="api.opsgenie.com", display_name="United States"),
        Region(name="eu", host="api.eu.opsgenie.com", display_name="Europe"),
    ),
    default="us",
)
...
SCHEMA: Final = credential_schema(
    ...
    secret(
        "api_key",
        "Opsgenie API key for an API integration with read and write access",
        min_length=8,
    ),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        HeaderInjection(header="Authorization", field="api_key", template="GenieKey {value}"),
    ),
)
