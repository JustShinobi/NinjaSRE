"""Reading a capped number of Slack's messages, after something narrowed the question.
...
Source of truth: Slack's ``/api/conversations.history``.
"""
...
TOOL_NAME = "slack_recent_messages"
...
@tool(
    name=TOOL_NAME,
    display_name="Slack recent messages",
    description=(
...
async def slack_recent_messages(
    channel: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Slack's messages, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} messages from Slack{tail}",
                reference=f"slack:recent_messages:{channel or 'all'}",
            ),
        ),
    )
