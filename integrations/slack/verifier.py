"""Proves a Slack credential works, and proves what it is allowed to do.
...
#: What each failure means for Slack, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Slack rejected the credential. Re-issue it: api.slack.com/apps → your app → OAuth & Permissions → Bot User OAuth Token"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Slack credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "api.slack.com/apps → your app → OAuth & Permissions → Bot User OAuth Token"
...
_NO_INTROSPECTION: Final = (
    "Slack has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="channels:history",
    grants="read messages in the channels the bot is in",
    capabilities=("slack_recent_messages",),
    where=_GRANTED_AT,
)
...
class SlackVerifier:
    """Checks a stored Slack credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/auth.test — the cheapest read Slack offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Slack accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Slack accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Slack said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> SlackClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a slack probe needs a proxy transport and a request context")
        return SlackClient(transport=transport, context=context, region=self.region)
