"""Reading a capped number of Jira's issues, after something narrowed the question.
...
Source of truth: Jira's ``/rest/api/3/search/jql``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Jira recent issues",
    description=(
...
async def jira_recent_issues(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Jira's issues, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.DOCUMENT,
                summary=f"{len(entries)} issues from Jira{tail}",
                reference=f"jira:recent_issues:{query or 'all'}",
            ),
        ),
    )
