"""Counting Jira's issues before reading any of them.

The first call of an investigation that reaches Jira, and the one
...
Source of truth: Jira's ``/rest/api/3/search/jql``, read through the
...
TOOL_NAME = "jira_issue_statistics"
...
DEFAULT_GROUP_BY = "fields.status.name"
...
@tool(
    name=TOOL_NAME,
    display_name="Jira issue statistics",
    description=(
...
async def jira_issue_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Jira's issues across ``group_by``.

    ``start`` and ``end`` are Jira's own time expressions. Both are
    ...
    client = access.client(JiraClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"jira:issue_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no issues were returned by Jira for this request"
