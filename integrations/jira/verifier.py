"""Proves a Jira credential works, and proves what it is allowed to do.
...
from integrations.jira.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Jira, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Jira rejected the credential. Re-issue it: id.atlassian.com → Security → API tokens"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Jira credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "Jira has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="Browse Projects",
    grants="search and read issues in a project",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="read:jira-user",
    grants="read the token owner, which the probe uses",
    capabilities=("jira_issue_statistics",),
    where=_GRANTED_AT,
)
...
class JiraVerifier:
    """Checks a stored Jira credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /rest/api/3/myself — the cheapest read Jira offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Jira accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Jira accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Jira said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> JiraClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a jira probe needs a proxy transport and a request context")
        return JiraClient(transport=transport, context=context, region=self.region)
