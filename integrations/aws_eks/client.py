"""AWS EKS's API, over the proxy, holding no credential.
...
AWS EKS's authentication changed tomorrow, because the authentication is
...
from integrations._base.payload import named, text
from integrations._base.retry import RetryPolicy
from integrations._base.transport import ProxyTransport, RequestContext
from integrations.aws_eks.schema import INTEGRATION, base_url

PING_PATH: Final = "/clusters"
LIST_RESOURCES_PATH: Final = "/clusters"
LIST_CHANGES_PATH: Final = "/clusters"
...
class AwsEksClient(IntegrationClient):
    """AWS EKS, reached through the credential proxy."""
    ...
    async def list_resources(
        ...
        *,
        ...
        """Return what AWS EKS holds for this request, bounded.
        ...
        """
        # AWS EKS narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(
                items=named(answer, "clusters", key="cluster"),
                cursor=text(answer, "nextToken") or None,
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call AWS EKS offers.
        ...
        """
        return await self.get(PING_PATH, params={"maxResults": "1"})
