"""Proves a AWS EKS credential works, and proves what it is allowed to do.
...
from integrations.aws_eks.client import AwsEksClient
from integrations.aws_eks.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for AWS EKS, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "AWS EKS rejected the credential. Re-issue it: IAM → Users → Security credentials, or the role your deployment assumes"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No AWS EKS credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "AWS EKS has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="eks:ListClusters",
    grants="list the clusters in this account and region",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read a cluster's version, endpoint, and status",
    capabilities=("aws_eks_recent_changes",),
    where=_GRANTED_AT,
)
...
class AwsEksVerifier:
    """Checks a stored AWS EKS credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /clusters — the cheapest read AWS EKS offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether AWS EKS accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="AWS EKS accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what AWS EKS said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> AwsEksClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a aws_eks probe needs a proxy transport and a request context")
        return AwsEksClient(transport=transport, context=context, region=self.region)
