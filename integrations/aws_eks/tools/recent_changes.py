"""Reading a capped number of AWS EKS's changes, after something narrowed the question.
...
Source of truth: AWS EKS's ``/clusters``.
"""
...
TOOL_NAME = "aws_eks_recent_changes"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS EKS recent changes",
    description=(
...
async def aws_eks_recent_changes(
    kind: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of AWS EKS's changes, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.CHANGE,
                summary=f"{len(entries)} changes from AWS EKS{tail}",
                reference=f"aws_eks:recent_changes:{kind or 'all'}",
            ),
        ),
    )
