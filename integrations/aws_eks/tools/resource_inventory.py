"""Counting AWS EKS's resources before reading any of them.

The first call of an investigation that reaches AWS EKS, and the one
...
Source of truth: AWS EKS's ``/clusters``, read through the
...
TOOL_NAME = "aws_eks_resource_inventory"
...
@tool(
    name=TOOL_NAME,
    display_name="AWS EKS resource inventory",
    description=(
...
async def aws_eks_resource_inventory(
    kind: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of AWS EKS's resources across ``group_by``.

    ``start`` and ``end`` are AWS EKS's own time expressions. Both are
    ...
    client = access.client(AwsEksClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"aws_eks:resource_inventory:{kind or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no resources were returned by AWS EKS for this request"
