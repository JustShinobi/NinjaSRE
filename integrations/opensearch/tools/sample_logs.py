"""Reading a capped number of OpenSearch's log lines, after something narrowed the question.
...
Source of truth: OpenSearch's ``/_search``.
"""
...
TOOL_NAME = "opensearch_sample_logs"
...
@tool(
    name=TOOL_NAME,
    display_name="OpenSearch log samples",
    description=(
...
async def opensearch_sample_logs(
    query: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of OpenSearch's log lines, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.LOG,
                summary=f"{len(entries)} log lines from OpenSearch{tail}",
                reference=f"opensearch:sample_logs:{query or 'all'}",
            ),
        ),
    )
