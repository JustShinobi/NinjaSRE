"""Counting OpenSearch's log lines before reading any of them.

The first call of an investigation that reaches OpenSearch, and the one
...
Source of truth: OpenSearch's ``/_search``, read through the
...
TOOL_NAME = "opensearch_log_statistics"
...
DEFAULT_GROUP_BY = "_source.log.level"
...
@tool(
    name=TOOL_NAME,
    display_name="OpenSearch log statistics",
    description=(
...
async def opensearch_log_statistics(
    query: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of OpenSearch's log lines across ``group_by``.

    ``start`` and ``end`` are OpenSearch's own time expressions. Both are
    ...
    client = access.client(OpensearchClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"opensearch:log_statistics:{query or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no log lines were returned by OpenSearch for this request"
