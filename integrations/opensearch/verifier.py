"""Proves a OpenSearch credential works, and proves what it is allowed to do.
...
from integrations.opensearch.client import OpensearchClient
from integrations.opensearch.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for OpenSearch, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "OpenSearch rejected the credential. Re-issue it: OpenSearch Dashboards → Security → Internal users, or your identity provider"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No OpenSearch credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "OpenSearch Dashboards → Security → Internal users, or your identity provider"
...
_NO_INTROSPECTION: Final = (
    "OpenSearch has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="indices:data/read/search",
    grants="search the indices this user can see",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="read cluster health, which the connectivity probe uses",
    capabilities=("opensearch_log_statistics",),
    where=_GRANTED_AT,
)
...
class OpensearchVerifier:
    """Checks a stored OpenSearch credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /_cluster/health — the cheapest read OpenSearch offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether OpenSearch accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="OpenSearch accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what OpenSearch said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> OpensearchClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a opensearch probe needs a proxy transport and a request context")
        return OpensearchClient(transport=transport, context=context, region=self.region)
