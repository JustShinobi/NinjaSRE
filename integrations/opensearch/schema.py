"""OpenSearch's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "opensearch"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "opensearch.example.com"

#: Every host OpenSearch serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="opensearch.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("username", "OpenSearch user with read access to the log indices", min_length=8),
    secret(
        "password",
        "That user's password, or the one the internal user database holds",
        min_length=8,
    ),
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    OpenSearch is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="opensearch.acme.example")``.
    """
