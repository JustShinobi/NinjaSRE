"""Counting FireHydrant's incidents before reading any of them.

The first call of an investigation that reaches FireHydrant, and the one
...
Source of truth: FireHydrant's ``/v1/incidents``, read through the
...
TOOL_NAME = "firehydrant_incident_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="FireHydrant incident statistics",
    description=(
...
async def firehydrant_incident_statistics(
    status: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of FireHydrant's incidents across ``group_by``.

    ``start`` and ``end`` are FireHydrant's own time expressions. Both are
    ...
    try:
        found = await client.list_incidents(status, start=start, end=end, limit=MAX_RECORDS)
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"firehydrant:incident_statistics:{status or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no incidents were returned by FireHydrant for this request"
    leader = leader_of(buckets)
    tail = ", and more matched than were counted" if truncated else ""
    return (
        f"{total} incidents across {len(buckets)} {group_by} group(s); the largest is "
        f"{leader!r}{tail}"
    )
