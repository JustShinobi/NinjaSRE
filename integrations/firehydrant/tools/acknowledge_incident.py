"""FireHydrant: the one change this integration is allowed to make.
...
Source of truth: FireHydrant's ``/v1/incidents/events``.
"""
...
TOOL_NAME = "firehydrant_acknowledge_incident"
...
class FirehydrantAcknowledgeIncidentRollback:
    ...
    def plan(self, arguments: Mapping[str, Any]) -> DeclaredRollbackPlan:
        ...
        return DeclaredRollbackPlan(
            summary=(
                f"Reverse the FireHydrant change made to {target}, and say in the same "
                "place that it was reversed."
            ),
            steps=(
                f"Record what FireHydrant returned for {target}, including its identifier.",
                "Undo the change in FireHydrant using that identifier.",
...
@tool(
    name=TOOL_NAME,
    display_name="FireHydrant acknowledge incident",
    description=(
    ...
    rollback_planner=FirehydrantAcknowledgeIncidentRollback(),
    tags=(
        "incident",
        "firehydrant",
        "write",
...
async def firehydrant_acknowledge_incident(incident: str, note: str) -> CapabilityResult:
    """Make the change in FireHydrant and return what it said."""
    access = current()
    if access is None:
        return unconfigured(TOOL_NAME, INTEGRATION)

    client = access.client(FirehydrantClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"FireHydrant accepted the change to {incident!r}",
                reference=f"firehydrant:acknowledge_incident:{incident}",
            ),
        ),
    )
