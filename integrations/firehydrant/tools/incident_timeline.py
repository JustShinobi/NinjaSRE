"""Reading a capped number of FireHydrant's timeline entries, after something narrowed the question.
...
Source of truth: FireHydrant's ``/v1/incidents``.
"""
...
TOOL_NAME = "firehydrant_incident_timeline"
...
@tool(
    name=TOOL_NAME,
    display_name="FireHydrant incident timeline",
    description=(
...
async def firehydrant_incident_timeline(
    incident: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of FireHydrant's timeline entries, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.INCIDENT,
                summary=f"{len(entries)} timeline entries from FireHydrant{tail}",
                reference=f"firehydrant:incident_timeline:{incident or 'all'}",
            ),
        ),
    )
