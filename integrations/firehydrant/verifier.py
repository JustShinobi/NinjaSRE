"""Proves a FireHydrant credential works, and proves what it is allowed to do.
...
from integrations.firehydrant.client import FirehydrantClient
from integrations.firehydrant.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for FireHydrant, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "FireHydrant rejected the credential. Re-issue it: FireHydrant → Settings → API keys"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No FireHydrant credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "FireHydrant → Settings → API keys"
...
_NO_INTROSPECTION: Final = (
    "FireHydrant has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="incidents:read",
    grants="read incidents and their events",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    grants="post a note onto an incident",
    capabilities=("firehydrant_acknowledge_incident",),
    where=_GRANTED_AT,
)
...
class FirehydrantVerifier:
    """Checks a stored FireHydrant credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v1/ping — the cheapest read FireHydrant offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether FireHydrant accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="FireHydrant accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what FireHydrant said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> FirehydrantClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a firehydrant probe needs a proxy transport and a request context")
        return FirehydrantClient(transport=transport, context=context, region=self.region)
