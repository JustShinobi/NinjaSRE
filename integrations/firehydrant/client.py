"""FireHydrant's API, over the proxy, holding no credential.
...
FireHydrant's authentication changed tomorrow, because the authentication is
...
from integrations.firehydrant.schema import INTEGRATION, base_url

PING_PATH: Final = "/v1/ping"
LIST_INCIDENTS_PATH: Final = "/v1/incidents"
INCIDENT_TIMELINE_PATH: Final = "/v1/incidents"
ACKNOWLEDGE_PATH: Final = "/v1/incidents/events"
...
class FirehydrantClient(IntegrationClient):
    """FireHydrant, reached through the credential proxy."""
    ...
    async def list_incidents(
        ...
        *,
        ...
        """Return what FireHydrant holds for this request, bounded.
        ...
        """
        # FireHydrant narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"current_milestones[]": status}
            asked.update(parameters)
            answer = (await self.get(LIST_INCIDENTS_PATH, params=asked)).json()
            return Page(items=records(answer, "data"), cursor="" or None)
    ...
    async def incident_timeline(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"current_milestones[]": "started"}
            asked.update(parameters)
    ...
    async def acknowledge(self, incident: str, note: str) -> dict[str, Any]:
        """Make the one change this integration may make, and return what FireHydrant said."""
        body: dict[str, Any] = {"incident_id": incident, "body": note, "type": "note"}
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call FireHydrant offers.
