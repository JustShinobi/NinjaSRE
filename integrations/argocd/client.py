"""Argo CD's API, over the proxy, holding no credential.
...
Argo CD's authentication changed tomorrow, because the authentication is
...
LIST_FAILED_RUNS_PATH: Final = "/api/v1/applications"
...
class ArgocdClient(IntegrationClient):
    """Argo CD, reached through the credential proxy."""
    ...
    async def list_runs(
        ...
        *,
        ...
        """Return what Argo CD holds for this request, bounded.
        ...
        """
        # Argo CD narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {"project": project}
            asked.update(parameters)
            answer = (await self.get(LIST_RUNS_PATH, params=asked)).json()
            return Page(items=records(answer, "items"), cursor="" or None)
    ...
    async def list_failed_runs(
        ...
        *,
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            asked: dict[str, str] = {
                "project": project,
                "selector": "argocd.argoproj.io/sync-status!=Synced",
            }
            asked.update(parameters)
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Argo CD offers.
