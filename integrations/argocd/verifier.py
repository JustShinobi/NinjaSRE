"""Proves a Argo CD credential works, and proves what it is allowed to do.
...
#: What each failure means for Argo CD, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Argo CD rejected the credential. Re-issue it: Settings → Accounts → generate token, or `argocd account generate-token`"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Argo CD credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Settings → Accounts → generate token, or `argocd account generate-token`"
...
_NO_INTROSPECTION: Final = (
    "Argo CD has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="applications, get",
    grants="read application state and sync status",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="account, get",
    grants="read the account, which the probe uses",
    capabilities=("argocd_pipeline_statistics",),
    where=_GRANTED_AT,
)
...
class ArgocdVerifier:
    """Checks a stored Argo CD credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/v1/session/userinfo — the cheapest read Argo CD offers that still "
    ...
    def probes(self) -> tuple[PermissionProbe, ...]:
        ...
        return (
            client_probe(
                PERMISSION_1,
                build=self._client,
                call=lambda client: client.list_runs(limit=1),
                fallback_note=_NO_INTROSPECTION,
            ),
            client_probe(
                PERMISSION_2,
                build=self._client,
                call=lambda client: client.list_failed_runs(limit=1),
                fallback_note=_NO_INTROSPECTION,
            ),
        )

    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Argo CD accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Argo CD accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Argo CD said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> ArgocdClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a argocd probe needs a proxy transport and a request context")
        return ArgocdClient(transport=transport, context=context, region=self.region)
