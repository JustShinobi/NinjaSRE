"""Counting Argo CD's runs before reading any of them.

The first call of an investigation that reaches Argo CD, and the one
...
Source of truth: Argo CD's ``/api/v1/applications``, read through the
...
TOOL_NAME = "argocd_pipeline_statistics"
...
DEFAULT_GROUP_BY = "status.sync.status"
...
@tool(
    name=TOOL_NAME,
    display_name="Argo CD pipeline statistics",
    description=(
...
async def argocd_pipeline_statistics(
    project: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Argo CD's runs across ``group_by``.

    ``start`` and ``end`` are Argo CD's own time expressions. Both are
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"argocd:pipeline_statistics:{project or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no runs were returned by Argo CD for this request"
