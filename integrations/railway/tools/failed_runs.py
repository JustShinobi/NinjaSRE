"""Reading a capped number of Railway's runs, after something narrowed the question.
...
Source of truth: Railway's ``/graphql/v2``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Railway failed runs",
    description=(
...
async def railway_failed_runs(
    project: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Railway's runs, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} runs from Railway{tail}",
                reference=f"railway:failed_runs:{project or 'all'}",
            ),
        ),
    )
