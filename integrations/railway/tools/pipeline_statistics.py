"""Counting Railway's runs before reading any of them.

The first call of an investigation that reaches Railway, and the one
...
Source of truth: Railway's ``/graphql/v2``, read through the
...
TOOL_NAME = "railway_pipeline_statistics"
...
@tool(
    name=TOOL_NAME,
    display_name="Railway pipeline statistics",
    description=(
...
async def railway_pipeline_statistics(
    project: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Railway's runs across ``group_by``.

    ``start`` and ``end`` are Railway's own time expressions. Both are
    ...
    client = access.client(RailwayClient, capability=TOOL_NAME)
    try:
        found = await client.list_runs(project, start=start, end=end, limit=MAX_RECORDS)
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"railway:pipeline_statistics:{project or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no runs were returned by Railway for this request"
    leader = leader_of(buckets)
    tail = ", and more matched than were counted" if truncated else ""
    return (
        f"{total} runs across {len(buckets)} {group_by} group(s); the largest is {leader!r}{tail}"
    )
