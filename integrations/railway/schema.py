"""Railway's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "railway"


#: Every host Railway serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="backboard.railway.app", name="global")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Railway API token, project-scoped or account-scoped", min_length=8),
    public("project_id", "Railway project id the queries are scoped to"),
)
