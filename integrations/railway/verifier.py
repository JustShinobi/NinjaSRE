"""Proves a Railway credential works, and proves what it is allowed to do.
...
from integrations.railway.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Railway, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Railway rejected the credential. Re-issue it: Railway → Account Settings → Tokens, or a project token from project settings"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Railway credential is configured for this team."
    ),
...
_GRANTED_AT: Final = "Railway → Account Settings → Tokens, or a project token from project settings"
...
_NO_INTROSPECTION: Final = (
    "Railway has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="deployments:read",
    grants="read the project's deployments",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    ...
    capabilities=("railway_pipeline_statistics",),
    where=_GRANTED_AT,
)
...
class RailwayVerifier:
    """Checks a stored Railway credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /graphql/v2 — the cheapest read Railway offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Railway accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Railway accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Railway said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> RailwayClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a railway probe needs a proxy transport and a request context")
        return RailwayClient(transport=transport, context=context, region=self.region)
