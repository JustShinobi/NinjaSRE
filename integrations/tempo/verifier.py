"""Proves a Tempo credential works, and proves what it is allowed to do.
...
from integrations.tempo.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Tempo, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Tempo rejected the credential. Re-issue it: Grafana Cloud → Access Policies, or whatever your gateway issues for a self-hosted install"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Tempo credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "Tempo has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="traces:read",
    grants="search the trace store",
    capabilities=(
...
PERMISSION_2: Final = RequiredPermission(
    name="echo",
    grants="the unauthenticated liveness endpoint the probe uses",
    capabilities=("tempo_trace_statistics",),
    where=_GRANTED_AT,
)
...
class TempoVerifier:
    """Checks a stored Tempo credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /api/echo — the cheapest read Tempo offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Tempo accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Tempo accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Tempo said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> TempoClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a tempo probe needs a proxy transport and a request context")
        return TempoClient(transport=transport, context=context, region=self.region)
