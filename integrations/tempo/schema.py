"""Tempo's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "tempo"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "tempo.example.com"

#: Every host Tempo serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="tempo.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    INTEGRATION,
    secret("token", "Tempo bearer token, or the Grafana Cloud access policy token", min_length=8),
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Tempo is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="tempo.acme.example")``.
    """
