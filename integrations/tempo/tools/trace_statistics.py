"""Counting Tempo's traces before reading any of them.

The first call of an investigation that reaches Tempo, and the one
...
Source of truth: Tempo's ``/api/search``, read through the
...
TOOL_NAME = "tempo_trace_statistics"
...
DEFAULT_GROUP_BY = "rootServiceName"
...
@tool(
    name=TOOL_NAME,
    display_name="Tempo trace statistics",
    description=(
...
async def tempo_trace_statistics(
    service: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Tempo's traces across ``group_by``.

    ``start`` and ``end`` are Tempo's own time expressions. Both are
    ...
    client = access.client(TempoClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"tempo:trace_statistics:{service or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no traces were returned by Tempo for this request"
