"""Reading a capped number of Tempo's traces, after something narrowed the question.
...
Source of truth: Tempo's ``/api/search``.
"""
...
@tool(
    name=TOOL_NAME,
    display_name="Tempo slow traces",
    description=(
...
async def tempo_slow_traces(
    service: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Tempo's traces, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.TRACE,
                summary=f"{len(entries)} traces from Tempo{tail}",
                reference=f"tempo:slow_traces:{service or 'all'}",
            ),
        ),
    )
