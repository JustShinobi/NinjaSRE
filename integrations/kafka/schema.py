"""Kafka's credential shape, hosts, and how its secret enters a request.
...
INTEGRATION: Final = "kafka"


#: Where a default install serves its API. See ``rule_for``.
DEFAULT_HOST: Final = "kafka-rest.example.com"

#: Every host Kafka serves its API from. Exact names, no wildcards.
REGIONS: Final = RegionMap.single(INTEGRATION, host="kafka-rest.example.com", name="self-hosted")
...
SCHEMA: Final = credential_schema(
    ...
    secret(
        "username",
        "Kafka REST Proxy API key, or the cluster API key on Confluent Cloud",
        min_length=8,
    ),
    secret("password", "That key's secret", min_length=8),
    public("cluster", "Kafka cluster id", required=True),
)
...
RULE: Final = InjectionRule(
    ...
    hosts=HOSTS,
    injections=(
        BasicAuthInjection(username_field="username", password_field="password"),
        PathSegmentInjection(placeholder="cluster", field="cluster"),
    ),
)
...
def rule_for(*hosts: str) -> InjectionRule:
    ...
    Kafka is self-hosted, so nobody writing this file knows where it is.
...
def regions_for(**endpoints: str) -> RegionMap:
    ...
    ``regions_for(prod="kafka.acme.example")``.
    """
