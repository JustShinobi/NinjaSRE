"""Proves a Kafka credential works, and proves what it is allowed to do.
...
from integrations.kafka.client import KafkaClient
from integrations.kafka.schema import INTEGRATION
from platform.credentials.verification import VerificationResult

#: What each failure means for Kafka, in the operator's terms. "401" and
...
_ADVICE: dict[IntegrationErrorReason, str] = {
    IntegrationErrorReason.UNAUTHENTICATED: (
        "Kafka rejected the credential. Re-issue it: Confluent Cloud → API keys, or the credentials your REST Proxy deployment uses"
    ),
    ...
    IntegrationErrorReason.CREDENTIAL_UNAVAILABLE: (
        "No Kafka credential is configured for this team."
    ),
...
_NO_INTROSPECTION: Final = (
    "Kafka has no permission-introspection endpoint, so each permission is probed "
    "by making the cheapest form of the read that needs it"
)

PERMISSION_1: Final = RequiredPermission(
    name="DescribeTopics",
    grants="list topics on the cluster",
    capabilities=("kafka_pipeline_health",),
    where=_GRANTED_AT,
)
PERMISSION_2: Final = RequiredPermission(
    name="DescribeGroups",
    grants="list consumer groups and their state",
    capabilities=("kafka_recent_failures",),
    where=_GRANTED_AT,
)
...
class KafkaVerifier:
    """Checks a stored Kafka credential end to end, and what it may do."""
    ...
    def probe_description(self) -> str:
        ...
        return (
            "calls /v3/clusters/{cluster} — the cheapest read Kafka offers that still "
    ...
    async def connect(self, transport: object, context: object) -> Connectivity:
        """Return whether Kafka accepted this team's credential."""
        ...
        return Connectivity(
            reachable=True,
            detail="Kafka accepted the credential.",
            status_code=response.status_code,
        )

    async def probe(self, transport: object, context: object) -> VerificationResult:
        """Return what Kafka said when this team's credential was used."""
    ...
    def _client(self, transport: object, context: object) -> KafkaClient:
        """Return a client for one probe. It holds no credential; the proxy injects one."""
        if not isinstance(transport, ProxyTransport) or not isinstance(context, RequestContext):
            raise TypeError("a kafka probe needs a proxy transport and a request context")
        return KafkaClient(transport=transport, context=context, region=self.region)
