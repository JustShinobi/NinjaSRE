"""Reading a capped number of Kafka's failures, after something narrowed the question.
...
Source of truth: Kafka's ``/v3/clusters/{cluster}/consumer-groups``.
"""
...
TOOL_NAME = "kafka_recent_failures"
...
@tool(
    name=TOOL_NAME,
    display_name="Kafka recent failures",
    description=(
...
async def kafka_recent_failures(
    scope: str = "",
    start: str = "",
    end: str = "",
    limit: int = DEFAULT_LIMIT,
) -> CapabilityResult:
    """Return up to ``limit`` of Kafka's failures, newest first."""
    access = current()
    if access is None:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                source=INTEGRATION,
                evidence_type=EvidenceType.EVENT,
                summary=f"{len(entries)} failures from Kafka{tail}",
                reference=f"kafka:recent_failures:{scope or 'all'}",
            ),
        ),
    )
