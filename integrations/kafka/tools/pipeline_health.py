"""Counting Kafka's tasks before reading any of them.

The first call of an investigation that reaches Kafka, and the one
...
Source of truth: Kafka's ``/v3/clusters/{cluster}/topics``, read through the
...
TOOL_NAME = "kafka_pipeline_health"
...
@tool(
    name=TOOL_NAME,
    display_name="Kafka pipeline health",
    description=(
...
async def kafka_pipeline_health(
    scope: str = "",
    start: str = "",
    end: str = "",
    group_by: str = DEFAULT_GROUP_BY,
) -> CapabilityResult:
    """Return the distribution of Kafka's tasks across ``group_by``.

    ``start`` and ``end`` are Kafka's own time expressions. Both are
    ...
    client = access.client(KafkaClient, capability=TOOL_NAME)
    try:
    ...
    return CapabilityResult.ok(
        ...
        evidence=(
            Evidence(
                ...
                reference=f"kafka:pipeline_health:{scope or 'all'}",
            ),
        ),
    )
...
def _summary(
    ...
    if not buckets:
        return "no tasks were returned by Kafka for this request"
