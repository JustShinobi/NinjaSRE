"""Kafka's API, over the proxy, holding no credential.
...
Kafka's authentication changed tomorrow, because the authentication is
...
from integrations.kafka.schema import INTEGRATION, base_url

PING_PATH: Final = "/v3/clusters/{cluster}"
LIST_TASKS_PATH: Final = "/v3/clusters/{cluster}/topics"
LIST_FAILURES_PATH: Final = "/v3/clusters/{cluster}/consumer-groups"
...
class KafkaClient(IntegrationClient):
    """Kafka, reached through the credential proxy."""
    ...
    async def list_tasks(
        ...
        *,
        ...
        """Return what Kafka holds for this request, bounded.
        ...
        """
        # Kafka narrows this endpoint by other means; the argument stays in the
        ...
        async def fetch(parameters: Mapping[str, str]) -> Page[dict[str, Any]]:
            ...
            return Page(
                items=records(answer, "data"), cursor=text(answer, "metadata", "next") or None
            )
    ...
    async def ping(self) -> ClientResponse:
        """Make the cheapest authenticated call Kafka offers.
