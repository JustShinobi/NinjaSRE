...
describe('the empty state answers what was fetched, not what survived the audience filter', () => {
  it('shows the table, not "nothing recorded", when every fetched event is system noise', async () => {
    serve([
      systemEvent('evt-1', '2026-08-13T11:59:50Z'),
    ...
    // Two events were genuinely fetched — they are just all system noise, kept
    // out of the default reading. The panel must not claim nothing happened.
  ...
  it('shows "nothing recorded" only when nothing was fetched at all', async () => {
    serve([], 0);
    await audit();
