 ...
 * tabs of Signals.
 *
 * Two tabs, in the order data moves: what arrives and what is done with it on
 * "Intake", where the result goes on "Destinations". That is the order an
 * operator traces when something did not happen, and putting it in one
 * screen's worth of tabs is the point — the alternative, and what this
 * replaces, is five screens and a guess.
 *
 ...
 *
 * **Each tab reads the delivery ledger on its own.** The two used to share one
 * read on a single three-column screen because both were on the page at once;
 * split into tabs, only one of them is ever rendered for a given request, so
 * each asking for its own half is one read, not two answers about one instant.
 */

/** The permission the gateway requires to change what this screen shows. */
const WRITE = 'config.write';

/** The "Intake" tab: what arrives, and what happens to it. */
export async function IntakeTab(context: SurfaceContext): Promise<ReactNode> {
  const { credential, locale, viewer, now, zone } = context;
  const init = authorised(credential);

  const [ingress, rules, deliveries, receivers, setup] = await Promise.all([
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2" data-testid="transit-columns">
      {/* --- What arrives ------------------------------------------------ */}
      <Panel
            ...
            return (
              ...
              >
                ...
                {silent ? (
                  ...
                  <span className="text-meta text-muted" data-testid="never-delivered">
                ...
                <span className="text-meta text-muted">{text(source, 'expects')}</span>
...
/** The "Destinations" tab: where the result goes. */
export async function DestinationsTab(context: SurfaceContext): Promise<ReactNode> {
