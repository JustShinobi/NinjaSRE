...
- [~] T001 Rodar (feito pelo orquestrador, não reexecutado — ver controle.md) `make verify` na árvore intacta e guardar o log **fora
...
- [x] T002 Registrar a lista de requests do load de `/` no staging atual
      (Playwright, `page.on('request')`) e guardar fora do repositório —
      é o "antes" da propriedade zero-egress, que deve ser idêntica no
      "depois".
- [x] T003 Registrar contagem e resultado da suíte de cenários sintéticos
...
- [x] T005 [P] Estender `console/tests/unit/design/tokens.test.ts` (ou o
...
- [x] T006 [P] Teste de unidade novo para as primitivas de motion: o CSS
...
- [x] T007 [P] Teste de unidade para o contorno do chip: `ROLE_SKIN` de
...
- [x] T007b [P] Caracterizar a superfície de export de
      `console/src/design/icons.tsx`: um teste que fixa a lista ordenada
      de nomes exportados e a forma das props de cada ícone, verde antes
      da mudança e obrigatoriamente verde depois. É a prova de que o
      redesenho não renomeia nem muda assinatura; `tsc` sozinho não é
      essa prova, e não estava escrita em lugar nenhum.

## Phase 2: A tabela

- [x] T008 Aplicar em `console/src/design/tokens.ts` a paleta completa da
...
- [x] T009 Atualizar o tema Tailwind em `console/src/app/globals.css`
...
- [x] T010 Registrar no controle a tabela final 26×2 com a origem de cada
      valor (board `SPEC.md` / decisão de contraste), como a spec exige.

## Phase 3: Fontes

- [x] T011 Obter os oito woff2 (SG 500/600/700; Plex Sans 400/500/600;
...
- [x] T012 Declarar os `@font-face` em `globals.css`
...
- [x] T013 Acrescentar em `console/src/app/layout.tsx` o preload de
      display-700, sans-400, sans-500 e mono-400
      (`<link rel="preload" as="font" type="font/woff2" crossOrigin>`).

## Phase 4: Chip, ícones, motion

- [x] T014 `console/src/components/status.tsx`: `ROLE_SKIN` soma
      `border-{papel}` ao tint de cada um dos cinco papéis; nada mais
      muda no arquivo.
- [x] T015 Redesenhar os ícones de `console/src/design/icons.tsx` no
      ...
      o estilo nos demais; exports e assinaturas intactos — a prova é o
      teste de superfície de export de T007b continuar verde, não o de
      T007, que trata do chip e não dos ícones.
- [x] T016 Declarar em `globals.css` os keyframes e classes `pulse-live`,
...
- [~] T017 `console/src/live/auto-refresh.tsx` (já existia de onda anterior — ver controle.md): a marca do estado live
      passa a usar `pulse-live`; os outros três estados ficam como estão.

## Phase 5: Shell

- [x] T018 `console/src/shell/topbar.tsx`: altura pelo token de shell
...
- [x] T019 Sidebar: largura pelo token (232px), rótulos dos três grupos
...
- [x] T020 Varredura de hex literal da paleta antiga em `console/src/`
...
- [x] T021 Acceptance da feature verde no harness local; as duas tarefas
      de caracterização (requests, formas) verdes antes e depois.
- [x] T022 Recapturar as baselines
...
- [ ] T023 `make verify` completo, verde, partindo do verde de T001;
      ...
      plano — nenhuma tela de área, rota, dado ou arquivo de gateway
      tocado. O resultado da conferência entra no controle; um arquivo
      fora da lista é defeito, não licença.
