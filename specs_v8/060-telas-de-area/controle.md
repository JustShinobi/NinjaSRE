...
## Verificação, limpa (sem contenção) — checkpoint de meio de slot

> Os números abaixo são deste checkpoint, **não** do estado final: as cinco rodadas
> de reparo do gate visual (T035–T039) acrescentaram testes depois dele. O verifier
> independente remediu em árvore mergeada e obteve **61 passaram, 8 pulados, 0
> falharam** nos mesmos quatro specs — estritamente mais cobertura, nada regredido.
...
| 5. O agente — os seis estágios eram ícone solto, não estação (T039) | FEITO | `console/src/surfaces/screens/agent-pipeline-metro.ts:26-62`: `StageTreatment` (`'passed' \| 'running' \| 'not-reached'`), `stageTreatment` (nova, pura, testada) decide o tratamento de um estágio a partir da ordem do pipeline e do estágio corrente (se algum); `STAGE_TREATMENT_CLASSES` (novo, testado) mapeia cada tratamento aos tokens do console — `passed`: `border-accent bg-success-bg text-accent`; `running`: `border-accent bg-accent text-on-accent`; `not-reached`: `border-border-strong bg-neutral-bg text-muted` — cada um dos cinco enumerados a seguir bate, valor por valor, com o hexadecimal que o board usa no tema escuro (`neutral-bg` é a exceção e não está na lista: o `SPEC.md` do board o declara `#0d1311`, que é o que o token tem, enquanto o estilo inline daquela estação no artboard usa `#131c18` — o console segue a paleta declarada, e a divergência é do artboard contra o próprio `SPEC.md`) (`accent`=`#3ad195`, `on-accent`=`#062018`, `success-bg`=`#12271f`, `border-strong`=`#35493f`, `muted`=`#9fb2aa`, todos exatos contra `TheAgent.dc.html:157-212`). `console/src/surfaces/screens/agent.tsx:519-537`: o `<span>` do ícone cresceu de `size-6` (32px) para `size-7` (48px — o degrau mais próximo que a escala de espaçamento tem do 54px do board; a escala não tem 54 e não ganhou um) com `edge-emphasis` (2px, o mesmo valor exato do board) por anel, `rounded-full` mantido; `data-testid="pipeline-metro-station"` e `data-treatment` novos para leitura direta por teste. O estágio `running` ganha `pulse-live`/`pulse-live-ring` (utilitário já existente em `globals.css`, o mesmo halo que `runs.tsx` já usa para um run ao vivo), com um `<span>` próprio em `text-accent` para que o halo pulse na cor de destaque mesmo com o ícone do estágio em `text-on-accent`. Nada em `console/src/design/` foi tocado — todo token usado já existia. **Por que `currentStageName` é sempre `undefined` na tela real, e não um defeito**: nem o dataset local (`fixtures/scenarios/populated/runs.json`, o único run `running` do cenário, `run-0003`) nem `InvestigationSummary` (`platform/incidents/investigation_summary.py`, confirmado por leitura direta e pelo schema em `fixtures/contract/openapi.json`) carregam um campo que nomeie em qual dos seis estágios um run em execução está — só `status`. Esse campo (`last_completed_stage`/`stage_index`) é dono de uma feature ainda não executada nesta onda; o mesmo grau de degradação honesta, pelo mesmo motivo, já está registrado no `controle.md` da feature 030 desta onda. Sem ele, todo estágio desenha `not-reached` — em staging (682 completos, 11 interrompidos, zero rodando, medido) e no dataset local igualmente — o que é a leitura honesta, não uma lacuna desta tarefa; o mecanismo aceita e testa os outros dois tratamentos mesmo assim. **Prova**: dois testes novos em `agent-pipeline.acceptance.spec.ts` (`console/tests/e2e/agent-pipeline.acceptance.spec.ts:104-159`) — geometria real da estação (caixa ≥40×40px quadrada, `border-style: solid`, largura de borda computada > 0, raio computado ≥ metade da caixa) e o tratamento honesto contra o dataset local (`not-reached` nos seis) — vermelhos confirmados contra o build sem a mudança (`pipeline-metro-station`: `Expected: 6, Received: 0`), verdes depois; treze testes novos em `agent-pipeline-metro.test.ts` para `stageTreatment`/`STAGE_TREATMENT_CLASSES` isolados, vermelhos primeiro (`stageTreatment is not a function`, `Cannot read properties of undefined`), verdes depois. Corte de fio duplo: (a) `edge-emphasis` removido da estação → `Expected: > 0, Received: 0` na largura de borda medida pelo teste de geometria; (b) o ramo "passado" de `stageTreatment` colapsado para sempre `not-reached` → vermelho isolado só no teste que fixa esse ramo (`Expected: "passed", Received: "not-reached"`) — ambos restaurados, `git diff` limpo, verde de novo. `console/visual/screens.json`: `pending_because` de `agent-1440-light` (já `pending`) estendido para nomear esta mudança; nenhum aceite rodado. |
...
> **Estado atual, escrito depois:** os dois foram declarados pelo orquestrador em
> `c04700c4` e registrados em `DIVERGENCIAS.md` §8 — `mitigated` como `warning` +
> `dimmed-circle`, `false_positive` como `neutral` + `dash`. A recomendação abaixo
> pedia declarar só o primeiro; o argumento foi revisto porque nenhum dos dois é
> produzido por uma investigação real (`MemoryEpisode.outcome` é um booleano) e
> ambos vêm do mesmo semeador de demonstração, o que os põe na posição idêntica.
> Fica registrada como o raciocínio, não como o estado.
...
## Achado do slot, registrado para não se perder (T039)

11. **Nem o dataset local nem o schema nomeiam em qual estágio um run em
    execução está — o campo é de outra feature desta onda, ainda não
    executada.** `InvestigationSummary` (`platform/incidents/investigation_summary.py`,
    confirmado também em `fixtures/contract/openapi.json` e no único run
    `running` do cenário `populated`, `run-0003` em
    `fixtures/scenarios/populated/runs.json`) carrega `status`, mas nada que
    nomeie um estágio. O campo que faria isso é o mesmo que o `controle.md`
    da feature 030 desta onda já cita como pendente, pelo mesmo motivo, para
    a lista de runs — a mesma dívida, encontrada de novo aqui de forma
    independente. `stageTreatment` foi escrito para aceitar esse valor
    (`currentStageName: string | undefined`) desde já; o dia em que o campo
    existir, ligar um valor real nele é a única mudança que a tela precisa.
12. **Rodar `tests/contract/console/test_console_visual_regression.py` para
    validar a edição de `screens.json` foi um comando de comparação visual,
    o que o despacho pede para não rodar.** Buscando um teste estrutural
    leve para `visual/screens.json`, rodei junto (por engano, não por
    necessidade) o par de testes de contrato que efetivamente builda e
    compara toda a suíte visual contra as baselines comitadas
    (`python -m tools.console_visual compare`, chamado por dentro do teste,
    não pelo próprio agente). Nenhuma baseline foi escrita — é uma
    comparação, não um aceite, e `git status` depois confirma zero arquivo
    de `console/visual/` além do `screens.json` que este relatório já
    documenta. O resultado, honesto porque já rodou: 32 telas *fora do
    escopo desta tarefa* (`administration`, `agent-autonomy`, `agent-tools`,
    `agent-team`, `autonomy-posture/rules-windows/guardrails`, `gallery` nos
    quatro breakpoints e dois temas, `incident`, `integrations`,
    `integrations-panel`, `knowledge-topology`, `run-detail` claro/escuro,
    `run-detail-live` claro/escuro, `runs` claro/escuro, `shell` em três
    breakpoints e dois temas, `signals`, `single-sign-on`,
    `machine-tokens`, `models-providers`, `notifications`) já não batem
    com sua própria baseline comitada, medido no ambiente desta worktree.
    `agent-1440-light` — a única tela que esta tarefa tocou — não está entre
    as 32, porque seu estado é `pending` e o teste a ignora por isso, exatamente
    como deveria. As 32 incluem três abas de O agente que esta tarefa não
    tocou (`agent-autonomy`, `agent-tools`, `agent-team`) e telas sem
    nenhuma relação com este slot (`gallery`, `shell`, `administration`),
    então a causa é do ambiente (provavelmente renderização de fonte ou
    driver gráfico diferente de onde as baselines foram capturadas), não
    desta mudança — mas não investiguei mais fundo porque julgamento visual
    não é meu, e não repito o comando. Nomeado aqui para quem for revisar
    baselines a seguir, não corrigido.
