---
name: data_platform-rabbitmq
display_name: RabbitMQ investigation
description: RabbitMQ's management API: which queues exist and how deep they are, which is the first question of every message-backlog incident.
domain: data_platform
applies_when:
  alert_sources: [rabbitmq]
  tags: [data_platform, queues, amqp]
directs_tools:
...
1. **Shape before detail.** Call `rabbitmq_pipeline_health` over the symptom
...
4. **Read where the counts point.** Only now call
   `rabbitmq_recent_failures`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for RabbitMQ
...
- `state` is `running`, `idle`, or `flow`; `flow` means the broker is applying back pressure to publishers and is the state most worth noticing.
- `messages` is the queue depth, `messages_ready` and `messages_unacknowledged` split it, and a large unacknowledged count means a stuck consumer rather than a slow one.
- Paging is page-numbered from one with `page_size`, and the totals are in the response envelope rather than a header.
