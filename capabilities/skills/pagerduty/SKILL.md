...
display_name: PagerDuty investigation
description: Who is being paged and for what: the incidents PagerDuty is holding, one incident's log, and the acknowledgement that stops the escalation clock.
domain: incident
applies_when:
  alert_sources: [pagerduty]
  tags: [incident, paging, oncall]
directs_tools:
...
# PagerDuty investigation

## Order of operations

1. **Shape before detail.** Call `pagerduty_incident_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `pagerduty_incident_timeline`, narrowed to the group the distribution
...
`pagerduty_acknowledge_incident` changes something outside NinjaSRE. It is
...
- **A question another system answers in one call.** Reaching for PagerDuty
...
- `statuses[]` is a repeated parameter — `triggered`, `acknowledged`, `resolved` — and omitting it returns everything including months of resolved incidents.
- Grouping by `service.summary` answers 'is this one service or several', which is the first question a page raises.
- `urgency` is `high` or `low`, and a low-urgency incident is not paging anybody, which changes what a responder should do about it.
