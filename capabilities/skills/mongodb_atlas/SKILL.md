---
name: database-mongodb_atlas
display_name: MongoDB Atlas investigation
description: The Atlas control plane: which clusters and processes exist in a project, and the slow-query entries Atlas's performance advisor has collected.
domain: database
applies_when:
  alert_sources: [mongodb_atlas]
...
1. **Shape before detail.** Call `mongodb_atlas_session_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `mongodb_atlas_slow_queries`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for MongoDB Atlas
...
- Every v2 endpoint needs `Accept: application/vnd.atlas.2023-02-01+json`, and a missing version header is a 406 rather than a 400.
- `typeName` distinguishes `REPLICA_PRIMARY` from `REPLICA_SECONDARY`, which is the grouping that answers 'did something fail over'.
- Paging is page-numbered from one with `itemsPerPage`, and the total is in `totalCount` rather than in a header.
