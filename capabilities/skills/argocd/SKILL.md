---
name: cicd-argocd
display_name: Argo CD investigation
description: What Argo CD has actually applied: which applications are synced and healthy, and the ones that are not.
domain: cicd
applies_when:
  alert_sources: [argocd]
  tags: [cicd, gitops, deployments]
directs_tools:
...
# Argo CD investigation

## Order of operations

1. **Shape before detail.** Call `argocd_pipeline_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `argocd_failed_runs`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Argo CD
...
- `status.sync.status` is `Synced` or `OutOfSync`; `status.health.status` is `Healthy`, `Degraded`, `Progressing`, or `Missing`. They answer different questions and an incident usually needs both.
- `status.operationState.finishedAt` is when the last sync completed, which is the timestamp to line up against the start of a symptom.
- The selector parameter takes Kubernetes label syntax, which is what narrows a large installation to the applications that are not where they should be.
