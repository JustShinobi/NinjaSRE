---
name: logstore-better_stack
display_name: Better Stack investigation
description: Better Stack's log search and the monitors it is currently reporting as down, for teams using it as both log store and uptime checker.
domain: logstore
applies_when:
  alert_sources: [better_stack]
  tags: [logstore, logs, uptime]
directs_tools:
  - better_stack_log_statistics
  - better_stack_sample_logs
requires:
  integrations: [better_stack]
---

# Better Stack investigation

## Order of operations

1. **Shape before detail.** Call `better_stack_log_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `better_stack_sample_logs`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Better Stack
...
## Better Stack specifics

- Query syntax is Better Stack's own simplified form: bare terms match the message, and `level:error` narrows by field.
- `from` and `to` are ISO 8601; a window given the wrong way round returns nothing and reads as a quiet period.
- `order=newest_first` is what makes a sample the most recent lines rather than the oldest in the window.
