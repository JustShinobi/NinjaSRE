---
name: cloud_control_plane-aws_ecs
display_name: AWS ECS investigation
description: The ECS control plane: which clusters this account runs and which task definitions have been registered, which is where a deployment shows up.
domain: cloud_control_plane
applies_when:
  alert_sources: [aws_ecs]
  tags: [cloud_control_plane, aws, containers]
directs_tools:
  - aws_ecs_resource_inventory
  - aws_ecs_recent_changes
requires:
  integrations: [aws_ecs]
---
...
1. **Shape before detail.** Call `aws_ecs_resource_inventory` over the symptom
...
4. **Read where the counts point.** Only now call
   `aws_ecs_recent_changes`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for AWS ECS
...
- `sort=DESC` on task definitions is what makes the newest revision first, which is the one a deployment just registered.
- A task-definition ARN ends in `:revision`, and the revision number is the deployment counter.
- Cluster ARNs carry the region and account, which is what links them to CloudTrail.
