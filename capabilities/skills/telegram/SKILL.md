...
display_name: Telegram investigation
description: A Telegram chat used as an alerting channel: what has arrived recently, and a finding delivered into it.
domain: communication
applies_when:
  alert_sources: [telegram]
  tags: [communication, chat, alerts]
directs_tools:
...
1. **Shape before detail.** Call `telegram_recent_messages` over the symptom
...
4. **Read where the counts point.** Only now call
   `telegram_recent_messages`, narrowed to the group the distribution
...
`telegram_post_message` changes something outside NinjaSRE. It is
...
- **A question another system answers in one call.** Reaching for Telegram
...
- `chat_id` is a number for a private chat and a `@channelusername` for a public channel; both are accepted and only one is stable.
- `message.chat.type` is `private`, `group`, `supergroup`, or `channel`, which is the grouping that says what kind of destination this is.
- Telegram rate-limits to roughly one message a second per chat and twenty a minute to a group, which is low enough to matter during an incident.
