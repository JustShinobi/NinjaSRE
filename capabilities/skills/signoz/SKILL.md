---
name: tracing-signoz
display_name: SigNoz investigation
description: SigNoz's span store: where latency and errors concentrate for a service, and the slowest traces behind that concentration.
domain: tracing
applies_when:
  alert_sources: [signoz]
  tags: [tracing, traces, apm]
directs_tools:
...
1. **Shape before detail.** Call `signoz_trace_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `signoz_slow_traces`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for SigNoz
...
- The v3 query API is one POST that serves traces, metrics, and logs; `dataSource` is what selects which.
- `durationNano` is the span duration field, and ordering by it descending is what makes the second capability exemplars.
- Grouping by `name` separates operations within a service, which is where latency usually concentrates.
