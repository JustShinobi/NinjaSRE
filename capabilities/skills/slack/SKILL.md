...
display_name: Slack investigation
description: The conversation an incident is already happening in: what responders have said, and a finding delivered where they will read it.
...
1. **Shape before detail.** Call `slack_recent_messages` over the symptom
...
4. **Read where the counts point.** Only now call
   `slack_recent_messages`, narrowed to the group the distribution
...
`slack_post_message` changes something outside NinjaSRE. It is
...
- **A question another system answers in one call.** Reaching for Slack
...
- `oldest` and `latest` are Slack timestamps — Unix seconds with a microsecond fraction — not ISO 8601.
- `subtype` distinguishes a human message from a join, a bot post, or a thread broadcast, and grouping by it is what separates conversation from noise.
- A thread's replies are not in the channel history; `conversations.replies` is a separate call keyed on the parent `ts`.
