---
name: logstore-splunk
display_name: Splunk investigation
description: SPL search against Splunk, counted before it is read, for the estates whose logs have been in Splunk longer than the services producing them.
domain: logstore
applies_when:
  alert_sources: [splunk]
  tags: [logstore, logs, spl]
directs_tools:
...
1. **Shape before detail.** Call `splunk_log_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `splunk_sample_logs`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Splunk
...
- SPL: `search index=app sourcetype=checkout error | head 100`. The leading `search` command is required by the export endpoint.
- `earliest_time` and `latest_time` accept Splunk's relative syntax — `-1h`, `@d` — as well as epoch seconds.
- Field names come from the sourcetype's extraction, so the grouping field is whatever this estate's props.conf called it.
