---
name: logstore-hermes
display_name: Hermes investigation
description: Hermes log tailing and classification: what a stream is currently emitting, grouped by the class its own model assigned.
domain: logstore
applies_when:
  alert_sources: [hermes]
  tags: [logstore, logs, classification]
directs_tools:
...
1. **Shape before detail.** Call `hermes_log_statistics` over the symptom
...
4. **Read where the counts point.** Only now call
   `hermes_sample_logs`, narrowed to the group the distribution
...
- **A question another system answers in one call.** Reaching for Hermes
...
- Queries are substring matches with `field:value` narrowing, the same shape most hosted log products use.
- `classification` is Hermes's own grouping and is the field worth counting first; `severity` is the emitter's own and is worth counting second.
- `next_cursor` is opaque and expires, so a paused walk restarts rather than continuing.
