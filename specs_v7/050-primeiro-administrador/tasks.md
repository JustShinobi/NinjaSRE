...
- [ ] T002 Registrar, a partir do banco de staging e não de um documento: se
...
- [ ] T003 Levantar um deployment limpo por `deploy/compose/docker-compose.yml`
...
- [ ] T004 Em `tests/unit/platform/identity/`, teste: um deployment sem conta
...
- [ ] T005 [P] No mesmo lugar, teste: um deployment sem conta de ambiente e sem
...
- [ ] T006 [P] Teste: criar um principal pela rota de identidade **não**
      registra abertura. Guarda a segunda metade da invariante.

- [ ] T007 [P] Teste: a conta de ambiente continua sendo resolvida na
...
- [ ] T008 [P] Teste: com abertura registrada e sem conta de ambiente, o
...
- [ ] T009 [P] Teste: a abertura é lida por tentativa de sign-in — registrar a
      abertura com o objeto de sign-in já construído passa a permitir a entrada
      sem reconstruir nada.

- [ ] T010 Em `tests/unit/platform/identity/`, teste da criação de
...
- [ ] T011 [P] Teste: criar administrador com um nome que já existe é recusado,
      e a recusa diz como rotacionar.

- [ ] T012 [P] Teste: rotação explícita troca a passphrase; a nova entra, a
      antiga é recusada.

- [ ] T013 [P] Teste: criar um segundo administrador num deployment já
      administrado funciona e **não** cria uma segunda abertura.

- [ ] T014 [P] Teste: com o identity provider ativo, a criação de administrador
      é recusada, a recusa nomeia o caminho de emergência, e não existe opção
      de override.

- [ ] T015 Em `tests/unit/surfaces/cli/commands/test_setup.py`, testes do
...
- [ ] T016 [P] Teste: a saída do comando, em texto e em dados, não contém a
...
- [ ] T017 Em `tests/contract/persistence/test_identity_repository.py`, teste
...
- [ ] T018 [P] No mesmo arquivo, teste: a busca por endereço com endereço vazio
      não devolve um principal sem endereço.

- [ ] T019 [P] No mesmo arquivo, teste: dois principals com o mesmo endereço em
...
- [ ] T020 Em `tests/contract/persistence/`, teste de concorrência contra o
...
- [ ] T021 [P] Teste: duas réplicas simuladas subindo juntas contra o mesmo
      banco, com uma delas já tendo aberto a porta, não produzem dois convites
      impressos.

- [ ] T022 Em `tests/unit/platform/startup/test_bootstrap.py`, testes do
...
- [ ] T023 [P] Teste do convite: o bloco impresso nomeia o comando completo, e
...
- [ ] T024 [P] Teste de que a credencial de bootstrap continua não aparecendo
...
- [ ] T025 Em `tests/contract/deployment/test_first_run_sign_in.py`, teste do
...
- [ ] T026 [P] Teste: a troca sem passphrase é recusada.

- [ ] T027 [P] Teste: o administrador criado pela troca e o dono do token
      durável são o mesmo principal.

- [ ] T028 [P] Teste: a troca é recusada quando já há abertura registrada,
      mesmo com o arquivo de credencial presente no host.

- [ ] T029 [P] Teste: a troca é recusada com identity provider ativo.

- [ ] T030 [P] Teste: a ordem preservada — a credencial antiga só é revogada
      depois de a substituta existir — e a segunda troca com a mesma credencial
      continua recusada.

- [ ] T031 [P] Teste da tabela de rotas: a rota de disponibilidade está
      declarada, é pública por declaração, e a permissão exigida pela rota de
      troca não foi alargada.

- [ ] T032 Escrever `console/tests/e2e/primeiro-administrador.acceptance.spec.ts`
...
- [ ] T033 [P] Em `console/tests/unit/`, testes do bloco de aviso: aparece no
...
- [ ] T034 [P] Em `console/tests/unit/`, teste-guarda: a recusa de sign-in é
...
- [ ] T035 Declarar na porta de identidade a operação de ler e a de registrar a
...
- [ ] T036 Implementar as duas operações no fake, com a unicidade da abertura
      valendo lá também.

- [ ] T037 Implementar as duas operações no repositório Postgres. A inserção da
      abertura não sobrescreve e é seguida de leitura de volta: quem leu a linha
      de outro perdeu a corrida.

- [ ] T038 Escrever a revisão de migração que cria a tabela da abertura, com
      chave primária por deployment e `downgrade` que a remove.

- [ ] T039 Escrever a revisão de migração que tira os principals sem endereço da
...
- [ ] T040 Escrever o `downgrade` dessa segunda revisão: ele reconverte e
...
- [ ] T041 Ajustar o repositório Postgres para escrever a dobra ausente quando
...
- [ ] T042 Fazer a colisão real de endereço sair pela fronteira que já traduz
...
- [ ] T043 Confirmar T017–T019 verdes contra os dois backends, e registrar a
...
- [ ] T044 Criar o módulo de identidade que concentra a regra: ler a abertura,
...
- [ ] T045 Usar, para a exclusão do caminho de criação, o mesmo tipo de
...
- [ ] T046 Ler o estado ativo do identity provider da mesma fonte que o console
      já lê, pelo serviço de configuração. Não introduzir uma segunda noção de
      "tem identity provider".

- [ ] T047 Escrever as recusas novas como erros de domínio com mensagem para
...
- [ ] T048 Redefinir a porta em `platform/identity/local_accounts.py`: o ramo
...
- [ ] T049 Manter a resolução da conta de ambiente na construção do processo, e
...
- [ ] T050 Implementar `ninjasre setup admin` em
...
- [ ] T051 **[composição]** Confirmar que o comando está alcançável no CLI que a
...
- [ ] T052 Confirmar T010–T016 verdes.

---

## Phase 7: O boot, o convite e o seam

- [ ] T053 Colocar o texto do comando canônico num lugar só, junto das outras
      constantes de first-run, para que o convite impresso, a mensagem da tela e
      a documentação não possam divergir em redação.

- [ ] T054 Gatear a emissão da credencial de bootstrap: nada é emitido quando há
...
- [ ] T055 Reescrever o bloco impresso no boot: ele nomeia o comando completo e
      **deixa de afirmar** que a credencial serve para o formulário de sign-in.
      Continua impresso e não logado.

- [ ] T056 Fazer a troca da credencial de bootstrap criar um administrador com
...
- [ ] T057 Recusar a troca quando já há abertura registrada e quando o identity
      provider está ativo, preservando a ordem de revogação e a recusa da
      segunda troca.

- [ ] T058 Regenerar o contrato HTTP publicado e conferir que a mudança da rota
      de troca aparece nele.

- [ ] T059 Confirmar T022–T031 verdes.

---

## Phase 8: O console

- [ ] T060 Declarar e servir a rota pública de disponibilidade: um fato
...
- [ ] T061 Ler esse fato no console pela leitura sem cache que já existe, e não
      por uma segunda forma de falar com o gateway.

- [ ] T062 Acrescentar o bloco de aviso à tela de sign-in, acima do formulário,
...
- [ ] T063 Acrescentar o mesmo bloco à tela de first-run, no mesmo estado, pela
...
- [ ] T065 Escrever, para o relatório final, o bloco de chaves i18n novas com
...
- [ ] T072 **[serving]** Depois do `make deploy-stg` do slot (que é do
...
- [ ] T073 **[serving]** Rodar contra o staging apenas os acceptance marcados
      como staging-safe, e anexar as capturas.

- [ ] T074 Consultas de contagem no banco de staging para a evidência: quantas
...
- [ ] T076 Escrever o relatório final: o bloco de chaves i18n de T065, as
