...
- [ ] T001 Rodar `make verify` na árvore de partida e guardar o log **fora do
...
- [ ] T002 Confirmar, contra o cliente gerado (`console/src/api/schema.ts`) e
...
- [ ] T003 Registrar as contagens de partida a partir da árvore, não de um
...
- [ ] T004 [P] Acrescentar a `fixtures/scenarios/populated/runs.json` e
...
- [ ] T005 [P] Acrescentar um run **com headline e report**, o caso do dia
      ...
      o recorte.
- [ ] T006 [P] Acrescentar um run **hostil**: report contendo HTML cru, uma
...
- [ ] T007 [P] Estender `fixtures/scenarios/populated/run-replay.json` com o
      ...
      contar.
- [ ] T008 [P] Garantir que um dos runs novos esteja **vinculado a um incidente**
...
- [ ] T009 Rodar a suíte de console inteira sobre as fixtures estendidas e
...
- [ ] T010 Criar `console/tests/e2e/010-leitura-do-relato.acceptance.spec.ts`
...
- [ ] T011 Marcar como **seguros para staging** os casos de leitura pura que podem
...
- [ ] T012 Rodar T010 e **confirmar o vermelho de cada grupo**, registrando a
...
- [ ] T013 [US4] Estender `console/tests/unit/design/status.test.ts`: todo valor
...
- [ ] T014 [US4] Estender o mesmo arquivo: um status que nenhuma lista conhece
      **não** é run vivo. Confirmar vermelho — hoje a decisão é a negação de
      "terminado" e um desconhecido passa por vivo.
- [ ] T015 [US4] Acrescentar em `console/src/design/status.ts` os status
...
- [ ] T016 [US4] Trocar a decisão de "vivo" de negativa para afirmativa: um run é
      vivo quando seu status está no conjunto vivo declarado. Verde em T013 e
      T014.
- [ ] T017 [US4] Estender `console/tests/unit/surfaces/run-detail.test.tsx`: um
...
- [ ] T018 [US1] Criar `console/tests/unit/surfaces/run-subject.test.ts` contra o
...
- [ ] T019 [US1] Criar `console/src/surfaces/run-subject.ts` como função pura
...
- [ ] T020 [US1] Fazer `run-detail.tsx` obter o título desse módulo, em lugar de
      `readFailure(...).title`. O `PageHeader` recebe o texto exibido; o texto
      completo vira tooltip quando houve recorte.
- [ ] T021 [US1] Fazer `runs/[runId]/page.tsx` obter o título da aba do mesmo
...
- [ ] T022 [US2] Fazer `runs.tsx` obter a célula de sujeito do mesmo módulo, e
...
- [ ] T023 [US1] Estender `console/tests/unit/surfaces/runs.test.tsx`: nenhuma
...
- [ ] T024 [US1] Acrescentar um teste de superfície que afirma que **as três
...
- [ ] T025 [US1] Atualizar o docstring de `console/src/surfaces/failures.ts` para
...
- [ ] T026 [US3] Resolver e instalar as dependências de renderização decididas no
...
- [ ] T027 [US3] Rodar `make console-lockfile` e confirmar que manifesto e
      lockfile se descrevem.
- [ ] T028 [US3] Acrescentar um teste de repositório que afirma, lendo o
...
- [ ] T029 [US3] Criar `console/tests/unit/surfaces/report.test.tsx` contra o
...
- [ ] T030 [US3] Criar `console/src/surfaces/report.tsx`: componente de servidor,
...
- [ ] T031 [US3] Acrescentar um teste de repositório que afirma que
      ...
      precisa de um gate.
- [ ] T032 [US3] Substituir o corpo do painel do relato em `run-detail.tsx`: o
...
- [ ] T033 [US3] Estender `run-detail.test.tsx` para o painel: report renderizado
...
- [ ] T034 [US3] Rodar `tests/e2e/network.spec.ts` contra o build com o run
      hostil carregado e confirmar que nenhum host fora do deployment aparece.
- [ ] T035 [US3] Rodar `make console-budget` e o orçamento de rolagem, e
      ...
      seguir.
- [ ] T036 [US3] Confirmar que o renderizador roda no componente de servidor —
...
- [ ] T037 [US5] Estender `run-detail.test.tsx`: um run com quatro chamadas
...
- [ ] T038 [US5] Remover de `run-detail.tsx` a injeção do relato no corpo do
      ...
      Verde em T037.
- [ ] T039 [US5] Conferir que `tests/contract/console/` continua verde: a
...
- [ ] T040 [US5] Estender `run-detail.test.tsx` para o custo: um run com turnos
      ...
      sejam verdade.
- [ ] T041 [US6] Estender `run-detail.test.tsx` para os vínculos: um run cujo
...
- [ ] T042 [US6] Fazer o painel de vínculos ler os recursos que o registro do run
...
- [ ] T043 [US6] Se o registro mergeado **não** carregar recursos de forma
...
- [ ] T044 [US2] Estender `runs.test.tsx`: nenhuma linha-meta de run exibe o mesmo
      ...
      duração.
- [ ] T045 [US2] Aplicar na lista a mesma regra que o subtítulo do detalhe já
      usa: filtrar o slot vazio antes de compor a linha. Verde em T044.
- [ ] T046 [US2] Conferir que a ordenação da lista não passou a ordenar por
...
- [ ] T047 [US7] Remover as entradas de rota de runs da tabela de exceções da
...
- [ ] T048 [US7] Rodar a suíte transversal inteira contra o build e confirmar que
...
- [ ] T049 [US7] Conferir que nenhuma outra rota perdeu ou ganhou exceção nesta
...
- [ ] T050 Acrescentar ao catálogo `en` toda string nova: o separador do nome
      ...
      componente.
- [ ] T051 Acrescentar as mesmas chaves ao catálogo `pt-BR`, porque esta feature
      toca o catálogo e a regra é que o par ande junto.
- [ ] T052 Rodar o teste de completude de catálogo e o de literais não traduzidos.
...
- [ ] T056 Rodar `make verify` inteiro e comparar com o log de T001. O resultado é
      uma comparação, não uma alegação.
- [ ] T057 Rodar o acceptance completo contra o build de produção e registrar os
      seis grupos verdes, com o vermelho de T012 ao lado, no `controle.md`.
- [ ] T058 Capturar screenshots **full-page em 1920×1080** das duas telas
      retocadas — a lista e o detalhe, este último com o run que tem report — e
      gravá-los em `specs_v7/010-leitura-do-relato/evidence/`.
- [ ] T059 Entregar ao orquestrador, no relatório final, o bloco de coordenação
...
- [ ] T060 Marcar quais casos do acceptance são seguros para rodar contra o
...
- [ ] T061 Depois de `make deploy-stg`, rodar contra
      `https://stg-ninjasre.lan.kyo.ninja` os casos marcados como seguros —
      leitura pura, nenhuma escrita.
- [ ] T062 Abrir o run mais recente do staging e conferir, na tela real: o
      ...
      para `evidence/`.
- [ ] T063 Abrir a lista de runs do staging e conferir que nenhuma das linhas
      mostra markdown truncado nem o texto de "não registrado" na coluna de
      sujeito. Screenshot full-page para `evidence/`.
- [ ] T064 Rodar a suíte transversal contra o staging e confirmar que as duas
      rotas de runs passam lá também.
- [ ] T065 Registrar no `controle.md`, item a item, quais alegações foram
...
- [ ] O acceptance foi visto **vermelho** em todos os seis grupos, com as
      mensagens registradas, antes de qualquer mudança de tela.
- [ ] O acceptance passa contra o build de produção, nos seis grupos.
...
- [ ] A tabela de exceções da suíte transversal não nomeia rota de runs, e a
...
- [ ] `dangerouslySetInnerHTML` continua exclusivo do arquivo de layout, provado
      por teste.
- [ ] O plugin de HTML cru não está no manifesto, provado por teste.
- [ ] As baselines visuais das duas telas foram recapturadas, revisadas imagem a
      imagem, e aceitas com razão escrita no registro.
- [ ] Os dois arquivos de teste unitário existentes foram **estendidos**; nenhum
      caso foi duplicado.
- [ ] `make verify` verde, comparado contra o log da linha de base.
- [ ] O bloco de coordenação do slot foi entregue no relatório final.
