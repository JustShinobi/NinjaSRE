...
- [ ] **T002** Registrar o estado de partida do Proxmox no staging, lido do
...
- [ ] **T003** [Teste] Rodar a suíte de transporte do Proxmox que já existe e
...
- [ ] **T004** [Teste] Escrever, em `tests/unit/platform/credentials/`, o teste
...
- [ ] **T005** Criar `platform/credentials/proxy/trust.py` com o vocabulário. É a
...
- [ ] **T006** Reescrever `integrations/proxmox/certificates.py` como declaração
...
- [ ] **T007** Rodar as checagens de contrato de import. O tier do proxy não pode
...
- [ ] **T008** [Teste] Em `tests/unit/platform/credentials/`, escrever o teste das
...
- [ ] **T009** [Teste] Escrever o teste das três frases: não confiado nomeia o
...
- [ ] **T010** Acrescentar a razão e as três formas de recusa em
...
- [ ] **T011** Fazer a razão nova atravessar `engine.py` sem ser reembalada.
...
- [ ] **T012** [Teste] Em `tests/contract/` (contrato do proxy), escrever o teste
...
- [ ] **T013** [Teste] No mesmo lugar, o teste do pin quebrado: fingerprint
...
- [ ] **T014** [Teste] O teste do nome que não confere: autoridade fornecida que
      valida a cadeia, endereço configurado que o certificado não nomeia ⇒ a
      terceira mensagem, não a primeira. Confirmar vermelho.

- [ ] **T015** [Teste] O teste de que o padrão não afrouxou: sem declaração e sem
...
- [ ] **T016** Implementar em `gateway/proxy/sender.py` a resolução de contexto
...
- [ ] **T017** Registrar no `controle.md` a decisão de forma que a pinagem com a
...
- [ ] **T018** [Teste] Em `tests/unit/platform/config_service/`, escrever o teste
...
- [ ] **T020** [Teste] O teste da varredura de valor com forma de segredo que o
...
- [ ] **T021** [Teste] Em `tests/unit/platform/identity/`, escrever o teste da
...
- [ ] **T022** Acrescentar a permissão em `platform/identity/permissions.py` e ao
...
- [ ] **T023** [Teste] Em `tests/contract/` (rota de integrações), escrever o
...
- [ ] **T024** [Teste] O teste que a experiência desta base de código pede: gravar
...
- [ ] **T025** Implementar a escrita em `gateway/http/integration_endpoints.py`,
...
- [ ] **T026** [Teste] O teste do evento de auditoria da escrita: um evento por
...
- [ ] **T027** Emitir o evento na rota, com a ação e o tipo de recurso nomeados
...
- [ ] **T028** [Teste] O teste dos dois escalares novos na linha de resolução:
...
- [ ] **T029** Implementar os dois campos em `platform/credentials/proxy/audit.py`
      e preenchê-los em `engine.py`.

- [ ] **T030** [Teste] O teste da invalidação por troca de endereço: declaração
...
- [ ] **T031** [Teste] Em `tests/unit/gateway/proxy/`, escrever o teste da
...
- [ ] **T032** Passar o registro em `gateway/proxy/composition.py`. O remetente
      não pode conseguir um registro por outro caminho: a composição é a única
      maneira.

- [ ] **T033** [Teste] O teste da leitura da configuração: a confiança sai da
...
- [ ] **T034** Implementar a leitura em `gateway/proxy/hosts.py`, ao lado da
      leitura de host que já está lá, e ligar no ciclo de
      `gateway/proxy/__main__.py`. Um ciclo, uma leitura, os dois reconstruídos
      juntos.

- [ ] **T035** [Teste] O teste de resiliência que o ciclo já promete: uma
...
- [ ] **T036** [Teste] Escrever o teste do braço novo na tabela de mensagens do
...
- [ ] **T037** Acrescentar o braço em `integrations/proxmox/verifier.py`.

- [ ] **T038** Fazer a verificação profunda relatar em uma linha a forma de
...
- [ ] **T041** Atualizar `integrations/proxmox/docs.md`: além das três formas
...
- [ ] **T042** [Teste] Arquitetura: existe exatamente **um** lugar na árvore capaz
...
- [ ] **T043** [Teste] Arquitetura: nenhum schema de credencial, de configuração
      ou de capacidade tem campo booleano que desligue verificação.

- [ ] **T044** [Teste] Nenhuma capacidade do catálogo lê ou escreve declaração de
      confiança.

- [ ] **T045** [Teste] A política de rede dos sandboxes é idêntica à de antes
      desta feature — sandbox não fala com o proxy e isso não mudou. "Não
      mexemos" não é evidência.

- [ ] **T046** Rodar `make verify` inteiro e comparar com a linha de base de T001.
...
- [ ] **T047** Declarar a confiança do Proxmox real pela interface do staging.
