...
| **Single datastore** | One Postgres, reached through the seventeen repository ports. `make check-raw-sql` fails on SQL, Cypher, or a database driver imported outside `platform/persistence/`. |
